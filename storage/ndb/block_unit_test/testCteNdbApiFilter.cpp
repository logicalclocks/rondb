...
/* ------------------------------------------------------------------ */
/* Test 24: CTE_LOOKUP filter — BRANCH_LINKED_OP_LINKED string         */
/*          column-vs-column (cte_output_string_colvscol_plan.md S1)   */
/*                                                                     */
/* CTE: SELECT grp, MIN(tag) AS mn, MAX(tag) AS mx                     */
/*      FROM filter_src GROUP BY grp                                   */
/* filter_src is reseeded so tag varies within some groups:            */
/*   grp=1: alpha/delta        -> mn < mx  (2 main rows)               */
/*   grp=2: beta/beta          -> mn = mx  (2 main rows)               */
/*   grp=3: gamma/omega/kappa  -> mn < mx  (3 main rows)               */
/*                                                                     */
/* Two sub-cases run the same query shape with different filters on    */
/* the CTE_LOOKUP op, comparing buffer positions 1 (mn) and 2 (mx)     */
/* directly via the new one-instruction BRANCH_LINKED_OP_LINKED:       */
/*   (a) accept mn < mx  -> reject via branch_linked_linked_le         */
/*       (inverted convention: _le branches when L >= R) -> COUNT=5    */
/*   (b) accept mn = mx  -> reject via branch_linked_linked_ne         */
/*       -> COUNT=2                                                    */
/* Result delivery via aggregator (COUNT(*) main agg), as in Test 22.  */
/* ------------------------------------------------------------------ */

static int
testCteLookupFilterLinkedLinkedString(Ndb *ndb, MYSQL *conn)
{
  printf("Test 24: CTE_LOOKUP filter (linked-vs-linked string) ... ");
  fflush(stdout);

  /* Reseed filter_src with per-group tag variety (the default seed
   * has one distinct tag per group, making mn == mx everywhere). */
  if (sqlExec(conn, "DELETE FROM filter_src") != 0 ||
      sqlExec(conn,
              "INSERT INTO filter_src VALUES "
              "(1,1,10,'alpha'),(2,1,20,'delta'),"
              "(3,2,30,'beta'), (4,2,40,'beta'),"
              "(5,3,50,'gamma'),(6,3,60,'omega'),(7,3,70,'kappa')") != 0) {
    printf("FAILED (reseed filter_src)\n");
    return -1;
  }
  ...
  /* In-memory virt table: [grp INT PK, mn CHAR(8), mx CHAR(8)] —
   * same synthetic-table pattern as Test 22. */
  NdbDictionary::Table virtTabSyn("__cte_strcvc_virt");
  {
  ...
  struct SubCase {
    const char *name;
    bool useLe;      /* true: reject via _le (accept mn < mx) */
    Int64 expected;
  };
  const SubCase cases[] = {
      { "mn<mx", true, 5 },
  ...
  for (const SubCase &sc : cases) {
    /* CTE 0 = GROUP BY grp, MIN(tag), MAX(tag). */
    NdbAggregator cteAgg(srcTab);
    if (!cteAgg.GroupBy("grp") ||
        !cteAgg.LoadColumn(tagSrcCol->getAttrId(), 0) ||
        !cteAgg.Min(0, 0) ||
        !cteAgg.Max(1, 0) ||
        !cteAgg.Finalize()) {
    ...
    /* Main aggregator: COUNT(*) over surviving main rows. */
    NdbAggregator mainAgg(srcTab);
    if (!mainAgg.LoadUint64(1, 0) ||
        !mainAgg.Count(0, 0) ||
        !mainAgg.Finalize()) {
    ...
    /* Filter: one BRANCH_LINKED_OP_LINKED comparing buffer positions
     * 1 (mn) and 2 (mx); position 0 is the GB key. */
    Uint32 codeBuf[64];
    NdbInterpretedCode filterCode(/*table=*/nullptr, codeBuf,
                                  sizeof(codeBuf) / sizeof(codeBuf[0]));
    const Uint32 REJECT = 0;
    const Uint32 typeId =
        static_cast<Uint32>(NdbDictionary::Column::Char);
    int rc = sc.useLe
        ? filterCode.branch_linked_linked_le(1, 2, typeId, csNumber,
                                             8, 8, REJECT)
        : filterCode.branch_linked_linked_ne(1, 2, typeId, csNumber,
                                             8, 8, REJECT);
    if (rc != 0 ||
        filterCode.interpret_exit_ok() != 0 ||
        filterCode.def_label(REJECT) != 0 ||
    ...
    /* Main: scan filter_src; lookupCte by grp with the string
     * col-vs-col filter. */
    const NdbQueryTableScanOperationDef *mainScanOp = qb->scanTable(srcTab);
    if (mainScanOp == nullptr) {
    ...
    cteLookupOpts.setInterpretedCode(filterCode);
    cteLookupOpts.setAggregation(mainAgg);
    if (qb->lookupCte(0, 3, virtTab, cteKey, &cteLookupOpts) == nullptr) {
  ...
  printf("OK (mn<mx COUNT=5, mn=mx COUNT=2)\n");
  return 0;
}
...
/* ------------------------------------------------------------------ */
/* Test 26: AVG at scale — sliced finalize + redistribute              */
/*          (cte_avg_plan.md V5 large-cardinality case)                */
/*                                                                     */
/* 1500 groups x 2 rows: group g holds val = g and val = g + 2, so     */
/* avg = g + 1 exactly (all values exact in double).  1500 groups      */
/* exceeds ZCTE_AVG_FINALIZE_BATCH (1024), so the owner-side finalize  */
/* walk takes at least two CONTINUEB slices; on a multi-node cluster   */
/* (ndb_push_agg_dist) the groups additionally arrive at the owner     */
/* through the redistribute + inbound-merge path before the divide.    */
/*                                                                     */
/* Sub-cases pin both slice-visited regions of the hash:               */
/*   (a) accept avg > 1000.0 -> g in [1000, 1500] = 501 groups         */
/*       x 2 main rows -> COUNT=1002                                   */
/*   (b) accept avg < 100.0  -> g in [1, 98] = 98 groups               */
/*       x 2 main rows -> COUNT=196                                    */
/* ------------------------------------------------------------------ */

static int
testCteLookupAvgLarge(Ndb *ndb, MYSQL *conn)
{
  printf("Test 26: CTE AVG at scale (sliced finalize) ... ");
  fflush(stdout);

  /* Re-seed cte_src with 1500 groups x 2 rows (bulk-insert in chunks
   * of 100 rows, mirroring the Test 19 seeding pattern). */
  if (sqlExec(conn, "DELETE FROM cte_src") != 0) {
    printf("FAILED (delete)\n");
    return -1;
  }
  {
    ...
    for (int g = 1; g <= 1500; g++) {
      for (int r = 0; r < 2; r++) {
        ...
        const int pk = 2 * g - 1 + r;
        const int val = (r == 0) ? g : g + 2;
        pos += snprintf(sql + pos, sizeof(sql) - pos, "(%d,%d,%d)",
                        pk, g, val);
        rows++;
        if (rows % 100 == 0) {
          if (sqlExec(conn, sql) != 0) {
    ...
    if (!first && sqlExec(conn, sql) != 0) {
      printf("FAILED (seed insert tail)\n");
      return -1;
    }
  }
  ...
  NdbDictionary::Table virtTabSyn("__cte_avg_lg_virt");
  {
    NdbDictionary::Column grpCol;
    grpCol.setName("grp");
    grpCol.setType(NdbDictionary::Column::Int);
  ...
  const SubCase cases[] = {
      { "avg>1000", 1000.0, true, 1002 },
      { "avg<100", 100.0, false, 196 },
  };
  ...
  /* Restore the small canonical dataset for any later test. */
  if (sqlExec(conn, "DELETE FROM cte_src") != 0 ||
      sqlExec(conn,
              "INSERT INTO cte_src VALUES "
              "(1,1,10),(2,1,20),(3,2,30),(4,2,40),(5,3,50)") != 0) {
    printf("FAILED (restore cte_src)\n");
    return -1;
  }

  printf("OK (avg>1000 COUNT=1002, avg<100 COUNT=196)\n");
  return 0;
}

/* ------------------------------------------------------------------ */
/* Test 27: ORDER BY / LIMIT CTE (cte_orderby_limit_plan.md L1-L2)    */
/*                                                                     */
/* CTE: SELECT grp, SUM(val) FROM cte_src GROUP BY grp                 */
/*      ORDER BY <spec> LIMIT n                                        */
/* Canonical 5-row cte_src: sums grp1=30, grp2=70, grp3=50; rows per   */
/* group 2/2/1.  Every group redistributes to the constant DBTC-node   */
/* owner, which selects the top-n under the ORDER BY spec after ALL    */
/* per-node partials merged (the multi-node correctness heart: grp1's  */
/* two rows can land on different nodes, so its rank is only known     */
/* post-merge), then truncates.  Dropped groups are CTE_LOOKUP misses, */
/* so the main COUNT over lookupCte-surviving rows pins the kept set:  */
/*   (a) ORDER BY SUM DESC LIMIT 2 -> keep grp2+grp3 -> COUNT=3        */
/*   (b) ORDER BY SUM ASC  LIMIT 1 -> keep grp1      -> COUNT=2        */
/*   (c) ORDER BY grp DESC LIMIT 1 -> keep grp3      -> COUNT=1        */
/*   (d) ORDER BY SUM DESC LIMIT 10 (> groups) -> all -> COUNT=5       */
/* ------------------------------------------------------------------ */

static int
testCteOrderByLimit(Ndb *ndb, MYSQL *conn)
{
  printf("Test 27: CTE ORDER BY / LIMIT (top-N truncation) ... ");
  fflush(stdout);
  ...
  struct SubCase {
    const char *name;
    bool orderByAgg;   /* true: agg slot 0 (SUM); false: GB col 0 */
    bool desc;
    Uint32 limit;
    Int64 expected;
  };
  const SubCase cases[] = {
      { "sum-desc-2", true, true, 2, 3 },
      { "sum-asc-1", true, false, 1, 2 },
      { "grp-desc-1", false, true, 1, 1 },
      { "sum-desc-10", true, true, 10, 5 },
  };

  for (const SubCase &sc : cases) {
    /* CTE 0 = GROUP BY grp, SUM(val), ORDER BY <spec> LIMIT n */
    NdbAggregator cteAgg(srcTab);
    if (!cteAgg.GroupBy("grp") ||
        !cteAgg.LoadColumn("val", 0) ||
        !cteAgg.Sum(0, 0) ||
        !cteAgg.OrderBy(0, sc.orderByAgg, sc.desc) ||
        !cteAgg.Limit(sc.limit) ||
        !cteAgg.Finalize()) {
      printf("FAILED (%s cteAgg: %s)\n", sc.name,
             cteAgg.GetError().err_msg_);
      return -1;
  ...
  printf("OK (desc2=3, asc1=2, grpdesc1=1, lim10=5)\n");
  return 0;
}
