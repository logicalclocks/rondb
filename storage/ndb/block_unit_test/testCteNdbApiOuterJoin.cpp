 ...
 *
 * Tests are added phase-by-phase (plan: cte_outer_join_plan.md):
 *   Phase 2: scanCte(0) as the LEFT-side parent of an outer join with a
 *            regular readTuple child.  Verifies that existing
 *            scanFrag-root + lookup-child NULL-row machinery works when
 *            the "scan" is actually a CTE scan.
 *   Phase 3: CTE_SCAN as outer-join child (requires new parent_row).
 *   Phase 4: consolidated coverage of Phase 1–3 shapes.
 *
 ...
 *   oj_rhs:     (1,'one'),(3,'three')       -- deliberately missing id=2
 *
...
insertTestData(MYSQL *conn)
{
  ...
  /* Deliberately missing id=2 → forces NULL-row synthesis for grp=2
   * in Phase 2's scanCte-as-LEFT-parent test. */
  return sqlExec(conn,
      "INSERT INTO oj_rhs VALUES (1,'one'),(3,'three')");
}
