...
/*
 * bench_q3_ndbapi — TPC-H Q3 style benchmark: 3-table join with SUM of
 *                    arithmetic expression, CHAR equality filter, compound
 *                    GROUP BY.
 *
 * SQL QUERY:
...
runBenchmark(Ndb *ndb, MYSQL *mysqlConn, int iteration)
{
  ...
  /* Column lookups for linked projections */
  const NdbDictionary::Column *orderyearCol =
      ordersTab->getColumn("o_orderyear");
  const NdbDictionary::Column *orderpriorityCol =
      ordersTab->getColumn("o_orderpriority");
  ...
  if (orderyearCol == nullptr || orderpriorityCol == nullptr ||
      extendedpriceCol == nullptr || discountCol == nullptr) {
    fprintf(stderr, "Column lookup failed\n");
    return -1;
  }

  /* Build aggregation program on customer (leaf) */
  NdbAggregator agg(customerTab);
  if (!agg.GroupByLinked(0, orderyearCol) ||          /* GROUP BY o_orderyear (linked pos 0) */
      !agg.GroupByLinked(1, orderpriorityCol) ||      /* GROUP BY o_orderpriority (linked pos 1) */
      !agg.LoadLinkedColumn(2, 0, extendedpriceCol) || /* reg0 = l_extendedprice */
      !agg.LoadDouble(1.0, 1) ||                       /* reg1 = 1.0 */
      !agg.LoadLinkedColumn(3, 2, discountCol) ||      /* reg2 = l_discount */
  ...
  /* Build CHAR equality filter: c_mktsegment = 'BUILDING' (CHAR(10)) */
  NdbInterpretedCode filter(customerTab);
  static const char BUILDING_PADDED[10] = {
    'B','U','I','L','D','I','N','G',' ',' '
  };
  Uint32 mktAttrId = customerTab->getColumn("c_mktsegment")->getColumnNo();
  filter.branch_col_eq(BUILDING_PADDED, 10, mktAttrId, 0);
  ...
  /* Node 2: lookup customer (key: orders.o_custkey, parent: orders,
   * filter + aggregate leaf) */
  ...
  const NdbLinkedOperand *link1 =
      qb->linkedValue(ordersOp, "o_orderpriority");   /* pos 1: GROUP BY */
  const NdbLinkedOperand *link2 =
      qb->linkedValue(lineitemOp, "l_extendedprice"); /* pos 2: computation */
  const NdbLinkedOperand *link3 =
      qb->linkedValue(lineitemOp, "l_discount");      /* pos 3: computation */

  if (link0 == nullptr || link1 == nullptr ||
      link2 == nullptr || link3 == nullptr) {
  ...
  typedef std::pair<int, std::string> GroupKey;
  std::map<GroupKey, Q3Result> ndbResults;
  Uint32 groupCount = 0;
  ...
  for (;;) {
    ...
    /* GROUP BY col 0: o_orderyear INT (linked) */
    NdbAggregator::Column yearCol = rec.FetchGroupbyColumn();
    Int32 year = yearCol.data_int32();

    /* GROUP BY col 1: o_orderpriority CHAR(15) (linked) */
    ...
    GroupKey key(year, priority);
    ndbResults[key] = r;
    groupCount++;

    V("  %d %-15s SUM=%.2f COUNT=%lld\n",
      year, priority.c_str(), r.sum, (long long)r.count);
  }
  ...
  if (mysqlConn != nullptr) {
    ...
    if (mysql_query(mysqlConn,
            ...
            "WHERE c.c_mktsegment = 'BUILDING' "
            "GROUP BY o.o_orderyear, o.o_orderpriority "
            "ORDER BY o.o_orderyear, o.o_orderpriority") != 0) {
      ...
      if (res == nullptr) {
        ...
        std::map<GroupKey, Q3Result> sqlResults;
        MYSQL_ROW row;
        while ((row = mysql_fetch_row(res)) != nullptr) {
          if (row[0] && row[1] && row[2] && row[3]) {
            int year = atoi(row[0]);
            std::string prio(row[1]);
            while (!prio.empty() && prio.back() == ' ') prio.pop_back();
            Q3Result r;
            r.sum = atof(row[2]);
            r.count = atoll(row[3]);
            sqlResults[GroupKey(year, prio)] = r;
          }
        }
        mysql_free_result(res);

        std::set<GroupKey> allKeys;
        ...
        for (const auto &key : allKeys) {
          ...
          if (!hasNdb || !hasSql) {
            fprintf(stderr, "FAIL: %d/'%s': missing in %s\n",
                    key.first, key.second.c_str(),
                    hasNdb ? "sql" : "ndbapi");
            failures++;
            continue;
          }
          const Q3Result &n = ndbResults[key];
          const Q3Result &s = sqlResults[key];

          double tol = std::max(0.01, fabs(s.sum) * 1e-9);
          if (fabs(n.sum - s.sum) > tol || n.count != s.count) {
            fprintf(stderr, "FAIL: %d/'%s': ndb(%.2f,%lld) sql(%.2f,%lld)\n",
                    key.first, key.second.c_str(),
...
int main(int argc, char **argv)
{
  ...
  printf("bench_q3_ndbapi: TPC-H Q3 style benchmark (NDB API)\n");
