...
MYSQL *CreateMySQLConnectionDataCluster() {
  ...
  if (mysql_real_connect(dbConnection, globalConfigs.testing.mySQL.servers[0].IP.c_str(),
    ...
    std::cerr << "Failed to connect to data database: " << mysql_error(dbConnection) << std::endl;
...
MYSQL *CreateMySQLConnectionMetadataCluster() {
  ...
  if (!mysql_real_connect(
          dbConnection, globalConfigs.testing.mySQLMetadataCluster.servers[0].IP.c_str(),
          globalConfigs.testing.mySQL.user.c_str(), globalConfigs.testing.mySQL.password.c_str(),
          nullptr, globalConfigs.testing.mySQLMetadataCluster.servers[0].port, nullptr, 0)) {
    std::cerr << "Failed to connect to metadata database: " << mysql_error(dbConnection)
              << std::endl;
    mysql_close(dbConnection);
    return nullptr;
  }
...
std::tuple<std::vector<std::string>, std::vector<std::string>, std::vector<std::string>, RS_Status>
getColumnInfo(const std::string &dbName, const std::string &tableName) {
  ...
  std::string query = "SELECT DATA_TYPE, COLUMN_NAME, COLUMN_KEY FROM information_schema.COLUMNS "
                      "WHERE TABLE_SCHEMA = '" +
...
bool isColNumerical(const std::string &colType) {
  static const std::map<std::string, bool> numericalType = {
      ...
      {"INTEGER", true}, {"BIGINT", true},   {"DECIMAL", true},   {"FLOAT", true},
...
std::tuple<std::vector<std::vector<std::vector<char>>>, std::vector<std::string>,
           std::vector<std::string>, RS_Status>
...
CreateFeatureStoreRequest(const std::string &fsName, const std::string &fvName, int fvVersion,
...
int SendHttpRequestWithClient(const std::shared_ptr<drogon::HttpClient> &client,
  ...
  if (result != drogon::ReqResult::Ok) {
    ADD_FAILURE() << "failed to perform HTTP request towards url: '" + url + "'\nrequest body:"
  ...
  if (idx == expectedStatus.end()) {
    ADD_FAILURE() << "Received unexpected status '" + std::to_string(respCode) +
                         "' expected status: " + std::to_string(expectedStatus[0]) + " url: '" +
                         url + "' body: '" + body + "' response body: " + respBody;
    return respCode;
  }

  if (respCode != static_cast<int>(drogon::HttpStatusCode::k200OK) &&
      respBody.find(expectedErrMsg) == std::string::npos) {
    ADD_FAILURE() << "Response error body does not contain '" + expectedErrMsg +
                         "'; received response body: '" + respBody + "'";
    return respCode;
  }
...
int SendHttpRequest(const std::string &httpVerb, const std::string &url, const std::string &body,
                    const std::string &expectedErrMsg, const std::vector<int> &expectedStatus,
                    std::vector<char> &responseBody) {
  auto client = drogon::HttpClient::newHttpClient("http://localhost:8080");
  return SendHttpRequestWithClient(client, httpVerb, url, body, expectedErrMsg, expectedStatus,
                                   responseBody);
}
...
std::string NewFeatureStoreURL() {
  auto url = globalConfigs.rest.serverIP + ":" + std::to_string(globalConfigs.rest.serverPort) +
...
GetFeatureStoreResponseWithDetail(const feature_store_data_structs::FeatureStoreRequest &req,
  ...
  if (status == drogon::k200OK) {
    ...
    auto status = feature_store_data_structs::FeatureStoreResponse::parseFeatureStoreResponse(
    ...
    // TODO debug logger
    return std::make_shared<feature_store_data_structs::FeatureStoreResponse>(fsResp);
  }
  return nullptr;
}
...
std::vector<std::vector<char>> GetPkValues(const std::vector<std::vector<char>> &row,
                                           const std::vector<std::string> &pks,
...
void ValidateResponseWithDataExcludeCols(
    const std::vector<std::vector<char>> &data, const std::vector<std::string> &cols,
    const std::unordered_map<std::string, bool> &exCols,
    const feature_store_data_structs::FeatureStoreResponse &resp) {
  auto status = data.empty() ? feature_store_data_structs::FeatureStatus::Error
  ...
  for (size_t k = 0; k < data.size(); ++k) {
    ...
    if (gotRaw.empty() && !_data.empty()) {
      FAIL() << "Got nil but expect " << std::string(_data.begin(), _data.end());
    } else if (!gotRaw.empty() && _data.empty()) {
      FAIL() << "Got " << std::string(gotRaw.begin(), gotRaw.end()) << " but expect nil";
    } else if (gotRaw.empty() && _data.empty()) {
    ...
    if (simdjson::dom::parser parser;
        parser.parse(std::string(gotRaw.begin(), gotRaw.end())).get(got) != simdjson::SUCCESS) {
      FAIL() << "Cannot parse gotRaw: " << std::string(gotRaw.begin(), gotRaw.end());
      continue;
    }
    ...
    if (_data.empty()) {
      ...
      if (simdjson::dom::parser parser;
          parser.parse(std::string(_data.begin(), _data.end())).get(expectedJson) !=
          simdjson::SUCCESS) {
        FAIL() << "Cannot parse _data: " << std::string(_data.begin(), _data.end());
        continue;
      }
    }
    ...
    EXPECT_EQ(got_json, expected_json)
        << "col: " << cols[k] << "; Got " << got_json << " but expect " << expected_json;
  }

  if (resp.status != status) {
    FAIL() << "Got status " << toString(resp.status) << " but expect " << toString(status);
  }
}

void ValidateResponseWithData(const std::vector<std::vector<char>> &data,
                              const std::vector<std::string> &cols,
                              const feature_store_data_structs::FeatureStoreResponse &resp) {
...
void ValidateResponseMetadataExCol(
    ...
    const std::map<std::string, bool> &exCol, const std::string &fsName, const std::string &fvName,
    int fvVersion) {

  auto fetchRows = [](const std::string &query, const std::vector<std::string> &colTypes) {
    auto [rows, status] = fetchMetadataRows(query, colTypes);
    if (status.http_code != static_cast<HTTP_CODE>(drogon::HttpStatusCode::k200OK)) {
      ADD_FAILURE() << "Failed to fetch rows with error: " << status.message;
    }
    return rows;
  };

  auto rows = fetchRows("SELECT id from hopsworks.feature_store where name = \"" + fsName + "\"",
                        {"bigint"});
  int fsId  = std::stoi(std::string(rows[0][0].begin(), rows[0][0].end()));

  rows = fetchRows(
      "SELECT id from hopsworks.feature_view where feature_store_id = " + std::to_string(fsId) +
          " and name = \"" + fvName + "\" and version = " + std::to_string(fvVersion),
      {"bigint"});
  int fvId = std::stoi(std::string(rows[0][0].begin(), rows[0][0].end()));

  rows =
      fetchRows("SELECT tdf.name, tdf.type, tdj.prefix from hopsworks.training_dataset_feature tdf "
                "inner join hopsworks.training_dataset_join tdj on tdf.td_join = tdj.id "
                "where tdf.feature_view_id = " +
                    std::to_string(fvId) + " order by tdf.idx",
  ...
  for (size_t i = 0; i < metadata.size(); ++i) {
    ...
    if (got != expect) {
      FAIL() << "Got " << got.toString() << " but expect " << expect.toString();
    }
    // TODO logger
  }
}

void ValidateResponseMetadata(std::vector<feature_store_data_structs::FeatureMetadata> &metadata,
                              const feature_store_data_structs::MetadataRequest &metadataRequest,
                              const std::string &fsName, const std::string &fvName, int fvVersion) {
  auto exCol = std::map<std::string, bool>();
  ValidateResponseMetadataExCol(metadata, metadataRequest, exCol, fsName, fvName, fvVersion);
}

void testConvertAvroToJson(const std::string &schema, const std::vector<uint8_t> &data,
  ...
  if (error != 0U) {
    FAIL() << "Failed to parse expected JSON: " << simdjson::error_message(error);
  }
...
std::string datumToJson(const avro::GenericDatum &datum, const avro::ValidSchema &schema) {
...
TEST_F(FeatureStoreTest, TestFeatureStoreMetaData) {
  ...
  std::tie(metadata, errorCode) = metadata::GetFeatureViewMetadata(FSDB002, "sample_2", 1);
  EXPECT_EQ(errorCode, nullptr) << "Read FS metadata failed " << errorCode->ToString();
}

TEST_F(FeatureStoreTest, TestMetadata_FsNotExist) {
  std::shared_ptr<metadata::FeatureViewMetadata> metadata;
  std::shared_ptr<RestErrorCode> errorCode;
  std::tie(metadata, errorCode) = metadata::GetFeatureViewMetadata("NA", "sample_2", 1);
  EXPECT_NE(errorCode, nullptr) << "This should fail.";
  EXPECT_TRUE(errorCode->GetReason().find(FS_NOT_EXIST->GetReason()) != std::string::npos)
...
TEST_F(FeatureStoreTest, TestMetadata_ReadDeletedFg) {
  std::shared_ptr<metadata::FeatureViewMetadata> metadata;
  std::shared_ptr<RestErrorCode> errorCode;
  std::tie(metadata, errorCode) = metadata::GetFeatureViewMetadata(FSDB001, "test_deleted_fg", 1);
  EXPECT_NE(errorCode, nullptr) << "This should fail.";
  EXPECT_TRUE(errorCode->GetReason().find(FG_NOT_EXIST->GetReason()) != std::string::npos)
      << "This should fail with error message: " << FG_NOT_EXIST->GetReason()
...
TEST_F(FeatureStoreTest, TestMetadata_FvNotExist) {
  std::shared_ptr<metadata::FeatureViewMetadata> metadata;
  std::shared_ptr<RestErrorCode> errorCode;
  std::tie(metadata, errorCode) = metadata::GetFeatureViewMetadata(FSDB002, "NA", 1);
  EXPECT_NE(errorCode, nullptr) << "This should fail.";
  EXPECT_TRUE(errorCode->GetReason().find(FV_NOT_EXIST->GetReason()) != std::string::npos)
...
TEST_F(FeatureStoreTest, TestConvertAvroToJson) {
  ...
  testConvertAvroToJson(
      R"(["null",{"type":"record","name":"r854762204","namespace":"struct","fields":[{"name":"int1","type":["null","long"]},{"name":"int2","type":["null","long"]}]}])",
      {0x02, 0x02, 0x56, 0x02, 0x88, 0x01}, s2v(R"({"int1":43,"int2":68})"));

  // array
  testConvertAvroToJson(R"(["null",{"type":"array","items":["null","long"]}])",
                        {0x02, 0x06, 0x02, 0x02, 0x00, 0x02, 0x06, 0x00}, s2v(R"([1,null,3])"));

  // array of map
  testConvertAvroToJson(
      R"(["null",{"type":"array","items":["null",{"type":"record","name":"r854762204","namespace":"struct","fields":[{"name":"int1","type":["null","long"]},{"name":"int2","type":["null","long"]}]}]}])",
      {0x02, 0x06, 0x02, 0x02, 0x22, 0x02, 0x48, 0x00, 0x02, 0x02, 0x24, 0x02, 0x4A, 0x00},
      s2v(R"([{"int1":17,"int2":36},null,{"int1":18,"int2":37}])"));

  // map of array
  testConvertAvroToJson(
      R"(["null",{"type":"record","name":"r854762204","namespace":"struct","fields":[{"name":"int1","type":["null",{"type":"array","items":["null","long"]}]},{"name":"int2","type":["null",{"type":"array","items":["null","long"]}]}]}])",
      {0x02, 0x02, 0x06, 0x02, 0x02, 0x02, 0x04, 0x02, 0x06, 0x00, 0x02, 0x06, 0x02, 0x06, 0x00,
       0x02, 0x0a, 0x00},
