...
RS_Status FeatureStoreResponse::parseFeatureStoreResponse(
  ...
  if (features_array.error() == simdjson::error_code::NO_SUCH_FIELD) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      "Missing 'features' field in response").status;
  }
  if (features_array.error() != simdjson::SUCCESS) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      error_message(features_array.error())).status;
  }
  if (!features_array.is_null()) {
    for (simdjson::dom::element feature : features_array) {
      ...
      fsResp.features.push_back(
        std::vector<char>(valueJson.begin(), valueJson.end()));
    }
  }
  ...
  if (metadata_array.error() == simdjson::error_code::NO_SUCH_FIELD) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      "Missing 'metadata' field in response").status;
  }
  if (metadata_array.error() != simdjson::SUCCESS) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      error_message(metadata_array.error())).status;
  }
  ...
  if (!metadata_array.is_null()) {
    for (auto metadata : metadata_array) {
      ...
      if (error != 0U) {
        return CRS_Status(static_cast<HTTP_CODE>(
          drogon::HttpStatusCode::k400BadRequest), error_message(error)).status;
      }
  ...
  if (status_.error() == simdjson::error_code::NO_SUCH_FIELD) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      "Missing 'status' field in response").status;
  }
  if (status_.error() != simdjson::SUCCESS) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      error_message(status_.error())).status;
  }
...
RS_Status
BatchFeatureStoreResponse::parseBatchFeatureStoreResponse(
  ...
  if (status_array.error() != simdjson::SUCCESS) {
    return CRS_Status(static_cast<HTTP_CODE>(
      drogon::HttpStatusCode::k400BadRequest),
      error_message(status_array.error())).status;
  }
...
Options FeatureStoreRequest::GetOptions() const {
  ...
  if (optionsRequest.validatePassedFeatures.has_value()) {
    defaultOptions.validatePassedFeatures =
      optionsRequest.validatePassedFeatures.value();
  }
  if (optionsRequest.includeDetailedStatus.has_value()) {
    defaultOptions.includeDetailedStatus =
      optionsRequest.includeDetailedStatus.value();
  }
  return defaultOptions;
}
