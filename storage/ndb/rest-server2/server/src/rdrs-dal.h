...
struct RS_Buffer {
    ...
    RS_Buffer(unsigned int buffSize) : size(buffSize), buffer(new char[buffSize]) {}
    ...
    RS_Buffer(const RS_Buffer&) = delete;
    RS_Buffer& operator=(const RS_Buffer&) = delete;
};

class RS_BufferManager {
public:
    RS_BufferManager(unsigned int size) : rsBuffer(std::make_unique<RS_Buffer>(size)) {}
