...
#include "batch_pk_delete_ctrl.hpp"
...
//#define DEBUG_BPK_DEL_CTRL 1
#endif

#ifdef DEBUG_BPK_DEL_CTRL
#define DEB_BPK_DEL_CTRL(...) do { g_eventLogger->info(__VA_ARGS__); } while (0)
#else
#define DEB_BPK_DEL_CTRL(...) do { } while (0)
#endif


void BatchPKDeleteCtrl::batchPKDelete(
  const drogon::HttpRequestPtr &req,
  std::function<void(const drogon::HttpResponsePtr &)> &&callback) {

  drogon::HttpResponsePtr resp = drogon::HttpResponse::newHttpResponse();
  BatchPkDeleteEndPointMetricsUpdater metricsUpdater(resp);
  ...
  RS_Status status = jsonParser.batch_parse_delete(
  ...
  for (auto reqStruct : reqStructs) {
    ...
    // For delete, readColumns is optional - if provided, return deleted row data
  ...
  // Execute
  {
    ...
    for (Uint32 i = 0; i < noOps; i++) {
      ...
      DEB_BPK_DEL_CTRL("Buffer: %p, current_head: %u",
    ...
    // pk_batch_delete
    status = pk_batch_delete(&amalloc,
                             noOps,
                             true,
                             reqBuffs.data(),
                             respBuffs.data(),
    ...
    if (unlikely(static_cast<drogon::HttpStatusCode>(status.http_code) !=
      ...
      for (unsigned int i = 0; i < noOps; i++) {
        ...
        DEB_BPK_DEL_CTRL("Response buffer[%u]: %p, size: %u",
