...
An attacker reading the source can see which violations are Tier B (log-only, non-disconnecting) and exploit this: generate Tier B violations as cover, as timing probes, or as a log-flooding attack. **Tier B `SECURITY_EVENT` log emission is rate-capped** to `MAX_SECURITY_EVENTS_PER_SEC` (10/s) per data node via a global 1-second window in `Qmgr::execMALICIOUS_SIGNAL_REPORT`; the RONDIS path has an equivalent per-worker cap. Counting is never throttled — `m_violationCounts[]` / `ndbinfo.security_violation_counts` stay exact — so forensic totals survive a flood even though individual log lines are capped. See Section 13.
...
|---|---|
| **Tier B log flood (mitigated 2026).** A connected, authenticated client can sustain thousands/sec of Tier B violations. This is now rate-capped: Tier B `SECURITY_EVENT:` emission is limited to `MAX_SECURITY_EVENTS_PER_SEC` (10/s) per data node via a global 1-second window in `Qmgr::execMALICIOUS_SIGNAL_REPORT`, with a `SECURITY_EVENT suppressed N events` summary line when a throttled window rolls; RONDIS has an equivalent per-worker cap (`rondis_security_event_gate`). | The cap is a **global** window (no per-node state — keeps the lean design) and throttles the **log line only**: `m_violationCounts[]` / `ndbinfo.security_violation_counts` are still incremented on every report, so per-type forensic totals remain exact under flood. Tier A is never suppressed (self-limits: offender disconnected on first strike). Offender `node_id` still appears in each emitted line; under heavy flood some individual lines are dropped, so operators should also monitor `security_violation_counts` externally and bound inflow via connection-level controls. |
