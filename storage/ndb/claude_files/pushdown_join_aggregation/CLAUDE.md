...
- `cte_filter_phase_i5_v3.md` — Phase I.5 v3 (planned): float / double operands in col-vs-col CASE.  Tiny scope after I.18: kernel `handleReadLinkedColumnToReg` gains `NDB_TYPE_FLOAT` / `NDB_TYPE_DOUBLE` arms (leaf side already handled by I.18 batch 2's typed `READ_ATTR_INTO_REG`); RonSQL `can_load_integer_reg` extended to accept Float / Double.  `compareTypedRegs` already promotes mixed int/float to double.  Col-vs-const float literal deferred to v3b
