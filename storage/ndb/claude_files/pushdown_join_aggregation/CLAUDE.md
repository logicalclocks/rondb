...
- `cte_filter_phase_i16.md` — Phase I.16a (shipped, `0906344763c`): partial-key joins to multi-key CTEs.  RonSQL `emit_child_ops` guards CTE_LOOKUP emission with a `groupby_columns` count check vs `op.num_key_cols`, throwing a clear permanent error naming the workaround (multi-key CTE on the joined root) when the join can't bind the full virtual PK.  I.16b (planner-side rewrite to CTE_SCAN root) and I.16c (non-root CTE_SCAN child) deferred
