...
- `cte_filter_phase_i24.md` — Phase I.24 (planned): migrate emit paths from legacy parallel arrays (`column_attrId_map`, `column_map`, `column_table_idx`) to explicit per-scope resolved-column descriptors.  Keep old arrays as compatibility output during migration; convert CTE metadata helpers, filter emit, aggregation emit, and pass-through result routing incrementally
