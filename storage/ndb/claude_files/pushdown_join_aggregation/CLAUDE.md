...
- `cte_filter_phase_i6_varchar.md` — Phase I.6 F.2 + F.3 (shipped): kernel string MIN/MAX execution via per-(group, slot) val_ptr state, AGG_CHAR_RESULT wire format (marker `0xFF02`), NdbAggregator API parse + `Result::data_str` accessor, RonSQL CTE virt-table type passthrough and filter accept-list extension.  Open follow-ups (linked-attr strings, CTE delivery substitution) live in `cte_filter_phase_i6_string_minmax_f4.md`
...
- `cte_filter_phase_i6_string_minmax_f4.md` — Phase I.6 F.4 (planned): CTE delivery + linked-attr strings.  Substitutes string payload bytes for `accumulators[i].value` in `Dblqh::buildCteLinkedBuffer` (K.1) and `Dblqh::emitCteGroupOutput` (K.2); drops the linked-attr string rejection in `JoinAggInterpreter::kOpLoadCol` (K.3); audits remaining 8-byte assumptions (K.4); adds block + MTR coverage (K.5 + R.1).  Without F.4, queries that re-aggregate or filter a CTE string output hang behind retried `ZCTE_LOOKUP_OUTPUT_OVERFLOW`
...
- `cte_filter_phase_i19.md` — Phase I.19 (planned): CASE literal normalisation and boundary fixes after the Phase I.5 v3b review.  Simplify whole embedded CASE atoms before side resolution so literal-on-left forms like `100.0 < col` normalise like WHERE expressions; fix signed INT literal bounds in `encode_constant()`; add v3b MTR coverage for leaf FLOAT vs negative fractional literal, leaf DOUBLE, literal-on-left, and INT_MIN / INT positive-overflow boundaries
- `cte_filter_phase_i20.md` — Phase I.20 (planned): Phase I.16 follow-up for CTE lookup key coverage and rewrite validation.  Replace key-count checks with ordered virtual-CTE-PK coverage checks; accept/reorder or reject full-key predicates in non-PK order; reject wrong-column full-count joins; constrain the I.16c root rewrite to partial-key CTE joins whose parent alias is the original root
- `cte_filter_phase_i21.md` — Phase I.21 (planned): scalar CTE semantic guardrails after I.17.  Keep the scalar dummy-key workaround local to scalar CTE child lookup; prevent root scalar CTE `WHERE` predicates from using keyless/dummy `lookupCte`; preserve nullable aggregate output metadata when the first scalar output can be NULL; constrain top-level implicit `GREATEST` / `LEAST` lowering to proven scalar-CTE expressions so ordinary row-wise table queries cannot silently become aggregate queries
- `cte_filter_phase_i22.md` — Phase I.22 (planned): DECIMAL MIN/MAX 64-bit range guard after Phase I.6 F.1.  Add a RonSQL prepare-time rejection for scale-zero DECIMAL MIN/MAX declarations that can exceed the kernel's current `decimal2longlong` / `decimal2ulonglong` target range: signed precision > 18 and unsigned precision > 19.  Add MTR coverage for safe boundaries and rejected unsafe declarations; full arbitrary-precision DECIMAL preservation remains out of scope
...
- `cte_filter_phase_i25.md` — Phase I.25 (planned): follow-up for two string CTE failures found while adding F.4 coverage.  Re-aggregating string CTE MIN/MAX through a CTE-root to non-unique ordered-index child join returned `NULL,NULL`, while the primary-key helper-table variant worked.  The intended `WHERE min_v < 'beta'` regression also needs quote-safe MTR compare handling plus clarified RonSQL string-literal parser semantics.  Also covers a CTE output comparison matrix for col-vs-col, const-vs-col, and col-vs-const across signed/unsigned integers, floats, DECIMAL widening, strings, exposed date/time types, and NULL semantics
