...
- `cte_filter_phase_i5_v3.md` — Phase I.5 v3a (shipped, `ee27b22dea0`): float / double operands in col-vs-col CASE.  Kernel `handleReadLinkedColumnToReg` gained `NDB_TYPE_FLOAT` / `NDB_TYPE_DOUBLE` arms; RonSQL `can_load_integer_reg` renamed to `can_load_typed_reg` and accepts Float / Double; `compareTypedRegs` already promotes mixed int/float to double.  Col-vs-const float literal deferred to v3b
