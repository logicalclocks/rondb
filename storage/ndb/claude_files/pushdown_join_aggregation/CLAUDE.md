...
- `cte_filter_phase_i16.md` — Phase I.16a + I.16b (shipped, `f417051a4c4`): partial-key joins to multi-key CTEs.  I.16a guards `emit_child_ops`'s CTE_LOOKUP arm with a `groupby_columns` count check, throwing a clear permanent error.  I.16b adds an AST-rewrite pre-pass in `load_join` that swaps the multi-key CTE to root and the original parent to a join child for the simple two-table inner-join shape, letting `QueryPlanner` produce a CTE_SCAN root + PK / unique / index child.  LEFT JOIN, multi-table chains, and CTE-on-CTE stay on the I.16a clean-reject path.  I.16c (non-root CTE_SCAN child with linked keys) still deferred
