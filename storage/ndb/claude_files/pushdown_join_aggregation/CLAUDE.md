...
- `cte_filter_phase_i17.md` — Phase I.17 (shipped, `d0b9d6e9dd2`): scalar aggregate CTEs without GROUP BY.  RonSQL relaxation accepts CTE bodies whose every output is an aggregate; kernel `Dblqh::continueJoinAggRedistribute` ships per-node `m_agg_results` to the DBTC-co-located owner via a `keyLen == 0` `JoinAggRedistributeReq` so the cluster has a single canonical scalar result.  `cteScanShouldEmitScalar` reduces to `refToNode(state->m_senderRef) == getOwnNodeId()`.  Watermark / cross-CTE-join shape pending RonSQL comma / cross-join parsing
