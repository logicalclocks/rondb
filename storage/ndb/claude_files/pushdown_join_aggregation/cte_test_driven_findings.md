...
| **D4 ✅ SUPPORTED (2026-09-01)** | CTE-body signed-int col-vs-col over lineitem feeding P-GB | `... WHERE l_quantity < l_partkey ...` (filter-13). The June 2026 hang was a STALE-BINARY artifact (W0 probe green on current binaries; D19/D20 precedent). Identical-type pairs on the body root op are now accepted (`check_cte_body_col_vs_col`, tier (a) of `cte_body_colvscol_plan.md`); filter-13 + `ronsql_cte_dd_d4_colvscol` are value compares, matrix in `body_filter.inc` Group 8 |
...
| ID | Shape | Error |
|----|-------|-------|
| **D1** | `SUM(<DECIMAL col>)` in a CTE body | ✅ FIXED — was "SUM over this column type in CTE not yet supported".  Mirrors the MIN/MAX DECIMAL widening into the SUM arm of `build_cte_virtual_tables` + `resolve_chained_column_type` (scale 0 → BIGINT exact; scale>0 → DOUBLE best-effort, kernel already widens on load via `AlignedType`).  Source DECIMAL scale now reaches the result printer (`resolve_cte_output_columns_for_scope` plumbs SUM's source `dict_column`; `aggregate_arg_scale`/`precision` accept `T_SUM`) so scale>0 sums print with the source scale (e.g. `15051277.50`) instead of dropping trailing zeros.  Re-enabled agg-d1a/b/c + J3 ×5 topologies |
...
| **D11** | `GREATEST`/`LEAST` in a **CTE body** WHERE term | ✅ FIXED — was a parser "Syntax error" (grammar allowed them only in the top-level SELECT scalar position).  Added `cond_expr` grammar rules + a `simplify_ce` boolean rewrite (`GREATEST(a,b) > c` → `a>c OR b>c`, AND for the opposite direction; OR direction adds `IS NOT NULL` guards; constant arms folded; `=`/`!=` and non-constant comparands rejected).  Re-enabled filter-14/14b/15/15b ×5 topologies |
| **D12 ✅ SUPPORTED (2026-09-01)** | CTE-body signed-int col-vs-col over orders | The bound misclassification was fixed by the join-root non-constant-RHS bound hardening (body plans TABLE_SCAN + residual filter); identical-type pairs now execute (same F-colvscol tier (a) as D4). filter-12 + `ronsql_cte_dd_d4_colvscol` are value compares |
| **D17** | `MIN`/`MAX` over a **DATE** column in a CTE | ✅ FIXED — root cause was `NdbAggregator::TypeSupported` rejecting `Date` (→ "Failed writing aggregation program").  Integer-day shortcut: DATE handled like `MEDIUMUNSIGNED` (3-byte `uint3korr`, `AlignedType`→BIGINT, `is_unsigned`), result returns Bigunsigned, RonSQL tags `ColumnMetadata::is_date` and unpacks `w`→`YYYY-MM-DD`.  GROUP-BY-over-DATE needed no kernel change (`initGBTypes` is type-generic).  Re-enabled agg-d17a/b/c ×5 topologies.  index-9 re-tagged D9 (DATE index-scan root).  See `cte_date_minmax_plan.md` |
