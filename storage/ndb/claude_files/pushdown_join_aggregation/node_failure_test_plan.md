...
| Item | Count | Where |
|---|---|---|
| New error inserts | 29 (DBLQH 5128-5150, DBTC 8311-8314, DBSPJ 17532-17533) | kernel blocks |
...
| NDBT node-failure cases | 12 + PK-8 + F-12 | `testNodeRestart` (new `testCteNodeFail` if the binary grows too large) |
| Block-level protocol / state-machine cases | 14 | `block_unit_test/testCteDbtc`, `testJoinAgg`, new `testCteProtocol` |
| Parking cases | 9 | PK-1..PK-7 and PK-9 in `block_unit_test/testCteProtocol`, PK-8 (a node kill) in `testNodeRestart` beside NF-11 / NF-12 |
| MTR cases | 7 RonSQL test files developed inside Phases 1-3 (maintainer direction) + 14 wrappers of the NDBT cases - **implemented; F-12 validation pending** | `mysql-test/suite/ronsql_cte` (4-node variant `ronsql_cte_ng2r2`), wrappers in `ndb_cte` / `ndb_cte_ng2r2` |
...
| NF-13 | `CteCoordinatorDiesWaitingScan` | none: the API holds the query after its first row, the 64-row main batch leaves every DBSPJ worker waiting for SCAN_NEXTREQ (RS_WAITING) | C | a survivor logs `SPJ_ORPHANED_SCANS` for C; after C rejoins, 2650 clean: no RS_ABORTED tombstone (RONDB-1121 A2, 2026-09-25) |
| NF-14 | `JoinAggCoordinatorDiesAtOwner` | main aggregation (no CTE): 5154 holds the owner LDM's merge (FINALIZING) and ignores m_connected | C | the owner's ZNODE_DOWN stops the merge: the survivor logs `JOIN_AGG_RELEASES_QUEUED` for C; 2361 clean (RONDB-1121 A2, 2026-09-25) |
...
right after the post-kill marker and the victim's shutdown, before the
query is waited for: with the coordinator alive, DBTC defers the JoinAgg
close until the replies the armed node still holds are in (F-9). A cleanup guard
...
NF-11 and still requires the confirmed hold, kill and sweep. P's insert
is cleared once the sweep marker is seen and R is down, before the query
is waited for: C is alive, so the API's close waits for DBTC's deferred
JoinAgg close, which needs P's held SETUP reply (F-9).
NF-12 kills the
...
NF-13 needs no error insert. DBTC take-over handles no scans, so a
DBSPJ scan request waiting for SCAN_NEXTREQ when its coordinator dies
never gets one; `Dbspj::cleanup` used to park such a request as an
RS_ABORTED tombstone for that SCAN_NEXTREQ, leaking it (with its arena
and CTE key arrays) until the node restarted. `Dbspj::nodeFail` now marks
requests of a failed coordinator `RT_REQUESTER_FAILED` and `cleanup`
releases them in full; tombstones remain only while the coordinator
lives (its next SCAN_NEXTREQ REFs and frees them). The close hook of
`CteQueryUtil` (after the first row, 300 ms settle) kills the coordinator
itself; the case then requires `[SPJ_ORPHANED_SCANS node=S instance=I
failed=C count=N]`, which `Dbspj::nodeFail_checkRequests` emits in every
build when it finds such requests, restarts C and runs the leak dumps.
The same change makes the DBSPJ node-failure sweep resume each
continuation at the bucket where the slice stopped; it restarted at the
slice's first bucket, so requests that stay in the hash (tombstones)
could hold the sweep on the same prefix for good. Runs on 2 nodes.
NF-14 covers the owner LDM of a main aggregation. `JOIN_AGG_NODE_FAIL_REP`
reclaims a failed coordinator's states only after every LDM handled the
NODE_FAILREP and skips FINALIZING / SENDING_RESULTS, so the owner's merge
and send continuations must stop before that. `checkJoinAggNodeFailed`
tested only `m_connected`, which clears on DISCONNECT_REP: CLOSE_COMREQ
halts input and only starts the disconnect, so it can complete after the
whole protocol. It now also tests the instance's host record
(`isJoinAggCoordinatorFailed`, ZNODE_DOWN from its own NODE_FAILREP, ahead
of the rest of its failure handling). No guard is needed for COMPLETE or
SEND_CONF from the dead coordinator: after NODE_FAILREP nothing arrives
from it, and the park flush that replays a parked COMPLETE comes from the
proxy, like the workers' NODE_FAILREP, so it runs first. The test runs
the self-join aggregation of `runJoinAggQuery` with 5154 on every node
(extra = iteration): the owner of a remote coordinator emits
`[JOIN_AGG_MERGE_HELD node=N iteration=I coordinator=C]`, the killer
kills C, requires `[JOIN_AGG_RELEASES_QUEUED node=N failed=C`, clears the
survivors' inserts, restarts C and runs the leak dumps. 5154 makes the
merge ignore `m_connected`, so only ZNODE_DOWN can stop it: before the
fix it outlived the protocol, was skipped and leaked. Two iterations;
runs on 2 nodes.
...
| ID-8 | COMPLETE_REQ, then REDISTRIBUTE_REQ | a CTE COMPLETE without its per-node key section; then a group with the correct identity | COMPLETE_REF 1251 and the state fails with it (ERROR, peers told by error FINAL_REP), so the late group answers REDISTRIBUTE_REF 1251 instead of CONF + a queued group; RELEASE asserts no owner phase (FINALIZING / SENDING_RESULTS / CTE_REDISTRIBUTING) is in progress - written 2026-09-25 (RONDB-1121 A2), not yet run |
...
| PK-9 | SETUP_CONF after a cancel, fragments closing | 5138 holds the main SETUP of ScanAggMain (no CTE) until its consumer scans park, then lets it run; DBTC 8315 fails the scan (`scanError` 270: `close_scan_req` cancels the SETUP round and sends the closes) right before accounting that SETUP_CONF (`testCteProtocol`) | the CONF meets AGG_SETUP_CANCELLED with fragments running: DBTC keeps the key for the scan's teardown (`JOIN_AGG_SETUP_CONF_DEFERRED` for the scan / request / key of `JOIN_AGG_SETUP_CONF_AFTER_CANCEL`) instead of the stale-SETUP reclaim, whose RELEASE freed the programs and interpreters under the replayed consumers (RONDB-1121, 2026-09-25); query fails 270; 2361 / 2363 clean - written, not yet run |

PK-1..PK-7 and PK-9 live in `testCteProtocol`; PK-8 kills a node and lives in
...
| `cte_nodefail_coordinator_waiting.test` (suite `ndb_cte`) | 1 | wrapper: `testNodeRestart -n CteCoordinatorDiesWaitingScan T1` (NF-13: coordinator dies while every DBSPJ worker waits for SCAN_NEXTREQ; no error insert, so no debug build needed) - written 2026-09-25, not yet run |
| `cte_nodefail_joinagg_owner.test` (suite `ndb_cte`) | 1 | wrapper: `testNodeRestart -n JoinAggCoordinatorDiesAtOwner T1` (NF-14: coordinator dies while the owner LDM holds a main aggregation's merge, 5154) - written 2026-09-25, not yet run |
| `cte_nodefail_peer.test` (suite `ronsql_cte_ng2r2`) | 1 | background CLI, 5140 hold, cookie-matched peer selection, abort restart, checked retry/error (286 / 20016 / API 40xx, and 1251 from the sweeper for an attempt started during failure handling, F-10), recovery query and leak acknowledgements (shared driver `ronsql_cte/include/cte_nodefail.inc`) - **done** |
...
| F-12 | `ronsql_cte_ng2r2.cte_nodefail_peer` soak, fourth failure (2026-09-17) | The attempt RonSQL retried right after the peer kill failed with 286 and then hung in `query->close()` for 120 s; no state of that attempt was ever released. Its SETUPs reached node 1 (main and CTE) at 11:30:17.6504, nodes 3 and 4 logged only the main SETUP, and both swept the CTE's parked consumers 10 ms later | The retry's coordinator was node 1. DBTC builds the JoinAgg node set from `getNodeInfo().m_connected`, and NODE_FAILREP (QMGR's failure protocol, processed by DBTC at .5568) precedes the node-level transporter disconnect that clears `m_connected` (.6509 on node 1, .6438 on node 4) by up to 100 ms with multi-transporters. The fan-out at .6504 therefore included dead node 2: its two SETUPs were dropped by the transporter and nothing faked their replies, since `handleJoinAggNodeFailure` had already run before this scan existed, so `m_aggSetupOutstanding` never reached 0, `joinAggSetupRoundDone` never ran, and the deferred close waited for ever. Nodes 3 and 4 had already seen node 2 go and rejected the CTE SETUP through the owner-list validation (286, the error the API saw); node 1 still saw node 2 connected and accepted it | fixed: `sendJoinAggSetupReqs` also requires `c_alive_nodes` (cleared in `execNODE_FAILREP`, set at `INCL_NODEREQ`, so a recovering node still qualifies). Follow-up implemented: COMPLETE/RELEASE and the CTE COMPLETE owner map also require the alive set; SETUP reconciliation queues synthetic REFs after fan-out and before close/abort/READY defers. Deterministic coverage added as CteSetupAlreadyFailedNode (8314); build/runtime validation pending |
| F-11 | `ronsql_cte_ng2r2.cte_nodefail_peer` soak, third failure (2026-09-17) | The recovery query, a fresh `ronsql_cli` started right after the victim rejoined, produced neither a result nor an error for 120 s. Every node reached CTE_READY, served its probes and nodes 2, 3 and 4 logged their main COMPLETE and one `TRANSID_AI` to API node 63 at 11:32:32.2008; nodes 3 and 4 logged `Node 63: API RonDB` only at .2124 | `ronsql_cli` called `wait_until_ready(30, 0)`, which returns as soon as one data node is alive, and started the query while nodes 3 and 4 had not yet processed its API_REGREQ. QMGR keeps an API node in ZAPI_INACTIVE until then; ENABLE_COMREQ to TRPMAN is what sets `m_connected` and enables the transporter, and `TransporterRegistry::prepareSend` discards a signal to a node that is not enabled (SEND_BLOCKED, treated as success by data nodes). The aggregate result rows those two nodes sent straight to the API were discarded, and the API waits for every row the SCAN_TABCONF promised. Classic scans share the exposure, which is why RDRS and mysqld wait for all nodes; the node-1 main COMPLETE line is missing from a log that lost lines under load and is not needed to explain the hang | fixed in `ronsql_cli`: `wait_until_ready(30, 30)` like RDRS, with a warning when nodes are still missing. Open product question for the API: a pushed aggregation whose rows are lost hangs until the API's own scan timeout (3 x WaitForTimeout); the API could refuse or delay an aggregating query while data nodes are not yet alive, or DBTC could route the result rows through itself |
| F-10 | `ronsql_cte_ng2r2.cte_nodefail_peer` soak, second failure (2026-09-17) | The first attempt failed with 286 as expected, `ronsql_cli` retried, the retry failed with 1251 and the third attempt returned the exact result; the driver rejected 1251 as a diagnostic | The retry started while the survivors were still handling the peer's failure (about 130 ms of DBTC take-over and table clean-up); its consumers reached node 1 and parked before the proxy processed the SETUP, and the 10 ms placeholder sweeper REFed them (`JOIN_AGG_PARK_SWEPT ... failed=0 count=4`). This is the P2 design: a SETUP that lags its consumers by more than 10 ms costs the query a temporary 1251, which the API and RonSQL retry | documented behaviour, no kernel change; the driver accepts 1251 among the attempts' diagnostics, as NF-11 already does, and still requires the exact result or a node-failure error at the end |
| F-9 | NF-11 first soak run, `ndb_cte_ng2r2.cte_nodefail_requester_parked` (2026-09-16) | After R was killed and P had swept the parked consumers, the query's `nextResult` failed with 286 within 139 ms but its close did not return for 120 s; it returned the moment the killer's guard cleared P's insert | With the coordinator alive, the API's close waits for DBTC's close confirmation, and DBTC defers a JoinAgg close until every SETUP reply is in (the H2 deferral); P's SETUP was still held by 5145 and the driver cleared the insert only after the query had completed. Before F-7 the duplicate SCAN_TABREF that a later worker failure produced woke the API's close, which is how NF-11, and NF-6 with its 5144 feed hold, passed until now | fixed in the driver: for requester kills (`CTE_NF_KILL_EVENT_REQUESTER`) the armed node's insert is cleared right after the post-kill marker and the victim's shutdown, before the query is waited for; coordinator kills keep the old order since the API fails on its own |
...
### F-12 deterministic regression: CteSetupAlreadyFailedNode

Wrapper: `ndb_cte.cte_nodefail_setup_failed_node`; also registered in
`daily-basic--16-tests.txt`. Implementation added; build/runtime validation
is pending.

Kill a replica other than the pinned coordinator and wait for NOT_STARTED.
DBTC insert 8314 (extra = victim node id) bypasses the SETUP destination
filter only after that instance's node-failure handling has finished.
It leaves the alive set unchanged and emits JOIN_AGG_SETUP_FAILED_NODE,
so no later failure scan can accidentally satisfy the new obligations.
Healthy warm-up queries may complete until the insert can activate.

Run LookupMain (CTE SETUP) and OuterAggMain (main-only SETUP) separately.
LookupMain does not contain a main aggregation. Each attempt must report
286 and finish query/close within 30 seconds; a separate watchdog aborts
the test process if the query or close blocks. Require the insert marker,
clear the insert, then require cookie-matched DUMP 2560 and 2361
acknowledgements from every survivor before restarting the victim.
Restore the victim on ordinary failure paths; after success, rerun both
shapes and all five existing leak checks. A watchdog abort leaves recovery
to the test harness.

This exercises reconciliation immediately after fan-out. It does not
independently force the three later deferral call sites, nor simulate
missing replies from live nodes.
