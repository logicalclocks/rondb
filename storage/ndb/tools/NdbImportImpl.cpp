...
NdbImportImpl::Mgm::Mgm(NdbImportImpl &impl)
    : m_impl(impl), m_util(m_impl.m_util), m_error(m_util.c_error) {
  m_handle = 0;
  m_connected = false;
  m_status = 0;
}
...
int NdbImportImpl::Mgm::do_connect() {
...
void NdbImportImpl::Mgm::do_disconnect() {
  if (m_handle != 0) {
    if (m_status != 0) {
      free(m_status);
      m_status = 0;
...
int NdbImportImpl::Mgm::get_status() {
...
NdbImportImpl::Node::Node() { m_nodeid = Inval_uint; }

NdbImportImpl::Nodes::Nodes() {
  m_nodecnt = 0;
  for (uint i = 0; i < g_max_nodes; i++) m_index[i] = Inval_uint;
}

int NdbImportImpl::get_nodes(Nodes &c) {
  log_debug(1, "get_nodes");
  c.m_nodecnt = 0;
  do {
    Mgm mgm(*this);
    if (mgm.do_connect() == -1) break;
    if (mgm.get_status() == -1) break;
...
int NdbImportImpl::do_connect() {
  ...
  }
  {
    ...
    for (uint i = 0; i < c.m_connectioncnt; i++) {
      ...
      c.m_connections[i] = new Ndb_cluster_connection(
          opt_ndb_connectstring, c.m_mainconnection, nodeid);
      c.m_connections[i]->configure_tls(opt_tls_search_path, opt_mgm_tls);
      if (i == 0) c.m_mainconnection = c.m_connections[i];
    }
    for (uint i = 0; i < c.m_connectioncnt; i++) {
...
void NdbImportImpl::do_disconnect() {
...
int NdbImportImpl::add_table(const char *database, const char *table,
                             uint &tabid, Error &error) {
  Connect &c = c_connect;
  if (!c.m_connected) {
  ...
  if (m_util.add_table(dic, tab, tabid, error) != 0) return -1;
  return 0;
}

int NdbImportImpl::remove_table(const uint table_id) {
  Connect &c = c_connect;
  if (!c.m_connected) {
    m_util.set_error_usage(m_error, __LINE__);
    return -1;
...
NdbImportImpl::Job::Job(NdbImportImpl &impl, uint jobno)
  ...
  m_fatal = false;
  m_teamcnt = 0;
  for (uint i = 0; i < g_max_teams; i++) m_teams[i] = 0;
  for (int k = 0; k < g_teamstatecnt; k++) m_teamstates[k] = 0;
  m_rows_relay = 0;
  for (uint i = 0; i < g_max_ndb_nodes; i++) m_rows_exec[i] = 0;
  m_rows_reject = 0;
  m_old_rows = 0;
  m_old_reject = 0;
  m_old_runtime = 0;
...
NdbImportImpl::Job::~Job() {
  ...
  delete m_rows_relay;
  for (uint i = 0; i < g_max_ndb_nodes; i++) delete m_rows_exec[i];
  delete m_rows_reject;
}

void NdbImportImpl::Job::do_create() {
  const Opt &opt = m_util.c_opt;
  ...
  Stats &stats = m_stats;
  {
    ...
    if (rowbatch != 0) m_rows_relay->m_rowbatch = rowbatch;
    uint rowbytes = opt.m_rowbytes;
    if (rowbytes != 0) m_rows_relay->m_rowbytes = rowbytes;
  }
  {
    for (uint i = 0; i < nodecnt; i++) {
      ...
      uint rowbatch = opt.m_rowbatch;
      if (rowbatch != 0) m_rows_exec[i]->m_rowbatch = rowbatch;
      uint rowbytes = opt.m_rowbytes;
      if (rowbytes != 0) m_rows_exec[i]->m_rowbytes = rowbytes;
    }
  }
  {
...
void NdbImportImpl::Job::add_team(Team *team) {
  require(team != 0);
...
int NdbImportImpl::Job::add_table(const char *database, const char *table,
                                  uint &tabid) {
  return m_impl.add_table(database, table, tabid, m_error);
}

void NdbImportImpl::Job::set_table(uint tabid) {
  (void)m_util.get_table(tabid);
  m_tabid = tabid;
}

int NdbImportImpl::Job::remove_table(const uint table_id) {
  return m_impl.remove_table(table_id);
}

void NdbImportImpl::Job::do_start() {
  const Opt &opt = m_util.c_opt;
  log_debug(1, "start");
  m_timer.start();
  do {
  ...
  while (m_state != JobState::State_stopped) {
    ...
    check_teams(true);
    if (m_state == JobState::State_stop) m_state = JobState::State_stopped;
    NdbSleep_MilliSleep(opt.m_checkloop);
  }
  log_debug(1, "stopped");
  collect_teams();
...
void NdbImportImpl::Job::start_diag_team() {
  Team *team = m_teams[0];
  team->do_create();
  team->do_start();
  if (team->has_error()) {
...
void NdbImportImpl::Job::start_resume() {
  ...
  // verify old stats counts against old rowmap
  {
    ...
    if (m_old_rows != old_rows || m_old_reject != old_reject) {
      m_util.set_error_gen(m_error, __LINE__,
                           "inconsistent counts from old state files"
                           " (*.stt vs *.map)"
                           " rows %" PRIu64 " vs %" PRIu64 " reject %" PRIu64
                           " vs %" PRIu64,
...
void NdbImportImpl::Job::start_teams() {
  ...
  for (uint i = 1; i < m_teamcnt; i++) {
    ...
    team->do_start();
    if (team->has_error()) {
      m_util.set_error_gen(m_error, __LINE__, "failed to start team %u-%s",
                           team->m_teamno, team->m_name.str());
      return;
    }
  }
...
void NdbImportImpl::Job::check_teams(bool dostop) {
  for (uint i = 1; i < m_teamcnt; i++) {
    Team *team = m_teams[i];
...
void NdbImportImpl::Job::check_userstop() {
  if (m_dostop && m_state != JobState::State_stop) {
    log_debug(1, "stop by user request");
    m_util.set_error_gen(m_error, __LINE__, "stop by user request");
    m_state = JobState::State_stop;
  }
  if (NdbImportUtil::g_stop_all && m_state != JobState::State_stop) {
    log_debug(1, "stop by user interrupt");
    m_util.set_error_gen(m_error, __LINE__, "stop by user interrupt");
    m_state = JobState::State_stop;
  }
}

void NdbImportImpl::Job::collect_teams() {
  char error_team[100] = "";
  {
    Team *team = m_teams[0];
    if (team->has_error())
      sprintf(error_team + strlen(error_team), " %u-%s", team->m_teamno,
              team->m_name.str());
  }
  for (uint i = 1; i < m_teamcnt; i++) {
    Team *team = m_teams[i];
  ...
  if (strlen(error_team) != 0) {
    m_util.set_error_gen(m_error, __LINE__, "error in teams:%s", error_team);
  }
}

void NdbImportImpl::Job::collect_stats() {
  m_timer.stop();
...
void NdbImportImpl::Job::stop_diag_team() {
  const Opt &opt = m_util.c_opt;
  Team *team = m_teams[0];
  team->do_stop();
...
void NdbImportImpl::Job::do_stop() { m_dostop = true; }

// team

NdbImportImpl::Team::Team(Job &job, const char *name, uint workercnt)
    : m_job(job),
      m_impl(job.m_impl),
      m_util(m_impl.m_util),
  ...
  m_workers = 0;
  for (int k = 0; k < g_workerstatecnt; k++) m_workerstates[k] = 0;
  m_tabid = Inval_uint;
  m_is_diag = false;
  // stats
...
void NdbImportImpl::Team::do_create() {
...
void NdbImportImpl::Team::do_start() {
  log_debug(1, "start");
  m_timer.start();
...
NdbImportImpl::Worker *NdbImportImpl::Team::get_worker(uint n) {
...
void NdbImportImpl::Team::start_worker(Worker *w) {
  NDB_THREAD_PRIO prio = NDB_THREAD_PRIO_MEAN;
  uint stack_size = 64 * 1024;
  w->m_thread =
      NdbThread_Create(start_worker_c, (void **)w, stack_size, w->m_name, prio);
  require(w->m_thread != 0);
}

void NdbImportImpl::Team::wait_workers(WorkerState::State state) {
...
void NdbImportImpl::Team::wait_worker(Worker *w, WorkerState::State state) {
...
void NdbImportImpl::Team::do_run() {
  log_debug(1, "do_run");
  if (has_error()) {
    if (m_state != TeamState::State_stopped) m_state = TeamState::State_stop;
    return;
  }
  for (uint n = 0; n < m_workercnt; n++) {
    Worker *w = get_worker(n);
...
void NdbImportImpl::Team::check_workers() {
...
void NdbImportImpl::Team::do_stop() {
  log_debug(1, "do_stop");
  for (uint n = 0; n < m_workercnt; n++) {
    Worker *w = get_worker(n);
    stop_worker(w);
...
void NdbImportImpl::Team::stop_worker(Worker *w) {
...
void NdbImportImpl::Team::set_table(uint tabid) {
  (void)m_util.get_table(tabid);
  m_tabid = tabid;
}
...
NdbImportImpl::Worker::~Worker() { log_debug(1, "dtor"); }

void NdbImportImpl::Worker::do_start() {
  log_debug(1, "start");
  const Opt &opt = m_util.c_opt;
  uint timeout = opt.m_idlesleep;
  do_init();
...
NdbImportImpl::Worker *NdbImportImpl::Worker::next_worker() {
  Team &team = m_team;
...
const char *NdbImportImpl::g_str_state(JobState::State state) {
  const char *str = 0;
  switch (state) {
...
const char *NdbImportImpl::g_str_state(TeamState::State state) {
  const char *str = 0;
  switch (state) {
...
const char *NdbImportImpl::g_str_state(WorkerState::State state) {
  const char *str = 0;
  switch (state) {
    case WorkerState::State_null:
      str = "null";
...
void NdbImportImpl::Job::str_state(char *str) const {
  strcpy(str, g_str_state(m_state));
}

void NdbImportImpl::Team::str_state(char *str) const {
  strcpy(str, g_str_state(m_state));
}

void NdbImportImpl::Worker::str_state(char *str) const {
  strcpy(str, g_str_state(m_state));
}
...
NdbImportImpl::RandomInputTeam::RandomInputTeam(Job &job, uint workercnt)
...
void NdbImportImpl::RandomInputTeam::do_init() {
  log_debug(1, "do_init");
  set_table(m_job.m_tabid);
}

void NdbImportImpl::RandomInputTeam::do_end() {
  log_debug(1, "do_end");
...
void NdbImportImpl::RandomInputWorker::do_init() { log_debug(1, "do_init"); }

void NdbImportImpl::RandomInputWorker::do_run() {
  log_debug(2, "do_run");
  const Opt &opt = m_util.c_opt;
...
void NdbImportImpl::RandomInputWorker::do_end() { log_debug(1, "do_end"); }

NdbImportImpl::Row *NdbImportImpl::RandomInputWorker::create_row(
    uint64 rowid, const Table &table) {
  ...
  for (uint i = 0; i < attrcnt; i++) {
    ...
    switch (attr.m_type) {
      ...
      case NdbDictionary::Column::Varchar: {
        ...
        for (uint i = 0; i < len; i++) val[i] = keychr[i % keylen];
...
NdbImportImpl::CsvInputTeam::CsvInputTeam(Job &job, uint workercnt)
    : Team(job, "csv-input", workercnt), m_file(m_util, m_error) {
  // stats
  Stats &stats = m_job.m_stats;
  {
...
void NdbImportImpl::CsvInputTeam::do_init() {
  log_debug(1, "do_init");
  const Opt &opt = m_util.c_opt;
...
void NdbImportImpl::CsvInputTeam::do_end() {
  log_debug(1, "do_end");
  WorkerFile &file = m_file;
  if (file.do_close() == -1) {
    require(has_error());
...
NdbImportImpl::CsvInputWorker::~CsvInputWorker() { delete m_csvinput; }

void NdbImportImpl::CsvInputWorker::do_init() {
  log_debug(1, "do_init");
  const Opt &opt = m_util.c_opt;
  ...
  m_csvinput = new CsvInput(m_impl.m_csv, Name("csvinput", m_workerno), csvspec,
                            table, m_buf, rows_out, rows_reject, rowmap_in,
...
void NdbImportImpl::CsvInputWorker::do_run() {
  log_debug(2, "do_run");
  switch (m_inputstate) {
...
void NdbImportImpl::CsvInputWorker::do_end() { log_debug(1, "do_end"); }

void NdbImportImpl::CsvInputWorker::state_lock() {
...
void NdbImportImpl::CsvInputWorker::state_read() {
...
void NdbImportImpl::CsvInputWorker::state_waittail() {
...
void NdbImportImpl::CsvInputWorker::state_parse() {
...
void NdbImportImpl::CsvInputWorker::state_movetail() {
...
void NdbImportImpl::CsvInputWorker::state_eval() {
  log_debug(2, "state_eval");
  m_csvinput->do_eval();
  m_inputstate = InputState::State_send;
}

void NdbImportImpl::CsvInputWorker::state_send() {
...
const char *NdbImportImpl::g_str_state(InputState::State state) {
  const char *str = 0;
  switch (state) {
    case InputState::State_null:
      str = "null";
...
void NdbImportImpl::CsvInputWorker::str_state(char *str) const {
  sprintf(str, "%s/%s", g_str_state(m_state), g_str_state(m_inputstate));
}

// null output team

NdbImportImpl::NullOutputTeam::NullOutputTeam(Job &job, uint workercnt)
...
void NdbImportImpl::NullOutputTeam::do_init() { log_debug(1, "do_init"); }

void NdbImportImpl::NullOutputTeam::do_end() { log_debug(1, "do_end"); }
...
void NdbImportImpl::NullOutputWorker::do_init() { log_debug(1, "do_init"); }

void NdbImportImpl::NullOutputWorker::do_run() {
  log_debug(2, "do_run");
...
void NdbImportImpl::NullOutputWorker::do_end() { log_debug(1, "do_end"); }
...
NdbImportImpl::Op::~Op() { delete m_row; }
...
NdbImportImpl::Tx::Tx(DbWorker *w) : m_worker(w) {
  Stats &stats = w->m_team.m_job.m_stats;
  m_trans = 0;
  m_ops.set_stats(stats, "op-used");
}

NdbImportImpl::Tx::~Tx() { require(m_trans == 0); }
...
NdbImportImpl::DbTeam::DbTeam(Job &job, const char *name, uint workercnt)
    : Team(job, name, workercnt) {}

NdbImportImpl::DbTeam::~DbTeam() {}

NdbImportImpl::DbWorker::DbWorker(Team &team, uint n) : Worker(team, n) {
  m_ndb = 0;
...
int NdbImportImpl::DbWorker::create_ndb(uint transcnt) {
...
NdbImportImpl::Op *NdbImportImpl::DbWorker::alloc_op() {
...
void NdbImportImpl::DbWorker::free_op(Op *op) { m_op_free.push_back(op); }

NdbImportImpl::Tx *NdbImportImpl::DbWorker::start_trans() {
...
NdbImportImpl::Tx *NdbImportImpl::DbWorker::start_trans(const NdbRecord *keyrec,
                                                        const char *keydata,
                                                        uchar *xfrmbuf,
                                                        uint xfrmbuflen) {
  log_debug(2, "start_trans");
  ...
  NdbTransaction *trans =
      m_ndb->startTransaction(keyrec, keydata, xfrmbuf, xfrmbuflen);
  if (trans == 0) {
    return 0;
  }
  Tx *tx = tx_free.pop_front();
...
NdbImportImpl::Tx *NdbImportImpl::DbWorker::start_trans(uint nodeid,
                                                        uint instanceid) {
  log_debug(2, "start_trans");
  TxList &tx_free = m_tx_free;
  TxList &tx_open = m_tx_open;
...
void NdbImportImpl::DbWorker::close_trans(Tx *tx) {
...
NdbImportImpl::RelayOpTeam::RelayOpTeam(Job &job, uint workercnt)
...
void NdbImportImpl::RelayOpTeam::do_init() { log_debug(1, "do_init"); }

void NdbImportImpl::RelayOpTeam::do_end() {
  log_debug(1, "do_end");
...
void NdbImportImpl::RelayOpWorker::do_init() {
  log_debug(1, "do_init");
  create_ndb(1);
  const Opt &opt = m_util.c_opt;
  if (!opt.m_no_hint) {
...
void NdbImportImpl::RelayOpWorker::do_run() {
  log_debug(2, "do_run");
  switch (m_relaystate) {
...
void NdbImportImpl::RelayOpWorker::state_receive() {
...
void NdbImportImpl::RelayOpWorker::state_define() {
...
void NdbImportImpl::RelayOpWorker::state_send() {
  log_debug(2, "state_send");
  const Opt &opt = m_util.c_opt;
  uint nodecnt = m_impl.c_nodes.m_nodecnt;
  uint left = 0;
...
void NdbImportImpl::RelayOpWorker::do_end() {
  log_debug(1, "do_end");
  if (!has_error()) {
    require(m_tx_open.cnt() == 0);
...
const char *NdbImportImpl::g_str_state(RelayState::State state) {
  const char *str = 0;
  switch (state) {
    case RelayState::State_null:
      str = "null";
...
void NdbImportImpl::RelayOpWorker::str_state(char *str) const {
  sprintf(str, "%s/%s", g_str_state(m_state), g_str_state(m_relaystate));
}

// exec op team

NdbImportImpl::ExecOpTeam::ExecOpTeam(Job &job, uint workercnt)
...
NdbImportImpl::Worker *NdbImportImpl::ExecOpTeam::create_worker(uint n) {
  ExecOpWorker *w = 0;
  const Opt &opt = m_util.c_opt;
  if (opt.m_no_asynch)
...
void NdbImportImpl::ExecOpTeam::do_init() { log_debug(1, "do_init"); }

void NdbImportImpl::ExecOpTeam::do_end() {
  log_debug(1, "do_end");
  for (uint i = 0; i < m_impl.c_nodes.m_nodecnt; i++) {
...
void NdbImportImpl::ExecOpWorker::do_init() {
  log_debug(1, "do_init");
  const Nodes &c = m_impl.c_nodes;
...
void NdbImportImpl::ExecOpWorker::do_run() {
  log_debug(2, "do_run");
  switch (m_execstate) {
 ...
 * in state_define().
 */
void NdbImportImpl::ExecOpWorker::state_receive() {
  log_debug(2, "state_receive");
  const Opt &opt = m_util.c_opt;
...
void NdbImportImpl::ExecOpWorker::handle_error(Op *op) {
  m_rows_free.push_back(op->m_row);
  op->m_row = NULL;
  free_op(op);
}

void NdbImportImpl::ExecOpWorker::reject_row(Row *row, const Error &error) {
  const Opt &opt = m_util.c_opt;
  RowList &rows_reject = *m_team.m_job.m_rows_reject;
  rows_reject.lock();
  ...
  m_util.set_reject_row(rejectrow, m_team.m_job.m_runno, error, reject,
                        rejectlen);
  require(rows_reject.push_back(rejectrow));
  // error if rejects exceeded
  if (rows_reject.totcnt() > opt.m_rejects) {
    // set team level error
    m_util.set_error_data(m_error, __LINE__, 0, "reject limit %u exceeded",
                          opt.m_rejects);
  }
  rows_reject.unlock();
}
...
void NdbImportImpl::ExecOpWorkerSynch::do_end() {
  log_debug(1, "do_end/synch");
  if (!has_error()) {
    require(m_tx_open.cnt() == 0);
  } else if (m_tx_open.cnt() != 0 && m_execstate == ExecState::State_prepare) {
...
void NdbImportImpl::ExecOpWorkerSynch::state_define() {
...
void NdbImportImpl::ExecOpWorkerSynch::state_prepare() {
  log_debug(2, "state_prepare/synch");
  // nothing to do
  m_execstate = ExecState::State_send;
}

void NdbImportImpl::ExecOpWorkerSynch::state_send() {
...
void NdbImportImpl::ExecOpWorkerSynch::state_poll() {
...
void NdbImportImpl::ExecOpWorkerAsynch::do_end() {
...
void NdbImportImpl::ExecOpWorkerAsynch::asynch_callback(Tx *tx) {
  NdbTransaction *trans = tx->m_trans;
  ...
  if (ndberror.status == NdbError::Success) {
    ...
    m_util.set_error_ndb(error, __LINE__, ndberror, "permanent error");
    while (tx->m_ops.cnt() != 0) {
      Op *op = tx->m_ops.pop_front();
      require(op != 0);
      require(op->m_row != 0);
...
void NdbImportImpl::ExecOpWorkerAsynch::state_define() {
  ...
  while (m_ops.cnt() != 0) {
    ...
    if (table.m_autoIncAttrId != Inval_uint) {
      ...
      if (ai_value_not_provided ||
         ...
         */
        if (m_ndb->getAutoIncrementValue(table.m_tab, val, opt.m_ai_prefetch_sz,
                                         opt.m_ai_increment,
                                         opt.m_ai_offset) == -1) {
          const NdbError &ndberror = m_ndb->getNdbError();
          require(ndberror.code != 0);
          if (ndberror.status == NdbError::TemporaryError) {
            ...
            if (temperrors <= opt.m_temperrors) {
              log_debug(1,
                        "get autoincr try " << temperrors << ": " << ndberror);
              m_ops.push_front(op);
    ...
    for (uint j = 0; j < table.m_blobids.size(); j++) {
      ...
      NdbBlob::BlobAction rc = bh->preExecute(NdbTransaction::Commit);
...
void NdbImportImpl::ExecOpWorkerAsynch::state_prepare() {
  Tx *tx = m_tx_open.front();
  while (tx != 0) {
    const NdbTransaction::ExecType et = NdbTransaction::Commit;
    NdbTransaction *trans = tx->m_trans;
...
void NdbImportImpl::ExecOpWorkerAsynch::state_send() {
...
void NdbImportImpl::ExecOpWorkerAsynch::state_poll() {
...
const char *NdbImportImpl::g_str_state(ExecState::State state) {
  const char *str = 0;
  switch (state) {
    case ExecState::State_null:
      str = "null";
...
void NdbImportImpl::ExecOpWorker::str_state(char *str) const {
  sprintf(str, "%s/%s tx:free=%u,open=%u", g_str_state(m_state),
          g_str_state(m_execstate), m_tx_free.cnt(), m_tx_open.cnt());
}

// diag team

NdbImportImpl::DiagTeam::DiagTeam(Job &job, uint workercnt)
...
void NdbImportImpl::DiagTeam::do_init() {
  log_debug(1, "do_init");
  const Opt &opt = m_util.c_opt;
  if (opt.m_resume) {
    read_old_diags();
...
void NdbImportImpl::DiagTeam::read_old_diags(const char *name, const char *path,
                                             const Table &table,
                                             RowList &rows_out) {
  ...
  for (uint i = 0; i < 2; i++) {
    ...
    csvinput[i] =
        new CsvInput(m_impl.m_csv, Name(name, i), csvspec, table, buf[i],
                     rows_out, rows_reject, rowmap_in[i], m_job.m_stats);
    csvinput[i]->do_init();
  }
  {
...
void NdbImportImpl::DiagTeam::read_old_diags() {
  ...
  // result
  {
    ...
    while ((row = rows.pop_front()) != 0) {
      // runno
      {
        ...
        if (runno == Inval_uint32 || runno < x) runno = x;
      }
      m_util.free_row(row);
    }
    if (runno == Inval_uint32) {
      m_util.set_error_gen(m_error, __LINE__, "%s: no valid records found",
                           path);
      return;
    }
    job.m_runno = runno + 1;
  ...
  // rowmap
  {
    ...
    if (rowmap_in.empty()) {
      m_util.set_error_gen(m_error, __LINE__, "%s: no records for run %u found",
                           path, job.m_runno - 1);
      return;
    }
  ...
  if (opt.m_stats) {
    ...
    log_debug(1, "old stats:"
                     << " rows=" << old_rows << " reject=" << old_reject
                     << " runtime=" << old_runtime);
  }
}

void NdbImportImpl::DiagTeam::open_new_diags() {
...
void NdbImportImpl::DiagTeam::do_end() {
  log_debug(1, "do_end");
  const Opt &opt = m_util.c_opt;
...
void NdbImportImpl::DiagWorker::do_init() {
  log_debug(1, "do_init");
  const Opt &opt = m_util.c_opt;
  ...
  // result
  {
    ...
    m_result_csv = new CsvOutput(m_impl.m_csv, csvspec, table, m_result_buf);
  ...
  // reject
  {
    ...
    uint pagecnt = opt.m_pagecnt;
    buf.alloc(pagesize, pagecnt);
    m_reject_csv = new CsvOutput(m_impl.m_csv, csvspec, table, m_reject_buf);
  ...
  // rowmap
  {
    ...
    uint pagecnt = opt.m_pagecnt;
    buf.alloc(pagesize, pagecnt);
    m_rowmap_csv = new CsvOutput(m_impl.m_csv, csvspec, table, m_rowmap_buf);
  ...
  if (opt.m_stats) {
    ...
    m_stopt_csv = new CsvOutput(m_impl.m_csv, csvspec, table, m_stopt_buf);
    m_stopt_csv->do_init();
    if (!opt.m_resume) {
  ...
  if (opt.m_stats) {
    ...
    m_stats_csv = new CsvOutput(m_impl.m_csv, csvspec, table, m_stats_buf);
    m_stats_csv->do_init();
    if (!opt.m_resume) {
...
void NdbImportImpl::DiagWorker::do_run() {
  log_debug(2, "do_run");
  // reject
  write_reject();
  // stop by request
...
void NdbImportImpl::DiagWorker::do_end() {
  const Opt &opt = m_util.c_opt;
  log_debug(1, "do_end");
  // result
  write_result();
...
void NdbImportImpl::DiagWorker::write_result() {
  ...
  if (m_util.has_error()) {
    ...
    m_util.set_result_row(row, job.m_runno, name, desc, rows, reject,
                          temperrors, runtime, utime, error);
...
void NdbImportImpl::DiagWorker::write_stopt() {
  ...
  const struct ov_st {
    const char *m_option;
    uint m_value;
  } ov_list[] = {{"connections", opt.m_connections},
...
NdbImportImpl::Jobs::Jobs() { m_jobno = 0; }

NdbImportImpl::Jobs::~Jobs() {
  // XXX delete jobs
}

NdbImportImpl::Job *NdbImportImpl::create_job() {
  Jobs &jobs = c_jobs;
...
NdbImportImpl::Job *NdbImportImpl::find_job(uint jobno) {
  Job *job = 0;
...
void NdbImportImpl::start_job(Job *job) {
  NDB_THREAD_PRIO prio = NDB_THREAD_PRIO_MEAN;
  uint stack_size = 64 * 1024;
  job->m_thread =
      NdbThread_Create(start_job_c, (void **)job, stack_size, "job", prio);
  require(job->m_thread != 0);
}

void NdbImportImpl::stop_job(Job *job) {
  job->do_stop();
  log_debug(1, "done");
}

void NdbImportImpl::wait_job(Job *job) {
  const Opt &opt = m_util.c_opt;
...
void NdbImportImpl::destroy_job(Job *job) {
  Jobs &jobs = c_jobs;
  require(job != 0);
