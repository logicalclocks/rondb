...
#define log_debug(n, x) \
  ...
  (*m_util.c_log.out) << m_util.c_log.start << *this << __LINE__ << " " << x \
                      << m_util.c_log.stop
...
class NdbImportUtil {
 public:
  ...
  struct OptGuard {
    OptGuard(NdbImportUtil &util) : m_util(util) { m_opt_save = m_util.c_opt; }
    ~OptGuard() { m_util.c_opt = m_opt_save; }
  ...
  struct Table {
    Table();
    void add_pseudo_attr(const char *name, NdbDictionary::Column::Type type,
                         uint length = 1);
  ...
  int add_table(NdbDictionary::Dictionary *dic, const NdbDictionary::Table *tab,
  ...
  struct RowList : private List, Lockable {
    ...
    uint64 totcnt() const { return m_totcnt; }
  ...
  struct BlobList : private List, Lockable {
    BlobList();
    ~BlobList() override;
    void push_back(Blob *blob) { List::push_back(blob); }
    Blob *pop_front() { return static_cast<Blob *>(List::pop_front()); }
  };
  ...
  struct Range : ListEnt {
    ...
    bool equal(const Range &range2) const {
      return m_start == range2.m_start && m_end == range2.m_end &&
             m_startpos == range2.m_startpos && m_endpos == range2.m_endpos &&
        m_reject == range2.m_reject;
    }
    Range *next() { return static_cast<Range *>(m_next); }
    const Range *next() const { return static_cast<const Range *>(m_next); }
    Range *prev() { return static_cast<Range *>(m_prev); }
    const Range *prev() const { return static_cast<const Range *>(m_prev); }
  ...
  struct RangeList : private List, Lockable {
    Range *front() { return static_cast<Range *>(m_front); }
    const Range *front() const { return static_cast<const Range *>(m_front); }
    Range *back() { return static_cast<Range *>(m_back); }
    const Range *back() const { return static_cast<const Range *>(m_back); }
    void push_back(Range *range) { List::push_back(range); }
    void push_front(Range *range) { List::push_front(range); }
    ...
    Range *pop_front() { return static_cast<Range *>(List::pop_front()); }
    void remove(Range *r) { List::remove(r); }
    void push_back_from(RangeList &src) { List::push_back_from(src); }
    uint cnt() const { return m_cnt; }
    bool empty() const { return m_cnt == 0; }
    void validate() const { List::validate(); }
  };

  struct RowMap : Lockable {
    RowMap(NdbImportUtil &util);
    bool empty() const { return m_ranges.empty(); }
    uint size() const { return m_ranges.cnt(); }
    void clear() { m_ranges_free.push_back_from(m_ranges); }
    ...
    void free_range(Range *r) { m_ranges_free.push_back(r); }
#if defined(VM_TRACE) || defined(TEST_NDBIMPORTUTIL)
    void validate() const;
#else
    void validate() const {}
  ...
  struct ErrorMap : Lockable {
    ...
    uint get_sum() const {
      ...
      for (it = m_map.begin(); it != m_map.end(); it++) sum += it->second;
      return sum;
    }
  ...
  void set_result_row(Row *row, uint32 runno, const char *name,
                      const char *desc, uint64 rows, uint64 reject,
                      uint64 temperrors, uint64 runtime, uint64 utime,
                      const Error &error);

  void set_reject_row(Row *row, uint32 runno, const Error &error,
                      const char *reject, uint rejectlen);

  void set_rowmap_row(Row *row, uint32 runno, const Range &range);

  void set_stopt_row(Row *row, uint32 runno, const char *option, uint32 value);

  void set_stats_row(Row *row, uint32 runno, const Stat &stat, bool global);

  void set_error_attrs(Row *row, const Table &table, const Error &error,
                       uint &id);
   ...
   */
  struct Stat {
    Stat(Stats &stats, uint id, const char *name, uint parent, uint level,
         uint flags);
    void add(uint64 val);
    void reset();
    // meta
  ...
  struct Stats : Lockable {
    ...
    struct Validate {
      Validate(uint parent, uint id, uint level, bool *seen)
          : m_parent(parent), m_id(id), m_level(level), m_seen(seen) {}
      uint m_parent;
      uint m_id;
      uint m_cnt;
      uint m_level;
  ...
  bool has_error() { return has_error(c_error); }
...
NdbOut &operator<<(NdbOut &out,
                   const NdbImportUtil::DebugLogger::MessageStart &);
NdbOut &operator<<(NdbOut &out,
                   const NdbImportUtil::DebugLogger::MessageStop &);
