...
class ExtraTableInfo {
 public:
  ExtraTableInfo(const char *db_name, const char *table_name)
      : m_dbName(db_name), m_tableName(table_name) {}
...
class ExtraRestoreInfo {
 public:
   ...
   */
  ExtraTableInfo *findTable(const char *db_name, const char *table_name) {
    for (Uint32 i = 0; i < m_tables.size(); i++) {
   ...
   */
  ExtraTableInfo *findOrAddTable(const char *db_name, const char *table_name) {
...
static struct my_option my_long_options[] = {
    ...
    NDB_STD_OPT_DEBUG{
        ...
        (uchar **)&opt_timestamp_printouts, (uchar **)&opt_timestamp_printouts,
        0, GET_BOOL, NO_ARG, true, 0, 0, 0, 0, 0},
    {"connect", 'c', "same as --connect-string", &opt_ndb_connectstring,
     nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"nodeid", 'n', "Backup files from node with id", &ga_nodeId, nullptr,
     nullptr, GET_INT, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"backup-password", NDB_OPT_NOSHORT, "Encryption password for backup file",
     nullptr, nullptr, 0, GET_PASSWORD, OPT_ARG, 0, 0, 0, nullptr, 0,
     &opt_backup_password},
    ...
    {"restore_data", 'r',
     "Restore table data/logs into RonDB using NDB API", &_restore_data,
     nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"restore_meta", 'm', "Restore meta data into RonDB using NDB API",
     &_restore_meta, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"no-upgrade", 'u',
     "Don't upgrade array type for var attributes, which don't resize VAR data "
     "and don't change column attributes",
     &ga_no_upgrade, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"promote-attributes", 'A',
     "Allow attributes to be promoted when restoring data from backup",
     &ga_promote_attributes, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    {"lossy-conversions", 'L',
     ...
     &ga_demote_attributes, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    {"preserve-trailing-spaces", 'P',
     "Allow to preserve the tailing spaces (including paddings) When "
     "char->varchar or binary->varbinary is promoted",
     &_preserve_trailing_spaces, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0,
     0, 0},
    {"no-restore-disk-objects", 'd',
     "Dont restore disk objects (tablespace/logfilegroups etc)",
     &_no_restore_disk, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"restore_epoch", 'e',
     "Restore epoch info into the status table. Convenient for starting RonDB "
     "cluster replication. The row in " NDB_REP_DB "." NDB_APPLY_TABLE
     " with id 0 will be updated/inserted.",
     &ga_restore_epoch, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"with-apply-status", 'w',
     "Restore the " NDB_APPLY_TABLE " system table content from the backup.",
     &ga_with_apply_status, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    {"skip-table-check", 's',
    ...
    {"print_data", NDB_OPT_NOSHORT, "Print data to stdout", &_print_data,
     nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"print_meta", NDB_OPT_NOSHORT, "Print meta data to stdout", &_print_meta,
     nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"print_log", NDB_OPT_NOSHORT, "Print log to stdout", &_print_log, nullptr,
     nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"print_sql_log", NDB_OPT_NOSHORT, "Print SQL log to stdout",
     &_print_sql_log, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"backup_path", NDB_OPT_NOSHORT, "Path to backup files", &ga_backupPath,
     nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"dont_ignore_systab_0", 'f',
     "Do not ignore system table during --print-data.",
     &ga_dont_ignore_systab_0, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0,
     0, 0},
    {"ndb-nodegroup-map", OPT_NODEGROUP_MAP,
     "Nodegroup specification. Not supported anymore, value will be ignored.",
     nullptr, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"fields-enclosed-by", NDB_OPT_NOSHORT, "Fields are enclosed by ...",
     &opt_fields_enclosed_by, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0,
     0, 0, 0},
    {"fields-terminated-by", NDB_OPT_NOSHORT, "Fields are terminated by ...",
     &opt_fields_terminated_by, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},
    {"fields-optionally-enclosed-by", NDB_OPT_NOSHORT,
     "Fields are optionally enclosed by ...",
     &opt_fields_optionally_enclosed_by, nullptr, nullptr, GET_STR,
     REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"hex", NDB_OPT_NOSHORT, "print binary types in hex format",
     &opt_hex_format, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"tab", 'T',
     "Creates tab separated textfile for each table to "
     "given path. (creates .txt files)",
     &tab_path, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"append", NDB_OPT_NOSHORT, "for --tab append data to file", &opt_append,
     nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"lines-terminated-by", NDB_OPT_NOSHORT, "", &opt_lines_terminated_by,
     nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"progress-frequency", NDB_OPT_NOSHORT,
     "Print status uf restore periodically in given seconds",
     &opt_progress_frequency, nullptr, nullptr, GET_INT, REQUIRED_ARG, 0, 0,
     65535, 0, 0, 0},
    {"no-binlog", NDB_OPT_NOSHORT,
     "If a mysqld is connected and has binary log, do not log the restored "
     "data",
     &opt_no_binlog, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"verbose", OPT_VERBOSE, "verbosity", &opt_verbose, nullptr, nullptr,
    ...
    {"include-databases", OPT_INCLUDE_DATABASES,
     ...
     &opt_include_databases, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0,
     0, 0, 0},
    {"exclude-databases", OPT_EXCLUDE_DATABASES,
     "Comma separated list of databases to not restore. Example: db1,db3",
     &opt_exclude_databases, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0,
     0, 0, 0},
    {"rewrite-database", OPT_REWRITE_DATABASE,
     ...
     &opt_rewrite_database, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0,
     0, 0},
    {"include-tables", OPT_INCLUDE_TABLES,
     ...
     &opt_include_tables, nullptr, nullptr, GET_STR, REQUIRED_ARG, 0, 0, 0, 0,
     0, 0},
    ...
    {"exclude-missing-columns", NDB_OPT_NOSHORT,
     ...
     &ga_exclude_missing_columns, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0,
     0, 0, 0},
    {"exclude-missing-tables", NDB_OPT_NOSHORT,
     "Ignore tables present in backup but not in database",
     &ga_exclude_missing_tables, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0,
     0, 0},
    ...
    {"num_slices", NDB_OPT_NOSHORT, "How many slices are being applied",
     &ga_num_slices, nullptr, nullptr, GET_UINT, REQUIRED_ARG, 1, 1, 1024,
     nullptr, 0, nullptr},
    {"slice_id", NDB_OPT_NOSHORT, "My slice id", &ga_slice_id, nullptr, nullptr,
     GET_INT, REQUIRED_ARG, 0, 0, 1023, nullptr, 0, nullptr},
    {"allow-pk-changes", NDB_OPT_NOSHORT,
     "Allow changes to the set of columns making up a table's primary key.",
     &ga_allow_pk_changes, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    {"ignore-extended-pk-updates", NDB_OPT_NOSHORT,
     ...
     &ga_ignore_extended_pk_updates, nullptr, nullptr, GET_BOOL, NO_ARG, 0, 0,
     0, 0, 0, 0},
    {"remap-column", OPT_REMAP_COLUMN,
     "Remap content for column while "
     "restoring, format <database>.<table>.<column>:<function>:<function_args>."
     "  <database> is remapped name, remapping applied before other "
     "conversions.",
...
static bool parse_remap_option(const BaseString option, BaseString &db_name,
  ...
  const int numOptParts = option.split(optParts, BaseString(":"), 3);

  if (numOptParts < 2) {
    error_msg.assfmt(
        "remap-column : Badly formed option : %s.  "
        "Expected format : %s.",
        option.c_str(), expectedFormat);
    return false;
  }

  Vector<BaseString> nameParts;
  const int numNameParts = optParts[0].split(nameParts, BaseString("."));
  if (numNameParts != 3) {
    error_msg.assfmt(
        "remap-column : Badly formed column specifier : %s "
        "in option %s.  "
        "Expected format : %s.",
        optParts[0].c_str(), option.c_str(), expectedFormat);
    return false;
  }
...
static bool parse_remap_column(const char *argument) {
  ...
  if (!parse_remap_option(option, db, tab, col, func, args, error_msg)) {
  ...
  ExtraTableInfo *eti =
      g_extra_restore_info.findOrAddTable(db.c_str(), tab.c_str());
...
int makeInternalTableName(const BaseString &externalName,
                          BaseString &internalName) {
  // Make dbname.table1 into dbname/def/table1
  Vector<BaseString> parts;

  // Must contain a dot
  if (externalName.indexOf('.') == -1) return -1;
...
void processTableList(const char *str, Vector<BaseString> &lst) {
...
BaseString makeExternalTableName(const BaseString &internalName) {
  ...
  externalName.append(".");
  externalName.append(
      internalName.substr(idx + SCHEMA_NAME_SIZE, internalName.length()));
  return externalName;
}
...
bool readArguments(Ndb_opts &opts, char ***pargv) {
  ...
  if (!opt_decrypt) {
    ...
    if (!have_password_option) {
      err << "Decrypting backup (--decrypt) requires password "
             "(--backup-password)."
          << endl;
      exitHandler(NdbToolsProgramExitCode::WRONG_ARGS);
    }
  ...
  if (g_databases.size() > 0) {
    info << "WARNING! Using deprecated syntax for selective object restoration."
         << endl;
...
bool create_consumers(RestoreThreadData *data) {
  ...
  char threadname[20];
  BaseString::snprintf(threadname, sizeof(threadname), "%d-%u-%u", ga_nodeId,
                       data->m_part_id, ga_slice_id);
  BackupRestore *restore =
      new BackupRestore(g_cluster_connection, threadname, ga_nParallelism);
  ...
  if (ga_rebuild_indexes) {
    restore->m_metadata_work_requested = true;
    if (data->m_part_id == 1) restore->m_rebuild_indexes = true;
  }
...
void clear_consumers(RestoreThreadData *data) {
  for (Uint32 i = 0; i < data->m_consumers.size(); i++)
    delete data->m_consumers[i];
  data->m_consumers.clear();
}

static inline bool checkSysTable(const TableS *table) {
  return !table->getSysTable();
}

static inline bool checkSysTable(const RestoreMetaData &metaData, uint i) {
  assert(i < metaData.getNoOfTables());
  return checkSysTable(metaData[i]);
}

static inline bool isBlobTable(const TableS *table) {
...
static void parse_rewrite_database(char *argument) {
  ...
  if ((n == 2) && (args[0].length() > 0) && (args[1].length() > 0)) {
...
static bool check_include_exclude(BaseString database, BaseString table) {
  ...
  if (g_include_databases.size() != 0 || g_include_tables.size() != 0) {
    /*
  ...
  if (do_include &&
      (g_exclude_databases.size() != 0 || g_exclude_tables.size() != 0)) {
    /*
...
static bool check_intermediate_sql_table(const char *table_name) {
  BaseString tbl(table_name);
  Vector<BaseString> fields;
  tbl.split(fields, "/");
  if ((fields.size() == 3) && !fields[2].empty() &&
      strncmp(fields[2].c_str(), TMP_TABLE_PREFIX, TMP_TABLE_PREFIX_LEN) == 0)
    return true;
  return false;
}

static inline bool checkDoRestore(const TableS *table) {
  bool ret = true;
  BaseString db, tbl;
...
static inline bool checkDbAndTableName(const TableS *table) {
  if (table->isBroken()) return false;

  const char *table_name = getTableName(table);
  if (opt_exclude_intermediate_sql_tables &&
      (check_intermediate_sql_table(table_name) == true)) {
    return false;
  }
  ...
  if (g_tables.size() == 0 && g_databases.size() == 0) return true;

  if (g_databases.size() == 0) g_databases.push_back("TEST_DB");

  // Filter on the main table name for indexes and blobs
  unsigned i;
  for (i = 0; i < g_databases.size(); i++) {
    if (strncmp(table_name, g_databases[i].c_str(), g_databases[i].length()) ==
            0 &&
        table_name[g_databases[i].length()] == '/') {
      // we have a match
      if (g_databases.size() > 1 || g_tables.size() == 0) return true;
      break;
    }
  }
  if (i == g_databases.size()) return false;  // no match found
  ...
  for (i = 0; i < g_tables.size(); i++) {
    if (strcmp(table_name, g_tables[i].c_str()) == 0) return true;
  }
  return false;
}
...
static TableS *find_table_spec(RestoreMetaData &metaData,
  ...
  for (Uint32 m = 0; m < metaData.getNoOfTables(); m++) {
    ...
    BaseString dbName, tabName;
    {
      ...
      if (externalName.split(components, BaseString(".")) != 2) {
        restoreLogger.log_info(
            "Error processing table name from "
            "backup %s from %s",
            externalName.c_str(), tableSpec->getTableName());
        return NULL;
      }
      dbName = components[0];
    ...
    if (dbName == searchDbName && tabName == searchTableName) {
      return tableSpec;
    }
  }
...
class OffsetTransform : public ColumnTransform {
 public:
  static OffsetTransform *parse(const NdbDictionary::Column *col,
...

    {
      ...
      char numbuf[22];
      BaseString::snprintf(numbuf, sizeof(numbuf), "%lld", offset_val);
    ...
    if (offset_val < min_sval || offset_val > max_sval) {
      error_msg.assfmt("Offset %lld is out of range for type.", offset_val);
      return NULL;
    }

    return new OffsetTransform(offset_val, sig, bits, min_sval, max_sval,
                               max_uval);
  }
 ...
 private:
  ...
  OffsetTransform(Int64 offset_val, bool sig, Uint32 bits, Int64 min_sval,
                  Int64 max_sval, Uint64 max_uval)
      : m_offset_val(offset_val), m_sig(sig), m_bits(bits) {
  ...
  bool apply(const NdbDictionary::Column *col, const void *src_data,
    ...
    if (m_sig) {
      ...
      if (unlikely(!src_in_bounds)) {
        fprintf(stderr,
                "Offset : Source value out of bounds : adding %lld to %lld "
                "gives an out of bounds value\n",
                m_offset_val, src_val);
      ...
      if (unlikely(!src_in_bounds)) {
        fprintf(stderr,
                "Offset : Source value out of bounds : adding %lld to %llu "
                "gives an out of bounds value\n",
                m_offset_val, src_val);
...
static ColumnTransform *create_column_transform(
    const NdbDictionary::Column *col, const BaseString &func_name,
    const BaseString &func_args, BaseString &error_msg) {
...
static bool setup_one_remapping(TableS *tableSpec, const BaseString &col_name,
                                const BaseString &func_name,
                                const BaseString &func_args,
                                BaseString &error_msg) {
  ...
  if (!col) {
    error_msg.assfmt("Failed to find column %s in table", col_name.c_str());
    return false;
  }
  ...
  if (ad->transform != NULL) {
    error_msg.assfmt("Duplicate remappings on column %s", col_name.c_str());
    return false;
  }

  restoreLogger.log_debug(
      "Initialising remap function "
      "\"%s:%s\" on column %s.%s",
      func_name.c_str(), func_args.c_str(), tableSpec->m_dictTable->getName(),
      col_name.c_str());

  ColumnTransform *ct =
      create_column_transform(col, func_name, func_args, error_msg);
...
static bool setup_column_remappings(RestoreMetaData &metaData) {
  for (Uint32 t = 0; t < g_extra_restore_info.m_tables.size(); t++) {
    const ExtraTableInfo *eti = g_extra_restore_info.m_tables[t];

    TableS *tableSpec = find_table_spec(metaData, eti->m_dbName.c_str(),
    ...
    if (tableSpec) {
      ...
      for (Uint32 a = 0; a < eti->m_remapColumnArgs.size(); a++) {
        ...
        if (!parse_remap_option(eti->m_remapColumnArgs[a], db_name, tab_name,
                                col_name, func_name, func_args, error_msg)) {
        ...
        if (!setup_one_remapping(tableSpec, col_name, func_name, func_args,
                                 error_msg)) {
          restoreLogger.log_info(
              "remap_column : Failed with \"%s\" "
              "while processing option \"%s\"",
              error_msg.c_str(), eti->m_remapColumnArgs[a].c_str());
          return false;
        }
        ...
        if (col_in_pk && haveBlobPartTables) {
          ...
          for (Uint32 b = 0; b < blobTables.size(); b++) {
            ...
            const NdbDictionary::Column *mainTabBlobCol =
                tableSpec->m_dictTable->getColumn(
                    blobPartTableSpec->getMainColumnId());

            if (unlikely(mainTabBlobCol->getBlobVersion() == NDB_BLOB_V1)) {
              restoreLogger.log_info(
                  "remap_column : Failed as table has "
                  "v1 Blob column %s when processing "
                  "option %s",
                  mainTabBlobCol->getName(), eti->m_remapColumnArgs[a].c_str());
              return false;
            }

            if (!setup_one_remapping(blobPartTableSpec, col_name.c_str(),
                                     func_name, func_args, error_msg)) {
              restoreLogger.log_info(
                  "remap_column : Failed with error %s "
                  "while applying remapping to blob "
                  "parts table %s from option : %s",
                  error_msg.c_str(), blobPartTableSpec->m_dictTable->getName(),
                  eti->m_remapColumnArgs[a].c_str());
              return false;
            }
          }
      ...
      restoreLogger.log_info(
          "remap_column : Failed to find table in Backup "
          "matching option : \"%s\"",
...
static void init_restore() {
  if (_restore_meta || _restore_data || ga_restore_epoch ||
    ...
    g_cluster_connection =
        new Ndb_cluster_connection(opt_ndb_connectstring, opt_ndb_nodeid);
...
static void init_progress() { g_report_prev = NdbTick_getCurrentTicks(); }

static int check_progress() {
  if (!opt_progress_frequency) return 0;
...
static void report_progress(const char *prefix, const BackupFile &f) {
  ...
  if (f.get_file_size())
    restoreLogger.log_info(
        "%s %llupercent(%llu bytes)\n", prefix,
        (f.get_file_pos() * 100 + f.get_file_size() - 1) / f.get_file_size(),
        f.get_file_pos());
  else
 ...
 */
static void check_data_truncations(const TableS *table) {
  assert(table);
 ...
 */
static bool determine_slice_skip_fragment(TableS *table, Uint32 fragmentId,
  ...
  restoreLogger.log_debug(
      "Table : %s blobRelated : %u frag id : %u "
      "slice id : %u fragmentRestoreSlice : %u "
      "apply : %u",
      table->m_dictTable->getName(), table->isBlobRelated(), fragmentId,
      ga_slice_id, fragmentRestoreSlice, (fragmentRestoreSlice == ga_slice_id));
 ...
 */
static bool check_slice_skip_fragment(const TableS *table, Uint32 fragmentId) {
  if (ga_num_slices == 1) {
    /* No slicing */
    return false;
  }
...
int detect_backup_format() {
  ...
  char name[PATH_MAX];
  const Uint32 sz = sizeof(name);
  BaseString::snprintf(name, sz, "%s%sBACKUP-%u.%d.ctl", ga_backupPath,
  ...
  if (my_stat(name, &buf, 0)) {
    ...
    for (ga_part_count = 1; ga_part_count <= g_max_parts; ga_part_count++) {
      // backup parts are named as
      // BACKUP-<backupid>-PART-<part_id>-OF-<total_parts> E.g. Part 2 of backup
      // 3 which has 4 parts will be in the path
      //    BACKUP-3/BACKUP-3-PART-2-OF-4/
      // Try out different values of <total_parts> for PART-1 until correct
      // total found E.g. for total = 4,
...
int main(int argc, char **argv) {
  ...
  if (_restore_meta) g_options.appfmt(" -m");
  if (ga_no_upgrade) g_options.appfmt(" -u");
  if (ga_promote_attributes) g_options.appfmt(" -A");
  if (ga_demote_attributes) g_options.appfmt(" -L");
  if (_preserve_trailing_spaces) g_options.appfmt(" -P");
  if (ga_skip_table_check) g_options.appfmt(" -s");
  if (_restore_data) g_options.appfmt(" -r");
  if (ga_restore_epoch) g_options.appfmt(" -e");
  if (ga_with_apply_status) {
  ...
  if (ga_num_slices > 1) {
    g_options.appfmt(" --num-slices=%u --slice-id=%u", ga_num_slices,
                     ga_slice_id);
  }
  ...
  if ((ga_slice_id < 0) || (ga_slice_id >= ga_num_slices)) {
    err << "Slice id " << ga_slice_id << " out of range (0-" << ga_num_slices
        << ")" << endl;
    exitHandler(NdbToolsProgramExitCode::WRONG_ARGS);
  } else {
    if (ga_num_slices > 1) {
  ...
  if (_print || _print_meta || _print_data || _print_log || _print_sql_log ||
    ...
    for (int i = 1; i <= ga_part_count; i++) {
      ...
      if (do_restore(&thrdata) == NdbToolsProgramExitCode::FAILED) {
        if (ga_backup_format == BF_SINGLE) {
          ...
          info << "Failed to restore BACKUP-" << ga_backupId << "-PART-" << i
               << "-OF-" << ga_part_count << endl;
        }
        ga_error_thread = i;
    ...
    for (int part_id = 1; part_id <= ga_part_count; part_id++) {
      ...
      NdbThread *thread = NdbThread_Create(
          start_restore_worker, (void **)(data), stack_size, name, prio);
      if (!thread) {
    ...
    for (Uint32 i = 0; i < thrdata.size(); i++) {
      void *status;
      if (ga_error_thread > 0) barrier.cancel();
