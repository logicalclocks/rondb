...
void BackupFile::twiddle_atribute(const AttributeDesc *const attr_desc,
  ...
  if (!aligned) {
    ...
    memcpy(attr_data->void_value, m_twiddle_buffer.get_data(), data_sz);
  }
}
...
bool BackupFile::Twiddle(const AttributeDesc *const attr_desc,
                         AttributeData *attr_data) {
...
Uint32 RestoreMetaData::getStopGCP() const { return m_stopGCP; }

int RestoreMetaData::loadContent() {
  ...
  if (!fixBlobs()) return 0;
  if (!readGCPEntry()) return 0;

  if (!readFragmentInfo()) return 0;
  return 1;
}

Uint32 RestoreMetaData::readMetaTableList() {
    ...
    if (count < 0) {
      if (!m_xfile.is_encrypted()) {
        restoreLogger.log_error(
            "readMetaTableList read tabCount error, corrupt file?");
      } else {
        restoreLogger.log_error(
            "readMetaTableList read tabCount error, wrong password or corrupt "
            "file?");
      }
      return 0;
    }
...
bool RestoreMetaData::readMetaTableDesc() {
  ...
  switch (obj.m_objType) {
    ...
    case DictTabInfo::Tablespace: {
      ...
      obj.m_objPtr = dst;
      restoreLogger.log_debug("%p %u %s", obj.m_objPtr, dst->getObjectId(),
                              dst->getName());
      break;
    }
    case DictTabInfo::LogfileGroup: {
    ...
    default:
      if (ga_skip_unknown_objects) {
        restoreLogger.log_info(
            "Skipping schema object with unknown table type %u",
            sectionInfo[2]);
        return true;
      } else {
  ...
  if (errcode) {
    restoreLogger.log_error("Unable to parse dict info...%u %u", sectionInfo[2],
                            errcode);
    return false;
  }
...
bool RestoreMetaData::markSysTables() {
  Uint32 i;
  for (i = 0; i < getNoOfTables(); i++) {
    ...
    if (  // XXX should use type
        ...
        */
        strcmp(tableName, "cluster_replication/def/" OLD_NDB_APPLY_TABLE) ==
            0 ||
  ...
  for (i = 0; i < getNoOfTables(); i++) {
    ...
    if ((sscanf(auxTableName, indxPattern, &id1) == 1) ||
      ...
      if (mainTable) {
        ...
        restoreLogger.log_error("Restore: Bad primary table id in %s",
                                auxTableName);
        return false;
      }
    }
...
bool RestoreMetaData::readGCPEntry() {
...
TableS::~TableS() {
  for (Uint32 i = 0; i < allAttributesDesc.size(); i++) {
    if (allAttributesDesc[i]->parameter) free(allAttributesDesc[i]->parameter);
...
bool RestoreMetaData::parseTableDescriptor(const Uint32 *data, Uint32 len) {
  ...
  restoreLogger.log_debug(
      ...
      table->getTableId(), table->getNoOfAttributes());
...
TupleS &TupleS::operator=(const TupleS &tuple) {
  ...
  if (allAttrData)
    memcpy(allAttrData, tuple.allAttrData,
           getNoOfAttributes() * sizeof(AttributeData));
...
TableS *TupleS::getTable() const { return m_currentTable; }

AttributeDesc *TupleS::getDesc(int i) const {
  return m_currentTable->allAttributesDesc[i];
}

AttributeData *TupleS::getData(int i) const { return &(allAttrData[i]); }

bool TupleS::prepareRecord(TableS &tab) {
  if (allAttrData) {
...
bool applyColumnTransform(const NdbDictionary::Column *col,
  ...
  if (dst_ptr == NULL) {
    ...
    switch (col->getArrayType()) {
      ...
      case NDB_ARRAYTYPE_MEDIUM_VAR:
        attr_data->size = 2 + size_t(dst_char[0]) + (256 * size_t(dst_char[1]));
        break;
      default:
        /* No change */
        break;
  ...
  assert(attr_data->size <= 4 * ((((Uint32)col->getSizeInBytes()) + 3) / 4));

  return true;
}

const TupleS *RestoreDataIterator::getNextTuple(int &res,
                                                const bool skipFragment) {
    ...
    if (m_current_table_has_transforms) {
      for (int i = 0; i < m_currentTable->getNoOfAttributes(); i++) {
        ...
        const NdbDictionary::Column *col =
            m_currentTable->m_dictTable->getColumn(i);
...
int RestoreDataIterator::readTupleData_packed(Uint32 *buf_ptr,
   ...
   */
  Uint32 rp = *ptr;
  if (unlikely(!m_hostByteOrder)) rp = Twiddle32(rp);
    ...
    switch (align) {
      case DictTabInfo::aBit: {  // Bit
        ...
        if (m_hostByteOrder) {
          ...
          Uint32 ii;
          for (ii = 0; ii < (1 + len32); ii++) src32[ii] = Twiddle32(src32[ii]);
...
int RestoreDataIterator::readTupleData_old(Uint32 *buf_ptr, Uint32 dataLength) {
  Uint32 *ptr = buf_ptr;
  ...
  if (!ndbd_drop6(m_currentTable->backupVersion)) {
    if ((res = readVarData(buf_ptr, ptr, dataLength))) return res;
  } else {
    if ((res = readVarData_drop6(buf_ptr, ptr, dataLength))) return res;
  }

  return 0;
}

int RestoreDataIterator::readVarData(Uint32 *buf_ptr, Uint32 *ptr,
                                     Uint32 dataLength) {
...
int RestoreDataIterator::readVarData_drop6(Uint32 *buf_ptr, Uint32 *ptr,
                                           Uint32 dataLength) {
  Uint32 i;
  for (i = 0; i < m_currentTable->m_variableAttribs.size(); i++) {
    const Uint32 attrId = m_currentTable->m_variableAttribs[i]->attrId;
...
bool BackupFile::validateBackupFile() {
  if (!m_buffer) {
...
bool BackupFile::openFile() {
  ...
  r = m_xfile.open(
      m_file,
      reinterpret_cast<const byte *>(g_backup_password_state.get_password()),
      g_backup_password_state.get_password_length());
  bool fail = (r != 0);
  if (g_backup_password_state.get_password() != nullptr) {
    if (!m_xfile.is_encrypted()) {
      restoreLogger.log_error(
          "Decryption requested but file not "
          "encrypted.");
      fail = true;
    } else if (r != 0) {
      restoreLogger.log_error(
          "Can not read decrypted file. Might be wrong password.");
    }
  } else {
    if (m_xfile.is_encrypted()) {
      restoreLogger.log_error(
          "File is encrypted but no decryption "
          "requested.");
      fail = true;
    } else if (r != 0) {
...
int BackupFile::buffer_get_ptr_ahead(void **p_buf_ptr, Uint32 size,
  ...
  if (sz > m_buffer_data_left) {
    ...
    if (m_is_undolog) {
      ...
      int r = 0;
      /* move the left data to the end of buffer
       * m_buffer_ptr point the end of the left data. buffer_data_start point
       * the start of left data m_buffer_data_left is the length of left data.
       */
       ...
       * If the length of left log entries less than the residual length of
       buffer,
       *   we just need to read all the left entries from log file into the
       buffer.
       *   and all the left entries in log file should been read into buffer. Or
       * If the length of left entries is bigger than the residual length of
       buffer,
      ...
      if (m_file_pos > sizeof(m_fileHeader)) {
        ...
        if (file_left_entry_data <= buffer_free_space) {
          ndbxfrm_output_reverse_iterator out(
              (unsigned char *)buffer_data_start,
              (unsigned char *)buffer_data_start - file_left_entry_data, false);
          ...
          ndbxfrm_output_reverse_iterator out(
              (unsigned char *)m_buffer + buffer_free_space,
              (unsigned char *)m_buffer, false);
          byte *out_beg = out.begin();
          r = m_xfile.read_backward(&out);
          if (r == -1) abort();  // TODO
          r = out_beg - out.begin();
      ...
      ndbxfrm_output_iterator it = {
          static_cast<byte *>(m_buffer) + m_buffer_data_left,
          static_cast<byte *>(m_buffer) + m_buffer_sz, false};
    ...
    if (sz > m_buffer_data_left) sz = size * (m_buffer_data_left / size);
  }
...
void BackupFile::setCtlFile(Uint32 nodeId, Uint32 backupId, const char *path) {
  m_nodeId = nodeId;
  ...
  if (m_part_count > 1) {
    ...
    BaseString::snprintf(multiset_name, sizeof(multiset_name),
                         "BACKUP-%u-PART-%u-OF-%u%s%s", backupId, m_part_id,
                         m_part_count, DIR_SEPARATOR, name);
...
void BackupFile::setDataFile(const BackupFile &bf, Uint32 no) {
  ...
  if (bf.m_part_count > 1) {
    BaseString::snprintf(
        name, sz, "BACKUP-%u-PART-%d-OF-%d%sBACKUP-%u-%u.%d.Data", backupId,
        bf.m_part_id, bf.m_part_count, DIR_SEPARATOR, backupId, no, m_nodeId);
  } else {
    BaseString::snprintf(name, sz, "BACKUP-%u-%u.%d.Data", backupId, no,
                         m_nodeId);
  }
...
void BackupFile::setLogFile(const BackupFile &bf, Uint32 no) {
  ...
  if (bf.m_part_count > 1) {
    BaseString::snprintf(name, sz, "BACKUP-%u-PART-%d-OF-%d%sBACKUP-%u.%d.log",
                         backupId, bf.m_part_id, bf.m_part_count, DIR_SEPARATOR,
                         backupId, m_nodeId);
  } else {
    BaseString::snprintf(name, sz, "BACKUP-%u.%d.log", backupId, m_nodeId);
  }
  setName(bf.m_path, name);
}
...
bool BackupFile::readHeader() {
  ...
  */
  ...
  if (backup_version >= NDBD_RAW_LCP) {
    int r =
        buffer_read(&m_fileHeader.NdbVersion, sizeof(m_fileHeader) - oldsz, 1);
    if (r != 1) {
      if (!m_xfile.is_encrypted()) {
        restoreLogger.log_error("readDataFileHeader: Error reading header");
      } else {
  ...
  restoreLogger.log_debug("FileHeader: %s %u %u %u %u %u %u %u %u",
                          m_fileHeader.Magic, m_fileHeader.BackupVersion,
                          m_fileHeader.SectionType, m_fileHeader.SectionLength,
                          m_fileHeader.FileType, m_fileHeader.BackupId,
                          m_fileHeader.BackupKey_0, m_fileHeader.BackupKey_1,
  ...
  if (m_fileHeader.FileType == BackupFormat::UNDO_FILE) {
    ...
    */
    ...
    ndbxfrm_output_reverse_iterator out(
        (unsigned char *)m_buffer + m_buffer_sz,
        (unsigned char *)m_buffer + m_buffer_sz - sz, false);
    byte *out_beg = out.begin();
...
bool BackupFile::validateFooter() { return true; }
...
bool RestoreDataIterator::readFragmentHeader(int &ret, Uint32 *fragmentId) {
  ...
  while (1) {
    ...
    if (r < 0) {
      if (!m_xfile.is_encrypted()) {
        ...
        restoreLogger.log_error(
            "readFragmentHeader: Error reading header. "
            "Wrong password?");
      }
      ret = -2;
      return false;
  ...
  restoreLogger.log_debug("FragmentHeader: %u %u %u %u %u", Header.SectionType,
                          Header.SectionLength, Header.TableId,
  ...
  restoreLogger.log_info(
      ...
      m_currentTable->getTableName(), Header.TableId, Header.FragmentNo);
...
bool RestoreDataIterator::validateFragmentFooter() {
...
void TableS::createAttr(NdbDictionary::Column *column) {
  ...
  if (d->m_column->getAutoIncrement()) m_auto_val_attrib = d;
...
bool TableS::get_auto_data(const TupleS &tuple, Uint32 *syskey,
                           Uint64 *nextid) const {
  /*
...
Uint16 Twiddle16(Uint16 in) {
  ...
  retVal = ((in & 0xFF00) >> 8) | ((in & 0x00FF) << 8);
...
Uint64 Twiddle64(Uint64 in) {
  ...
  retVal = ((in & (Uint64)0x00000000000000FFLL) << 56) |
...
const LogEntry *RestoreLogIterator::getNextLogEntry(int &res) {
   ...
   * The backup have collected 'before values' for undoing 'delete+update' to
   * make this work. To undo insert, we only need primary key.
   */
  switch (triggerEvent) {
...
#endif
  assert(
      (desc.m_column->getArrayType() == NdbDictionary::Column::ArrayTypeFixed)
          ? (data.size == length)
          : (data.size <= length));
...
NdbOut &operator<<(NdbOut &ndbout, const TupleS &tuple) {
  for (int i = 0; i < tuple.getNoOfAttributes(); i++) {
    if (i > 0) ndbout << g_ndbrecord_print_format.fields_terminated_by;
...
NdbOut &operator<<(NdbOut &ndbout, const LogEntry &logE) {
  ...
  for (Uint32 i = 0; i < logE.size(); i++) {
    ...
    ndbout << (*attr);
    if (i < (logE.size() - 1)) ndbout << ", ";
  }
  return ndbout;
}

void AttributeS::printAttributeValue() const {
  NdbDictionary::Column::Type columnType = this->Desc->m_column->getType();
  switch (columnType) {
...
void LogEntry::printSqlLog() const {
