...
NdbImport::NdbImport() : m_impl(*new NdbImportImpl(*this)) {}

NdbImport::NdbImport(NdbImportImpl &impl) : m_impl(impl) {}
...
int NdbImport::set_opt(Opt &opt) {
  ...
  if (opt.m_input_type != 0) {
    ...
    if (strcmp(opt.m_input_type, "csv") == 0 && opt.m_input_workers < 2) {
      util.set_error_usage(util.c_error, __LINE__,
                           "number of csv input workers must be >= 2");
      return -1;
    }
    if (strcmp(opt.m_input_type, "random") == 0 && opt.m_rowbatch == 0) {
      util.set_error_usage(util.c_error, __LINE__,
  ...
  if (opt.m_output_type != 0) {
    ...
    if (strcmp(opt.m_output_type, "ndb") == 0 && opt.m_db_workers < 1) {
      util.set_error_usage(util.c_error, __LINE__,
  ...
  if (opt.m_ai_prefetch_sz == 0 || opt.m_ai_increment == 0 ||
      opt.m_ai_offset == 0) {
    util.set_error_usage(util.c_error, __LINE__,
...
int NdbImport::do_connect() {
  if (m_impl.do_connect() == -1) return -1;
  if (m_impl.get_nodes(m_impl.c_nodes) == -1) return -1;
  return 0;
}

void NdbImport::do_disconnect() { m_impl.do_disconnect(); }

// table

int NdbImport::add_table(const char *database, const char *table, uint &tabid) {
  return m_impl.add_table(database, table, tabid, m_impl.m_error);
}

int NdbImport::remove_table(const uint table_id) {
...
NdbImport::Job::Job(NdbImport &imp) : m_imp(imp) {
  m_jobno = Inval_uint;
  m_runno = Inval_uint;
...
int NdbImport::Job::do_create() {
  ...
  for (uint i = 0; i < m_teamcnt; i++) m_teams[i] = new Team(*this, i);
  m_status = JobStatus::Status_created;
  return 0;
}

int NdbImport::Job::do_start() {
  NdbImportImpl &impl = m_imp.m_impl;
...
int NdbImport::Job::add_table(const char *database, const char *table,
                              uint &tabid) {
  NdbImportImpl &impl = m_imp.m_impl;
  NdbImportImpl::Job *jobImpl = impl.find_job(m_jobno);
...
int NdbImport::Job::remove_table(const uint table_id) {
  NdbImportImpl &impl = m_imp.m_impl;
  NdbImportImpl::Job *jobImpl = impl.find_job(m_jobno);
  return jobImpl->remove_table(table_id);
}

void NdbImport::Job::set_table(uint tabid) {
  NdbImportImpl &impl = m_imp.m_impl;
  NdbImportImpl::Job *jobImpl = impl.find_job(m_jobno);
  jobImpl->set_table(tabid);
}

bool NdbImport::Job::has_error() const {
  NdbImportImpl &impl = m_imp.m_impl;
...
const Error &NdbImport::Job::get_error() const {
  NdbImportImpl &impl = m_imp.m_impl;
  NdbImportImpl::Job *jobImpl = impl.find_job(m_jobno);
...
void NdbImport::Job::get_status() {
  ...
  if (m_stats.m_runtime == 0) m_stats.m_runtime = 1;
  ...
  if (jobImpl->has_error()) {
    ...
    if (jobImpl->m_fatal) m_status = JobStatus::Status_fatal;
  }
  m_str_status = g_str_status(m_status);
}

NdbImport::Team::Team(const Job &job, uint teamno)
    : m_job(job), m_teamno(teamno) {
  m_status = TeamStatus::Status_null;
  m_str_status = g_str_status(m_status);
}

const char *NdbImport::Team::get_name() {
...
bool NdbImport::Team::has_error() const {
  NdbImportImpl &impl = m_job.m_imp.m_impl;
  NdbImportUtil &util = impl.m_util;
  NdbImportImpl::Job *jobImpl = impl.find_job(m_job.m_jobno);
  NdbImportImpl::Team *teamImpl = jobImpl->m_teams[m_teamno];
...
const NdbImport::Error &NdbImport::Team::get_error() const {
  NdbImportImpl &impl = m_job.m_imp.m_impl;
  NdbImportImpl::Job *jobImpl = impl.find_job(m_job.m_jobno);
  NdbImportImpl::Team *teamImpl = jobImpl->m_teams[m_teamno];
...
const char *NdbImport::Error::gettypetext() const {
  const char *typetext = "";
  switch (type) {
  case Type_noerror:
    typetext = "noerror";
...
bool NdbImport::has_error() const {
  NdbImportImpl &impl = m_impl;
  NdbImportUtil &util = impl.m_util;
  return util.has_error();
}

const NdbImport::Error &NdbImport::get_error() const {
  NdbImportImpl &impl = m_impl;
  NdbImportUtil &util = impl.m_util;
  return util.c_error;
}

NdbOut &operator<<(NdbOut &out, const NdbImport::Error &error) {
  const char *typetext = error.gettypetext();
  out << "error[" << typetext << "-" << error.code << "]";
  if (error.text[0] != 0) out << ": " << error.text;
  out << " (source:" << error.line << ")";
  return out;
}

void NdbImport::set_stop_all() { NdbImportUtil::g_stop_all = true; }
