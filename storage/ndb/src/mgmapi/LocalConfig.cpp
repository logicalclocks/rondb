...
bool LocalConfig::init(const char *connectString, const char *fileName) {
  /**
   * Escalation:
...
bool LocalConfig::parseHostName(const char *value) {
  ...
  if (bind_address.length()) mgmtSrvrId.bind_address.assign(bind_address);
...
bool LocalConfig::parseString(const char *connectString, BaseString &err) {
  char *for_strtok;
  ...
  for (char *tok = my_strtok_r(copy, ";,", &for_strtok); tok != nullptr;
    ...
    for (size_t i = 0; i < std::size(param_prefixes); i++) {
      ok = false;
      if (strncmp(tok, param_prefixes[i].prefix,
...
bool LocalConfig::readConnectString(const char *connectString,
                                    const char *info) {
  BaseString err;
...
char *LocalConfig::makeConnectString(char *buf, int sz) {
  ...
  if (p < sz && bind_address.length()) {
    ...
    char *sockaddr_string = Ndb_combine_address_port(
        addrbuf, sizeof(addrbuf), bind_address.c_str(), bind_address_port);
  ...
  if (p < sz)
    for (unsigned i = 0; i < ids.size(); i++) {
      if (ids[i].type != MgmId_TCP) continue;
      int new_p = 0;
      char *sockaddr_string = Ndb_combine_address_port(
          addrbuf, sizeof(addrbuf), ids[i].name.c_str(), ids[i].port);

      new_p = p + BaseString::snprintf(buf + p, sz - p, ",%s", sockaddr_string);
      if (new_p < sz)
        p = new_p;
      else {
        buf[p] = 0;
      ...
      if (!bind_address.length() && ids[i].bind_address.length()) {
        char *sockaddr_string = Ndb_combine_address_port(
            addrbuf, sizeof(addrbuf), ids[i].bind_address.c_str(),
