 ...
 */
bool check_param(const ConfigInfo::ParamInfo &param) {
  ...
  if (strcmp(section, "NDBD") != 0) fprintf(config_file, "[ndbd]\n");
  if (strcmp(param._fname, "NoOfReplicas") != 0)
    fprintf(config_file, "NoOfReplicas=1\n");

  if (strcmp(section, "NDB_MGMD") != 0) fprintf(config_file, "[ndb_mgmd]\n");
  if (strcmp(param._fname, "Hostname") != 0)
    fprintf(config_file, "HostName=localhost\n");

  if (strcmp(section, "MYSQLD") != 0) fprintf(config_file, "[mysqld]\n");
...
Config *create_config(const char *first, ...) {
  va_list args;
...
Config *create_mycnf(const char *first, ...) {
  va_list args;

  NdbDir::Temp tempdir;
  BaseString mycnf_file;
  mycnf_file.assfmt("%s%stest_my.%u.cnf", tempdir.path(), DIR_SEPARATOR,
                    ndb_procid());
...
void diff_config(void) {
  Config *c1 = create_config("[ndbd]", "NoOfReplicas=1", "[ndb_mgmd]",
                             "HostName=localhost", "[mysqld]", NULL);
  CHECK(c1);
  Config *c2 =
      create_config("[ndbd]", "NoOfReplicas=1", "[ndb_mgmd]",
                    "HostName=localhost", "[mysqld]", "[mysqld]", NULL);
  CHECK(c2);
...

  {
    ...
    Config *c1_bug47306 = create_config(
        "[ndbd]", "NoOfReplicas=1", "DataMemory=100M", "IndexMemory=100M",
        "[ndb_mgmd]", "HostName=localhost", "[mysqld]", NULL);
    CHECK(c1_bug47306);
    ...
    while ((name = prop_it.next())) {
      if (strcmp(name, "IndexMemory") == 0) found++;
      if (strcmp(name, "DataMemory") == 0) found++;
      count++;
    }
    CHECK(found == 2 && count == found + 2);  // Overhead == 2
    ndbout_c("==================");
...
static void print_restart_info(void) {
...
static void checksum_config(void) {
  Config *c1 = create_config("[ndbd]", "NoOfReplicas=1", "[ndb_mgmd]",
                             "HostName=localhost", "[mysqld]", NULL);
  CHECK(c1);
  Config *c2 =
...
static void test_config_v1_with_dyn_ports(void) {
  Config *c1 = create_config("[ndbd]", "[ndbd]", "[ndb_mgmd]",
                             "HostName=localhost", "[mysqld]", NULL);
  CHECK(c1);
...
static void test_param_values(void) {
  ...
  for (struct test *t = tests; t->param; t++) {
    ...
    {
      const Config *c =
          create_config("[ndbd]", "NoOfReplicas=1", t->param, "[ndb_mgmd]",
                        "HostName=localhost", "[mysqld]", NULL);
      if (t->result) {
        CHECK(c);
        delete c;
    ...
    {
      const Config *c = create_mycnf(
          "[cluster_config]", "ndb_mgmd=localhost", "ndbd=localhost,localhost",
          "ndbapi=localhost", "NoOfReplicas=1", t->param, NULL);
      if (t->result) {
        CHECK(c);
        delete c;
      } else {
...
static void test_hostname_mycnf(void) {
  ...
  // the two hostname specs must match
  {
    ...
    const Config *c = create_mycnf(
        "[cluster_config]", "ndb_mgmd=localhost", "ndbd=localhost,localhost",
        "ndbapi=localhost", "NoOfReplicas=1", "[cluster_config.ndbd.1]",
