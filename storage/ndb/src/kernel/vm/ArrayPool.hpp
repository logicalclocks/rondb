...
class ArrayPool {
  ...
  */
  ...
  class ErrorHandlerImpl : public ErrorHandler {
   public:
    ErrorHandlerImpl(CallBack *f, ArrayPool<T> &p) : func(f), pool(p) {}
   ...
   */
  bool setSize(Uint32 noOfElements, bool align = false,
               bool exit_on_error = true, bool guard = true,
               Uint32 paramId = 0);
  bool set(T *, Uint32 cnt, bool align = false);
  void clear() { theArray = 0; }

  inline Uint32 getNoOfFree() const { return noOfFree; }

  inline Uint32 getSize() const { return size; }

  inline Uint32 getUsed() const { return size - noOfFree; }

  inline Uint32 getUsedHi() const { return size - noOfFreeMin; }

  /**
    Set the low water mark equal to the current value, so that we can track
    the lowest value seen after this call.
   */
  void resetFreeMin() { noOfFreeMin = noOfFree; }

  inline void updateFreeMin(void) {
    if (noOfFree < noOfFreeMin) noOfFreeMin = noOfFree;
  }
  ...
  inline Uint32 getEntrySize() const { return sizeof(T); }
 ...
 public:
  T *getArrayPtr() { return theArray; }
  void setArrayPtr(T *newArray) { theArray = newArray; }
  Uint32 getSize() { return size; }
  void setNewSize(Uint32 newSize) { size = newSize; }
};

template <class T>
inline ArrayPool<T>::ArrayPool(const ErrorHandler *seizeErrorHandler)
...
template <class T>
inline ArrayPool<T>::~ArrayPool() {
  if (theArray != 0) {
    ndbd_free(alloc_ptr, size * sizeof(T));
    theArray = 0;
    alloc_ptr = 0;
#ifdef ARRAY_GUARD
    if (theAllocatedBitmask) delete[] theAllocatedBitmask;
    theAllocatedBitmask = 0;
#endif
  }
  delete seizeErrHand;
 ...
 */
template <class T>
inline bool ArrayPool<T>::setSize(Uint32 noOfElements, bool align,
                                  bool exit_on_error, bool guard,
                                  Uint32 paramId) {
  if (size == 0) {
    if (noOfElements == 0) {
      return true;
    ...
    if (theArray == 0) {
      ...
      ErrorReporter::handleAssert(errmsg, __FILE__, __LINE__,
                                  NDBD_EXIT_MEMALLOC);
...
template <class T>
inline bool ArrayPool<T>::set(T *ptr, Uint32 cnt, bool align) {
  if (size == 0) {
    alloc_ptr = ptr;
    if (align) {
      UintPtr p = (UintPtr)alloc_ptr;
  ...
  ErrorReporter::handleAssert("ArrayPool<T>::set called twice", __FILE__,
                              __LINE__);
  return false;  // not reached
}

template <class T>
inline void ArrayPool<T>::getPtr(Ptr<T> &ptr) const {
  Uint32 i = ptr.i;
  if (likely(i < size)) {
    ptr.p = &theArray[i];
#ifdef ARRAY_GUARD
    if (theAllocatedBitmask) {
      if (BitmaskImpl::get(bitmaskSz, theAllocatedBitmask, i)) return;
      /**
...
template <class T>
inline void ArrayPool<T>::getPtr(ConstPtr<T> &ptr) const {
  Uint32 i = ptr.i;
  if (likely(i < size)) {
    ptr.p = &theArray[i];
#ifdef ARRAY_GUARD
...
template <class T>
inline bool ArrayPool<T>::getPtr(Ptr<T> &ptr, Uint32 i) const {
  if (likely(i < size)) {
    ptr.i = i;
    ptr.p = &theArray[i];
#ifdef ARRAY_GUARD
    if (theAllocatedBitmask) {
      if (BitmaskImpl::get(bitmaskSz, theAllocatedBitmask, i)) return true;
      /**
       * Getting a non-seized element
       */
      return false;
...
template <class T>
inline T *ArrayPool<T>::getPtr(Uint32 i) const {
  if (likely(i < size)) {
#ifdef ARRAY_GUARD
    if (theAllocatedBitmask) {
...
*/
template <class T>
inline void ArrayPool<T>::getPtrIgnoreAlloc(Ptr<T> &ptr) const {
  Uint32 i = ptr.i;
  if (likely(i < size)) {
    ptr.p = &theArray[i];
  } else {
...
template <class T>
inline bool ArrayPool<T>::seize(Ptr<T> &ptr) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
...
template <class T>
inline bool ArrayPool<T>::seizeId(Ptr<T> &ptr, Uint32 i) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
  ...
  if (ff != RNIL) {
    ...
    if (theAllocatedBitmask) {
      if (!BitmaskImpl::get(bitmaskSz, theAllocatedBitmask, ff)) {
         ...
         * Seizing an already seized element
         */
        ErrorReporter::handleAssert("ArrayPool<T>::seizeId", __FILE__,
                                    __LINE__);
        return false;
      }
    }
...
template <class T>
inline bool ArrayPool<T>::findId(Uint32 i) const {
...
template <class T>
inline void ArrayPool<T>::releaseN(Uint32 base, Uint32 n) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
  ...
  for (Uint32 i = base; i < end; i++) {
    ...
    if (theAllocatedBitmask) {
      if (BitmaskImpl::get(bitmaskSz, theAllocatedBitmask, i)) {
         ...
         */
        ErrorReporter::handleAssert("ArrayPool<T>::release", __FILE__,
                                    __LINE__);
        return;
      }
    }
...
template <class T>
inline void ArrayPool<T>::releaseList(Uint32 n, Uint32 first, Uint32 last) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
    ...
    if (theAllocatedBitmask) {
      ...
      for (Uint32 i = 0; i < n; i++) {
        if (BitmaskImpl::get(bitmaskSz, theAllocatedBitmask, tmp)) {
           ...
           * Releasing an already released element
           */
          ErrorReporter::handleAssert("ArrayPool<T>::releaseList", __FILE__,
                                      __LINE__);
          return;
        }
 ...
 */
template <class T>
inline void ArrayPool<T>::release(Uint32 _i) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
 ...
 */
template <class T>
inline void ArrayPool<T>::release(Ptr<T> &ptr) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
...
template <class T>
inline void ArrayPool<T>::releaseLast(Ptr<T> &ptr) {
#ifdef ARRAY_GUARD
  assert(chunk == false);
#endif
  ...
  if (likely(i < size)) {
    ...
    if (theAllocatedBitmask) {
       ...
       * Releasing an already released element
       */
      ErrorReporter::handleAssert("ArrayPool<T>::releaseLast", __FILE__,
                                  __LINE__);
    }
#endif
    noOfFree++;
...
#define DUMP(a, b)                                                         \
  do {                                                                     \
    printf("%s c.m_first_free: %u c.m_free_cnt: %u %s", a, c.m_first_free, \
           c.m_free_cnt, b);                                               \
    fflush(stdout);                                                        \
  } while (0)
#else
...
class CachedArrayPool : public ArrayPool<T> {
 public:
  ...
  class ErrorHandlerImpl : public ArrayPool<T>::ErrorHandler {
   public:
    ErrorHandlerImpl(CallBack *f, CachedArrayPool<T> &p) : func(f), pool(p) {}
    ~ErrorHandlerImpl() override {}
  ...
  void releaseList(Uint32 n, Uint32 first, Uint32 last) {
    ArrayPool<T>::releaseList(n, first, last);
  }
...
template <class T>
inline void CachedArrayPool<T>::setChunkSize(Uint32 sz) {
...
#ifdef ARRAY_CHUNK_GUARD
template <class T>
inline void CachedArrayPool<T>::checkChunks() {
  T *const &theArray = this->theArray;
...
template <class T>
inline bool CachedArrayPool<T>::seizeChunk(Uint32 &cnt, Ptr<T> &ptr) {
    ...
    if (theAllocatedBitmask) {
      ...
      for (Uint32 i = 0; i < cnt; i++) {
        if (BitmaskImpl::get(bitmaskSz, theAllocatedBitmask, tmpI)) {
           ...
           * Seizing an already seized element
           */
          ErrorReporter::handleAssert("ArrayPool<T>::seizeChunk", __FILE__,
                                      __LINE__);
        } else {
...
template <class T>
inline void CachedArrayPool<T>::releaseChunk(Uint32 cnt, Uint32 first,
                                             Uint32 last) {
  Uint32 &firstFree = this->firstFree;
  T *const &theArray = this->theArray;
...
template <class T>
inline bool CachedArrayPool<T>::seize(LockFun l, Cache &c, Ptr<T> &p) {
  DUMP("seize", "-> ");
...
template <class T>
inline void CachedArrayPool<T>::release(LockFun l, Cache &c, Uint32 i) {
  Ptr<T> tmp;
...
template <class T>
inline void CachedArrayPool<T>::release(LockFun l, Cache &c, Ptr<T> &p) {
...
template <class T>
inline void CachedArrayPool<T>::releaseList(LockFun l, Cache &c, Uint32 n,
...
template <class T>
inline void CachedArrayPool<T>::releaseChunk(LockFun l, Cache &c, Uint32 n) {
  T *const &theArray = this->theArray;
  DUMP("releaseList", "-> ");
  Uint32 ff = c.m_first_free;
  Uint32 prev = ff;
...
template <class T>
inline SafeArrayPool<T>::SafeArrayPool() {
  m_mutex = 0;
  m_mutex_owner = false;
}

template <class T>
inline void SafeArrayPool<T>::setMutex(NdbMutex *mutex) {
  if (mutex != 0) {
    m_mutex = mutex;
    m_mutex_owner = false;
  } else {
...
template <class T>
inline SafeArrayPool<T>::~SafeArrayPool() {
  if (m_mutex_owner) NdbMutex_Destroy(m_mutex);
}

template <class T>
inline int SafeArrayPool<T>::lock() {
  return NdbMutex_Lock(m_mutex);
}

template <class T>
inline int SafeArrayPool<T>::unlock() {
  return NdbMutex_Unlock(m_mutex);
}

template <class T>
inline bool SafeArrayPool<T>::seize(Ptr<T> &ptr) {
  bool ok = false;
  if (lock() == 0) {
...
template <class T>
inline void SafeArrayPool<T>::release(Uint32 i) {
  int ret = lock();
  require(ret == 0);
...
template <class T>
inline void SafeArrayPool<T>::release(Ptr<T> &ptr) {
  int ret = lock();
  require(ret == 0);
