...
class ComposedSlotPool {
 public:
  typedef Slot Type;
  ComposedSlotPool();
  Uint32 getUncheckedPtrs(Uint32 *from, Ptr<Slot> ptrs[], Uint32 cnt,
  ...
  void init(Uint32 type_id, unsigned int slot_size, Uint32 *min_recs,
            Uint32 max_recs, const Pool_context &pool_ctx);
 ...
 private:
  ...
  Pool1 m_pool1;
#if !defined(COMPOSED_SLOT_POOL_NO_MAX_COUNT) || defined(VM_TRACE) || \
