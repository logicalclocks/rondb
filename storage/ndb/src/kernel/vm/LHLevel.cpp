...
int main(int argc, char *argv[]) {
  ...
  for (int lap = 1; lap <= 2; lap++) {
    ...
    if (lap == 1) {
      ...
      ok((n >= 0),
         "all element hash values match stored hash value and bucket address");
      if (n < 0) n = -n;
      ok((c_inserts == c_deletes + n),
         "scanned element count (%u) matches difference between inserts (%llu) "
         "and deletes (%llu)",
         n, c_inserts, c_deletes);
    }
  }
  ok((c_inserts == c_deletes), "inserts (%llu) equals deletes (%llu)",
     c_inserts, c_deletes);
  ok((c_expands == c_shrinks), "expands (%llu) equals shrinks (%llu)",
     c_expands, c_shrinks);
  delete[] arr;
  return exit_status();
}
...
bool delete_elem(LHLevel &lh, elem (*arr)[BUCKSIZE], Uint32 w) {
  ...
  if (found) {
    ...
    for (j = i + 1; j < BUCKSIZE; j++, i++) arr[addr][i] = arr[addr][j];
...
bool shrink(LHLevel &lh, elem (*arr)[BUCKSIZE]) {
  ...
  if (!lh.getMergeBuckets(from, to)) {
    int c = 0;
    for (int i = 0; i < BUCKSIZE && arr[from][i].head != 0; i++) c++;
  ...
  int i, j;
  int c = 0;
  for (i = 0; i < BUCKSIZE && arr[to][i].head != 0; i++) c++;
  for (j = 0; j < BUCKSIZE && arr[from][j].head != 0; j++) c++;
...
void expand(LHLevel &lh, elem (*arr)[BUCKSIZE]) {
  ...
  int i, j, k;
  for (i = j = k = 0; i < BUCKSIZE && (arr[from][i].head != 0); i++) {
    ...
    if (lh.shouldMoveBeforeExpand(hash)) {
      ...
      if (k < i) arr[from][k] = arr[from][i];
      k++;
    }
  }
  for (; j < BUCKSIZE; j++) std::memset(&arr[to][j], 0, sizeof(arr[to][j]));
  for (; k < BUCKSIZE; k++) std::memset(&arr[from][k], 0, sizeof(arr[from][k]));
  lh.expand();
  c_expands++;
}
...
int count_elem(LHLevel &lh, elem (*arr)[BUCKSIZE]) {
  ...
  int failures = 0;
  if (!lh.isEmpty())
    for (Uint32 addr = 0; addr <= lh.getTop(); addr++) {
