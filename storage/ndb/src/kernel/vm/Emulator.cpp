 ...
 * check that you get the same line count in both cases.
 *
 * find storage/ndb  -name '*.?pp' -exec egrep '^ *# *define *JAM_FILE_ID' {}
 * \;|sed -e 's/ \+//g'|sort|uniq|wc -l find storage/ndb  -name '*.?pp' -exec
 * egrep '^ *# *define *JAM_FILE_ID' {} \;|sed -e 's/ \+//g'|wc -l
 *
 * TROUBLESHOOTING:
 *
 * Symptom: Compiler error stating that JAM_FILE_ID is undefined.
...
bool JamEvent::verifyId(Uint32 fileId, const char *pathName) {
  if (fileId >= sizeof jamFileNames / sizeof jamFileNames[0]) {
    ...
    /*
      ...
      names.
    */
    return pathLen >= baseLen && native_strcasecmp(pathName + pathLen - baseLen,
                                                   jamFileNames[fileId]) == 0;
  }
}
