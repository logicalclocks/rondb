...
Uint32 Tup_fixsize_page::alloc_record(Uint32 page_idx) {
  loc_assert(page_idx + 1 < DATA_WORDS);
...
Uint32 Tup_fixsize_page::free_record(Uint32 page_idx) {
...
Uint32 Tup_varsize_page::alloc_record(Uint32 page_idx, Uint32 alloc_size,
...
Uint32 Tup_varsize_page::alloc_record(Uint32 alloc_size, Tup_varsize_page *temp,
                                      Uint32 chain) {
...
Uint32 Tup_varsize_page::free_record(Uint32 page_idx, Uint32 chain) {
  // g_eventLogger->info("%p->free_record(%d%s)", this, page_idx, (chain ? "
  // CHAIN": ""));
...
void Tup_varsize_page::rebuild_index(Uint32 *index_ptr) {
  Uint32 empty = 1;
...
void Tup_varsize_page::reorg(Tup_varsize_page *copy_page) {
  ...
  for (; index_ptr < end_of_page; index_ptr++) {
    ...
    if (!(index_word & FREE) && entry_len) {
      ...
      *index_ptr = (new_insert_pos << POS_SHIFT) + (index_word & ~POS_MASK);
      memcpy(m_data + new_insert_pos, copy_page->m_data + entry_pos,
             4 * entry_len);
  ...
  for(Uint32 i = 1; i<page.high_index; i++, index_ptr--)
  {
    ...
    if (!(*index_ptr & page.FREE))
      out << " pos: " << ((*index_ptr & page.POS_MASK) >> page.POS_SHIFT)
          ...
          << ((*index_ptr & page.CHAIN) ? " CHAIN " : " ") << "]" << flush;
    else
      out << " FREE ]" << flush;
  }
...
NdbOut &operator<<(NdbOut &out, const Tup_fixsize_page &page) {
  out << "[ Fixpage " << &page << ": frag_page: " << page.frag_page_id
      << " page_no: " << page.m_page_no << " file_no: " << page.m_file_no
      << " table: " << page.m_table_id << " fragment: " << page.m_fragment_id
