...
struct Tup_page 
{
  ...
  static constexpr Uint32 DATA_WORDS =
      File_formats::NDB_PAGE_SIZE_WORDS - HEADER_WORDS;
  ...
  void set_page_to_skip_lcp() { m_flags |= LCP_SKIP_FLAG; }
  void clear_page_to_skip_lcp() { m_flags &= (~LCP_SKIP_FLAG); }
};
...
struct Tup_fixsize_page {
  ...
  void set_change_maps(Uint32 page_index) {
     ...
     * 4 kB.
     */
    Uint32 large_map_idx =
        Tup_fixsize_page::FIRST_BIT_CHANGE_MAP + (page_index >> 10);
  ...
  void clear_max_gci() { m_gci = 0; }
  Uint32 get_max_gci() { return m_gci; }
  void set_max_gci(Uint32 gci) {
    if (gci > m_gci) m_gci = gci;
  }
...
struct Tup_varsize_page {
  ...
  void grow_entry(Uint32 page_index, Uint32 growth_len) {
    ...
    Uint32 idx = *pos;
    assert(!(idx & FREE));
    assert((((idx & POS_MASK) >> POS_SHIFT) +
            ((idx & LEN_MASK) >> LEN_SHIFT)) == insert_pos);
...
#endif
    free_space += shrink;
    if (insert_pos == (old_pos + old_size)) insert_pos -= shrink;
  }
  ...
  bool is_empty() const { return high_index == 1; }
};
