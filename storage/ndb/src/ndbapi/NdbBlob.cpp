...
NdbBlob::State NdbBlob::getState() { return theState; }

void NdbBlob::getVersion(int &version) { version = theEventBlobVersion; }

// set state (inline)

inline void NdbBlob::setState(State newState) {
...
int NdbBlob::getBlobTableName(char *btname, Ndb *anNdb, const char *tableName,
                              const char *columnName) {
  DBUG_ENTER("NdbBlob::getBlobTableName");
  NdbTableImpl *t = anNdb->theDictionary->m_impl.getTable(tableName);
  if (t == nullptr) DBUG_RETURN(-1);
  NdbColumnImpl *c = t->getColumn(columnName);
  if (c == nullptr) DBUG_RETURN(-1);
  getBlobTableName(btname, t, c);
  DBUG_RETURN(0);
}

void NdbBlob::getBlobTableName(char *btname, const NdbTableImpl *t,
                               const NdbColumnImpl *c) {
  DBUG_ENTER("NdbBlob::getBlobTableName");
  assert(t != nullptr && c != nullptr && c->getBlobType() &&
         c->getPartSize() != 0);
...
int NdbBlob::getBlobTable(NdbTableImpl &bt, const NdbTableImpl *t,
                          const NdbColumnImpl *c, NdbError &error) {
  ...
  */
  ...
  DBUG_PRINT("info",
             ("Define BLOB table V%d with"
              ...
              blobVersion, bt.m_primaryTableId, (uint)bt.getFragmentType()));
...
int NdbBlob::getBlobEventName(char *bename, Ndb *anNdb, const char *eventName,
                              const char *columnName) {
...
void NdbBlob::getBlobEventName(char *bename, const NdbEventImpl *e,
                               const NdbColumnImpl *c) {
  // XXX events should have object id
  BaseString::snprintf(bename, MAX_TAB_NAME_SIZE, "NDB$BLOBEVENT_%s_%d",
                       e->m_name.c_str(), (int)c->m_column_no);
}

void NdbBlob::getBlobEvent(NdbEventImpl &be, const NdbEventImpl *e,
                           const NdbColumnImpl *c) {
...
NdbBlob::Buf::Buf() : data(nullptr), size(0), maxsize(0) {}

NdbBlob::Buf::~Buf() { delete[] data; }
...
void NdbBlob::Buf::copyfrom(const NdbBlob::Buf &src) {
  size = src.size;
...
inline bool NdbBlob::isTableOp() { return theTable == theAccessTable; }

inline bool NdbBlob::isIndexOp() { return theTable != theAccessTable; }

inline bool NdbBlob::isKeyOp() {
...
inline bool NdbBlob::isReadOp() {
  return theNdbOp->theOperationType == NdbOperation::ReadRequest ||
         theNdbOp->theOperationType == NdbOperation::ReadExclusive;
}

inline bool NdbBlob::isInsertOp() {
  return theNdbOp->theOperationType == NdbOperation::InsertRequest;
}

inline bool NdbBlob::isUpdateOp() {
  return theNdbOp->theOperationType == NdbOperation::UpdateRequest;
}

inline bool NdbBlob::isWriteOp() {
  return theNdbOp->theOperationType == NdbOperation::WriteRequest;
}

inline bool NdbBlob::isDeleteOp() {
  return theNdbOp->theOperationType == NdbOperation::DeleteRequest;
}

inline bool NdbBlob::isScanOp() {
  return theNdbOp->theOperationType == NdbOperation::OpenScanRequest ||
...
inline bool NdbBlob::isTakeOverOp() {
  return TcKeyReq::getTakeOverScanFlag(theNdbOp->theScanInfo);
}

// computations (inline)

inline Uint32 NdbBlob::getPartNumber(Uint64 pos) {
...
inline Uint32 NdbBlob::getPartOffset(Uint64 pos) {
  assert(thePartSize != 0 && pos >= theInlineSize);
  return (pos - theInlineSize) % thePartSize;
}

inline Uint32 NdbBlob::getPartCount() {
  if (theLength <= theInlineSize) return 0;
  return 1 + getPartNumber(theLength - 1);
}

inline Uint32 NdbBlob::getDistKey(Uint32 part) {
...
inline void NdbBlob::setHeadPartitionId(NdbOperation *anOp) {
   ...
   */
  if (userDefinedPartitioning && (thePartitionId != noPartitionId())) {
    anOp->setPartitionId(thePartitionId);
  }
}

inline void NdbBlob::setPartPartitionId(NdbOperation *anOp) {
  /* For UserDefined partitioned tables
...
int NdbBlob::packKeyValue(const NdbTableImpl *aTable, const Buf &srcBuf) {
...
int NdbBlob::unpackKeyValue(const NdbTableImpl *aTable, Buf &dstBuf) {
...
int NdbBlob::copyKeyFromRow(const NdbRecord *record, const char *row,
  ...
  for (Uint32 i = 0; i < record->key_index_length; i++) {
    ...
    if (packed_pad > 0) std::memset(packed + len, 0, packed_pad);
    if (unpacked_pad > 0) std::memset(unpacked + len, 0, unpacked_pad);
    packed += packed_len;
...
void NdbBlob::packBlobHead(const Head &head, char *buf, int blobVersion) {
  ...
  if (unlikely(blobVersion == NDB_BLOB_V1)) {
    ...
    // all little-endian
    uint i, n;
    for (i = 0, n = 0; i < 2; i++, n += 8) *p++ = (head.varsize >> n) & 0xff;
    for (i = 0, n = 0; i < 2; i++, n += 8) *p++ = (head.reserved >> n) & 0xff;
    for (i = 0, n = 0; i < 4; i++, n += 8) *p++ = (head.pkid >> n) & 0xff;
    for (i = 0, n = 0; i < 8; i++, n += 8) *p++ = (head.length >> n) & 0xff;
    assert(p - (uchar *)buf == 16);
    assert(head.reserved == 0);
    DBUG_DUMP("info", (uchar *)buf, 16);
  }
  DBUG_PRINT("info", ("pack: varsize=%u length=%u pkid=%u", (uint)head.varsize,
                      (uint)head.length, (uint)head.pkid));
  DBUG_VOID_RETURN;
}

void NdbBlob::unpackBlobHead(Head &head, const char *buf, int blobVersion) {
  ...
  if (unlikely(blobVersion == NDB_BLOB_V1)) {
    ...
    // all little-endian
    uint i, n;
    for (i = 0, n = 0; i < 2; i++, n += 8) head.varsize |= ((Uint16)*p++ << n);
    for (i = 0, n = 0; i < 2; i++, n += 8) head.reserved |= ((Uint32)*p++ << n);
    for (i = 0, n = 0; i < 4; i++, n += 8) head.pkid |= ((Uint32)*p++ << n);
    for (i = 0, n = 0; i < 8; i++, n += 8) head.length |= ((Uint64)*p++ << n);
 ...
 *  -1 - on error
 */
int NdbBlob::getBlobKeysEqual(NdbBlob *other) {
  ...
  for (unsigned i = 0; n < numKeyCols; i++) {
    ...
    if (colImpl->m_pk) {
      ...
      Uint32 lbA, lenA;
      bool ok;
      ok = NdbSqlUtil::get_var_length(colImpl->m_type, dataPtrA, maxLen, lbA,
                                      lenA);
      assert(ok);
      if (unlikely(!ok)) {
      ...
      Uint32 lbB, lenB;
      ok = NdbSqlUtil::get_var_length(colImpl->m_type, dataPtrB, maxLen, lbB,
                                      lenB);
      assert(ok);
      if (unlikely(!ok)) {
        DBUG_PRINT("error", ("Corrupt length of key column %u", i));
      ...
      assert(lbA == lbB);
      CHARSET_INFO *cs = colImpl->m_cs ? colImpl->m_cs : &my_charset_bin;
      int res = (cs->coll->strnncollsp)(cs, dataPtrA + lbA, lenA,
                                        dataPtrB + lbB, lenB);
      if (res != 0) {
...
void NdbBlob::setBlobHashNext(NdbBlob *next) { m_keyHashNext = next; }

NdbBlob *NdbBlob::getBlobHashNext() const { return m_keyHashNext; }

// in V2 operation can also be on blob part
int NdbBlob::setTableKeyValue(NdbOperation *anOp) {
...
int NdbBlob::setAccessKeyValue(NdbOperation *anOp) {
...
int NdbBlob::setDistKeyValue(NdbOperation *anOp, Uint32 part) {
  DBUG_ENTER("NdbBlob::setDistKeyValue");
  if (theStripeSize != 0) {
    Uint32 dist = getDistKey(part);
    DBUG_PRINT("info", ("dist=%u", dist));
    if (anOp->equal(theBtColumnNo[BtColumnDist], dist) == -1) DBUG_RETURN(-1);
  }
  DBUG_RETURN(0);
}

int NdbBlob::setPartKeyValue(NdbOperation *anOp, Uint32 part) {
  ...
  if (unlikely(theBlobVersion == NDB_BLOB_V1)) {
    ...
    if (setTableKeyValue(anOp) == -1 || setDistKeyValue(anOp, part) == -1 ||
...
int NdbBlob::setPartPkidValue(NdbOperation *anOp, Uint32 pkid) {
...
int NdbBlob::getPartDataValue(NdbOperation *anOp, char *buf, Uint16 *aLenLoc) {
...
int NdbBlob::setPartDataValue(NdbOperation *anOp, const char *buf,
                              const Uint16 &aLen) {
...
int NdbBlob::getHeadInlineValue(NdbOperation *anOp) {
  ...
  if (userDefinedPartitioning) {
     ...
     * it's as expected.
     */
    thePartitionIdRecAttr = anOp->getValue_impl(
        &NdbColumnImpl::getImpl(*NdbDictionary::Column::FRAGMENT));
...
void NdbBlob::getHeadFromRecAttr() {
  ...
  DBUG_PRINT("info", ("theNullFlag=%d theLength=%llu", theNullFlag, theLength));
  DBUG_VOID_RETURN;
}

void NdbBlob::prepareSetHeadInlineValue() {
  theHead.length = theLength;
  if (unlikely(theBlobVersion == NDB_BLOB_V1)) {
    if (theLength < theInlineSize)
      std::memset(theInlineData + theLength, 0,
...
int NdbBlob::setHeadInlineValue(NdbOperation *anOp) {
...
int NdbBlob::getValue(void *data, Uint32 bytes) {
...
int NdbBlob::setValue(const void *data, Uint32 bytes) {
  ...
  if (isInsertOp() || isWriteOp()) {
    ...
    if (theSetBuf != nullptr) {
      ...
      if (n > theInlineSize) n = theInlineSize;
      assert(thePos == 0);
      if (writeDataPrivate(theSetBuf, n) == -1) DBUG_RETURN(-1);
     ...
     */
    if (!theNdbRecordFlag) {
      if (setHeadInlineValue(theNdbOp) == -1) DBUG_RETURN(-1);
    }
  }
  DBUG_RETURN(0);
}
...
int NdbBlob::setActiveHook(ActiveHook activeHook, void *arg) {
...
int NdbBlob::getDefined(int &isNull)  // deprecated
{
...
int NdbBlob::getNull(bool &isNull)  // deprecated
{
...
int NdbBlob::setNull() {
  ...
  if (theNullFlag) DBUG_RETURN(0);
  if (deletePartsThrottled(0, getPartCount()) == -1) DBUG_RETURN(-1);
  theNullFlag = true;
  theLength = 0;
...
int NdbBlob::getLength(Uint64 &len) {
...
int NdbBlob::truncate(Uint64 length) {
  ...
  if (theLength > length) {
    if (length > theInlineSize) {
      ...
      if (off != 0) {
         ...
         */
        if (executePendingBlobWrites() == -1) DBUG_RETURN(-1);
        Uint16 len = 0;
        if (readPart(thePartBuf.data, part1, len) == -1) DBUG_RETURN(-1);
        if (executePendingBlobReads() == -1) DBUG_RETURN(-1);
        assert(len != 0);
        DBUG_PRINT("info",
                   ("part %u length old=%u new=%u", part1, (Uint32)len, off));
        if (theFixedDataFlag)
          std::memset(thePartBuf.data + off, theFillChar, thePartSize - off);
        if (updatePart(thePartBuf.data, part1, off) == -1) DBUG_RETURN(-1);
      }
    } else {
      if (deletePartsThrottled(0, getPartCount()) == -1) DBUG_RETURN(-1);
    }
    theLength = length;
    theHeadInlineUpdateFlag = true;
    if (thePos > length) thePos = length;
  }
  DBUG_RETURN(0);
}
...
int NdbBlob::readData(void *data, Uint32 &bytes) {
...
int NdbBlob::readDataPrivate(char *buf, Uint32 &bytes) {
  DBUG_ENTER("NdbBlob::readDataPrivate");
  DBUG_PRINT("info", ("bytes=%u thePos=%u theLength=%u", bytes, (Uint32)thePos,
                      (Uint32)theLength));
  assert(thePos <= theLength);
  Uint64 pos = thePos;
  if (bytes > theLength - pos) bytes = Uint32(theLength - pos);
  Uint32 len = bytes;
  if (len > 0) {
    // inline part
  ...
  if (len > 0) {
    ...
    if (off != 0) {
      ...
      Uint16 sz = 0;
      if (readPart(thePartBuf.data, part, sz) == -1) DBUG_RETURN(-1);
    ...
    if (len >= thePartSize) {
      ...
      do {
        ...
        if (theEventBlobVersion == -1) {
          ...
          const Uint32 remainingQuota = theNdbCon->maxPendingBlobReadBytes -
                                        MIN(theNdbCon->maxPendingBlobReadBytes,
                                            theNdbCon->pendingBlobReadBytes);
          const Uint32 maxPartsThisTrip =
              MAX(remainingQuota / thePartSize, 1);  // always read one part
          partsThisTrip = MIN(count, maxPartsThisTrip);
        }

        if (readParts(buf, part, partsThisTrip) == -1) DBUG_RETURN(-1);
...
int NdbBlob::writeData(const void *data, Uint32 bytes) {
...
int NdbBlob::writeDataPrivate(const char *buf, Uint32 bytes) {
  ...
  if (len > 0) {
    ...
    if (off != 0) {
      ...
      if (updatePart(thePartBuf.data, part, newsz) == -1) DBUG_RETURN(-1);
      pos += n;
      buf += n;
      len -= n;
    }
    ...
    if (len >= thePartSize) {
      ...
      for (unsigned i = 0; i < count; i++) {
        if (part + i < getPartCount()) {
          if (updateParts(buf, part + i, 1) == -1) DBUG_RETURN(-1);
        } else {
          if (insertParts(buf, part + i, 1) == -1) DBUG_RETURN(-1);
        }
  ...
  if (len > 0) {
    ...
    if (theLength > pos + len) {
      ...
      // no length change
      if (updatePart(thePartBuf.data, part, sz) == -1) DBUG_RETURN(-1);
    } else {
      memcpy(thePartBuf.data, buf, len);
      if (theFixedDataFlag) {
      ...
      if (part < getPartCount()) {
        ...
        if (insertPart(thePartBuf.data, part, sz) == -1) DBUG_RETURN(-1);
      }
    }
    Uint32 n = len;
    pos += n;
...
int NdbBlob::readParts(char *buf, Uint32 part, Uint32 count) {
  DBUG_ENTER("NdbBlob::readParts");
  DBUG_PRINT("info", ("part=%u count=%u", part, count));
  if (theEventBlobVersion == -1) {
    if (readTableParts(buf, part, count) == -1) DBUG_RETURN(-1);
  } else {
    if (readEventParts(buf, part, count) == -1) DBUG_RETURN(-1);
  }
  DBUG_RETURN(0);
}

int NdbBlob::readPart(char *buf, Uint32 part, Uint16 &len) {
  DBUG_ENTER("NdbBlob::readPart");
  DBUG_PRINT("info", ("part=%u", part));
  if (theEventBlobVersion == -1) {
    if (readTablePart(buf, part, len) == -1) DBUG_RETURN(-1);
  } else {
    if (readEventPart(buf, part, len) == -1) DBUG_RETURN(-1);
  }
  DBUG_PRINT("info", ("part=%u len=%u", part, (Uint32)len));
  DBUG_RETURN(0);
}

int NdbBlob::readTableParts(char *buf, Uint32 part, Uint32 count) {
...
int NdbBlob::readTablePart(char *buf, Uint32 part, Uint16 &len) {
...
int NdbBlob::readEventParts(char *buf, Uint32 part, Uint32 count) {
  DBUG_ENTER("NdbBlob::readEventParts");
  // length not asked for - event code checks each part is full
  if (theEventOp->readBlobParts(buf, this, part, count, (Uint16 *)nullptr) ==
      -1) {
...
int NdbBlob::readEventPart(char *buf, Uint32 part, Uint16 &len) {
...
int NdbBlob::insertParts(const char *buf, Uint32 part, Uint32 count) {
...
int NdbBlob::insertPart(const char *buf, Uint32 part, const Uint16 &len) {
...
int NdbBlob::updateParts(const char *buf, Uint32 part, Uint32 count) {
...
int NdbBlob::updatePart(const char *buf, Uint32 part, const Uint16 &len) {
...
int NdbBlob::deletePartsThrottled(Uint32 part, Uint32 count) {
  DBUG_ENTER("NdbBlob::deletePartsThrottled");
  DBUG_PRINT("info", ("part=%u count=%u maxPendingBlobWriteBytes=%u", part,
                      count, theNdbCon->maxPendingBlobWriteBytes));

  if (thePartSize) {
    do {
      /* How much quota left, avoiding underflow? */
      const Uint32 remainingQuota = theNdbCon->maxPendingBlobWriteBytes -
...
int NdbBlob::deleteParts(Uint32 part, Uint32 count) {
 ...
 */
int NdbBlob::deletePartsUnknown(Uint32 part) {
  ...
  while (true) {
    ...
    bat *= 4;
    if (bat > maxbat) bat = maxbat;
  }
}

int NdbBlob::writePart(const char *buf, Uint32 part, const Uint16 &len) {
...
int NdbBlob::executePendingBlobReads() {
...
int NdbBlob::executePendingBlobWrites() {
 ...
 */
int NdbBlob::atPrepare(NdbTransaction *aCon, NdbOperation *anOp,
                       const NdbColumnImpl *aColumn) {
  DBUG_ENTER("NdbBlob::atPrepare");
  DBUG_PRINT("info", ("this=%p op=%p con=%p version=%d fixed data=%d", this,
                      theNdbOp, theNdbCon, theBlobVersion, theFixedDataFlag));
  if (atPrepareCommon(aCon, anOp, aColumn) == -1) DBUG_RETURN(-1);
  ...
  if (isKeyOp()) {
    if (isTableOp()) {
      ...
      thePackKeyBuf.zerorest();
      if (unpackKeyValue(theTable, theKeyBuf) == -1) DBUG_RETURN(-1);
    }
    if (isIndexOp()) {
      // get index key
      Uint32 *data = (Uint32 *)thePackKeyBuf.data;
 ...
 */
int NdbBlob::atPrepareCommon(NdbTransaction *aCon, NdbOperation *anOp,
                             const NdbColumnImpl *aColumn) {
  ...
  if (isKeyOp()) {
    if (isReadOp()) {
      ...
      if (getHeadInlineValue(theNdbOp) == -1) return -1;
    }
    if (isInsertOp()) {
  ...
  if (isScanOp()) {
    ...
    // add read of head+inline in this op
    if (getHeadInlineValue(sop) == -1) return -1;
  }
  setState(Prepared);
  return 0;
}
...
int NdbBlob::atPrepareNdbRecord(NdbTransaction *aCon, NdbOperation *anOp,
                                const NdbColumnImpl *aColumn,
...
int NdbBlob::atPrepareNdbRecordScan(NdbTransaction *aCon, NdbOperation *anOp,
                                    const NdbColumnImpl *aColumn) {
...
int NdbBlob::atPrepare(NdbEventOperationImpl *anOp,
  ...
  theColumn = aColumn;
  // prepare blob column and table
  if (prepareColumn() == -1) DBUG_RETURN(-1);
  DBUG_PRINT("info", ("this=%p main op=%p blob op=%p version=%d fixed data=%d",
                      this, anOp, aBlobOp, theBlobVersion, theFixedDataFlag));
  // tinyblob sanity
  ...
  theHeadInlineRecAttr =
      theEventOp->getValue(aColumn, theHeadInlineBuf.data, version);
  if (theHeadInlineRecAttr == nullptr) {
    setErrorCode(theEventOp);
    DBUG_RETURN(-1);
  }
  ...
  if (theBlobEventOp != nullptr) {
    ...
    if (unlikely(theBlobVersion == NDB_BLOB_V1)) {
      ...
      if (unlikely(theBlobEventPkRecAttr == nullptr ||
                   theBlobEventDistRecAttr == nullptr ||
                   theBlobEventPartRecAttr == nullptr ||
                   theBlobEventDataRecAttr == nullptr)) {
      ...
      buf = theBlobEventDataBuf.data;
      theBlobEventDataRecAttr = theBlobEventOp->getValue(bc, buf, version);
      if (unlikely((theStripeSize != 0 && theBlobEventDistRecAttr == nullptr) ||
                   theBlobEventPartRecAttr == nullptr ||
                   theBlobEventPkidRecAttr == nullptr ||
                   theBlobEventDataRecAttr == nullptr)) {
        setErrorCode(theBlobEventOp);
...
int NdbBlob::prepareColumn() {
  ...
  if (thePartSize > 0) {
    ...
    if ((bt = theColumn->m_blobTable) == nullptr ||
        ...
        bc->getType() != partType || bc->getLength() != (int)thePartSize) {
  ...
  thePackKeyBuf.alloc(
      MAX(theTable->m_keyLenInWords, theAccessTable->m_keyLenInWords) << 2);
 ...
 */
NdbBlob::BlobAction NdbBlob::preExecute(NdbTransaction::ExecType anExecType) {
  ...
  DBUG_PRINT("info", ("optype=%d theGetSetBytes=%d theSetFlag=%d",
                      theNdbOp->theOperationType, theGetSetBytes, theSetFlag));
  if (theState == Invalid) DBUG_RETURN(BA_ERROR);
  assert(theState == Prepared);
  // handle different operation types
  assert(isKeyOp());
   ...
   * before proceeding
   */
  if (!theColumn->m_nullable && (isInsertOp() || isWriteOp()) && !theSetFlag) {
  ...
  if (isInsertOp() && theSetFlag) {
    ...
    DBUG_PRINT(
        "info",
        ("theSetValueInPreExecFlag %u : (Ao %u theGetSetBytes %u "
         "theInlineSize %u maxPending %u pending %u",
         theSetValueInPreExecFlag, theNdbOp->m_abortOption, theGetSetBytes,
         theInlineSize, theNdbCon->maxPendingBlobWriteBytes,
         theNdbCon->pendingBlobWriteBytes));

    if (theSetValueInPreExecFlag) {
      DBUG_PRINT("info", ("Insert extra ops added in preExecute"));
      rc = handleBlobTask(anExecType);

      DBUG_PRINT("info", ("handleBlobTask returned %u", rc));
      if (rc == BA_ERROR) {
        DBUG_RETURN(BA_ERROR);
      }
    } else {
      DBUG_PRINT("info", ("Insert waiting for Blob head insert"));
  ...
  if (isTableOp()) {
    if (isUpdateOp() || isWriteOp() || isDeleteOp()) {
       ...
       */
      NdbOperation::LockMode lockMode = !isTakeOverOp()
                                            ? NdbOperation::LM_Read
                                            : NdbOperation::LM_CommittedRead;
      if (tOp == nullptr || tOp->readTuple(lockMode) == -1 ||
          setTableKeyValue(tOp) == -1 || getHeadInlineValue(tOp) == -1) {
        setErrorCode(tOp);
        DBUG_RETURN(BA_ERROR);
      }
  ...
  if (isIndexOp()) {
    ...
    if (this == tFirstBlob) {
      ...
      if (g_ndb_blob_ok_to_read_index_table) {
        ...
        NdbOperation *tOp =
            theNdbCon->getNdbOperation(theAccessTable, theNdbOp);
        if (tOp == nullptr || tOp->readTuple() == -1 ||
        ...
        NdbIndexOperation *tOp = theNdbCon->getNdbIndexOperation(
            theAccessTable->m_index, theTable, theNdbOp);
        if (tOp == nullptr || tOp->readTuple() == -1 ||
            setAccessKeyValue(tOp) == -1 || getTableKeyValue(tOp) == -1) {
          setErrorCode(tOp);
          DBUG_RETURN(BA_ERROR);
        }
        if (userDefinedPartitioning && isWriteOp()) {
           ...
           * here.
           */
          thePartitionIdRecAttr = tOp->getValue_impl(
              &NdbColumnImpl::getImpl(*NdbDictionary::Column::FRAGMENT));
    ...
    if (isUpdateOp() || isDeleteOp()) {
      ...
      NdbIndexOperation *tOp = theNdbCon->getNdbIndexOperation(
          theAccessTable->m_index, theTable, theNdbOp);
      if (tOp == nullptr || tOp->readTuple() == -1 ||
          setAccessKeyValue(tOp) == -1 || getHeadInlineValue(tOp) == -1) {
        setErrorCode(tOp);
        DBUG_RETURN(BA_ERROR);
      }
      theHeadInlineReadOp = tOp;
 ...
 *
 */
int NdbBlob::initBlobTask(NdbTransaction::ExecType anExecType
                          [[maybe_unused]]) {
  ...
  if (isReadOp()) {
    ...
    DBUG_PRINT(
        "info",
        ("Write op, read result = %u",
         (theHeadInlineReadOp ? theHeadInlineReadOp->theError.code : 999)));
    assert(anExecType == NdbTransaction::NoCommit);
 ...
 *
 */
NdbBlob::BlobAction NdbBlob::handleBlobTask(
    NdbTransaction::ExecType anExecType) {
  ...
  while (!done && !executeDefinedOpsNow) {
    DBUG_PRINT("info", ("Blob op state : %u rbuff %p rbufflen %llu wbuff %p "
                        ...
                        m_blobOp.m_state, m_blobOp.m_readBuffer,
                        m_blobOp.m_readBufferLen, m_blobOp.m_writeBuffer,
                        m_blobOp.m_writeBufferLen, m_blobOp.m_oldLen,
                        m_blobOp.m_position));
    ...
    switch (m_blobOp.m_state) {
      ...
      case BlobTask::BTS_READ_HEAD: {
        ...
        const Uint32 inlineBytesToRead =
            MIN(theInlineSize, m_blobOp.m_readBufferLen);
        DBUG_PRINT("info",
                   ("Copying %u bytes from inline header", inlineBytesToRead));
      ...
      case BlobTask::BTS_READ_PARTS: {
        ...
        const bool lastPart = (m_blobOp.m_readBufferLen <= nextPartStart);
        const bool partialReadLast = (m_blobOp.m_readBufferLen < nextPartStart);
        /* We don't check the actual length of the last part read */
        m_blobOp.m_lastPartLen = thePartSize;
        if (partialReadLast) {
          DBUG_PRINT("info",
                     ("Partial read last part %llu to partBuf", partIdx));
          if (readPart(thePartBuf.data, partIdx, m_blobOp.m_lastPartLen) ==
              -1) {
            DBUG_RETURN(BA_ERROR);
          }
        } else {
          DBUG_PRINT("info", ("Non partial read part %llu to offset %llu",
                              partIdx, m_blobOp.m_position));
          if (readPart(m_blobOp.m_readBuffer + m_blobOp.m_position, partIdx,
        ...
        if (lastPart) {
          ...
          assert(m_blobOp.m_position >= m_blobOp.m_readBufferLen);
      ...
      case BlobTask::BTS_READ_LAST_PART: {
        ...
        const Uint32 lastPartBytes =
            (m_blobOp.m_readBufferLen - theInlineSize) % thePartSize;
        const Uint32 lastPartBuffOffset = m_blobOp.m_position - thePartSize;
        DBUG_PRINT("info", ("Copying out %u bytes to offset %u", lastPartBytes,
                            lastPartBuffOffset));

        memcpy(m_blobOp.m_readBuffer + lastPartBuffOffset, thePartBuf.data,
               lastPartBytes);
      ...
      case BlobTask::BTS_WRITE_HEAD: {
        ...
        /* U/W */
        const bool setNull =
            (m_blobOp.m_writeBuffer == nullptr && !isDeleteOp());
        ...
        if (setNull) {
          ...
          updateHead = true;
          DBUG_PRINT("info",
                     ("Wrote %u of %u bytes to header", n, theInlineSize));
      ...
      case BlobTask::BTS_WRITE_PARTS: {
        ...
        if (m_blobOp.m_oldLen > theInlineSize) {
          numOldParts =
              ((m_blobOp.m_oldLen - theInlineSize) + thePartSize - 1) /
              thePartSize;
          if (oldLenUnknown) {
        ...
        if (theGetSetBytes > theInlineSize) {
          numNewParts = ((theGetSetBytes - theInlineSize) + thePartSize - 1) /
                        thePartSize;
          lastPartLen = (theGetSetBytes - theInlineSize) -
                        ((numNewParts - 1) * thePartSize);
        }
        /* Are we done? */
        if (partIdx >= numOldParts && partIdx >= numNewParts) {
          /* DONE */
          m_blobOp.m_state = BlobTask::BTS_DONE;
          break;
        }
        ...
        if (partIdx < numNewParts) {
          /* Overwrite */
          const char *src =
              m_blobOp.m_writeBuffer + theInlineSize + (partIdx * thePartSize);
          ...
          if (partIdx == (numNewParts - 1)) {
            if (!theFixedDataFlag) {
              ...
              memset(thePartBuf.data + lastPartLen, theFillChar,
                     thePartSize - lastPartLen);
              src = thePartBuf.data;
            }
          }

          if (writePart(src, partIdx, partLen) != 0) {
            DBUG_RETURN(BA_ERROR);
          }
        } else {
          ...
          if (!oldLenUnknown) {
            ...
            if (numPartsDeleted != 0) {
              ...
              DBUG_PRINT(
                  "info",
                  ("partIdx = %llu, oldLenUnknown, last delete op result %u",
                   partIdx, rc));
              if (rc != 0) {
                if (rc == 626) {
                  ...
                  m_blobOp.m_oldLen =
                      (theInlineSize + ((partIdx - 1) * thePartSize));
                  oldLenUnknown = false;
                } else {
            ...
            if (oldLenUnknown) {
              ...
              if (numPartsDeleted == 0 || numPartsDeleted == 5 ||
                  numPartsDeleted == 21 || numPartsDeleted == 85 ||
                  numPartsDeleted == 256) {
        ...
        DBUG_PRINT("info", ("Position moves to %llu", m_blobOp.m_position));
 ...
 *    necessary
 */
NdbBlob::BlobAction NdbBlob::postExecute(NdbTransaction::ExecType anExecType) {
  DBUG_ENTER("NdbBlob::postExecute");
  DBUG_PRINT("info", ("this=%p op=%p con=%p anExecType=%u", this, theNdbOp,
                      theNdbCon, anExecType));
  if (theState == Closed) DBUG_RETURN(BA_DONE);  // Nothing to do here
  if (theState == Invalid) DBUG_RETURN(BA_ERROR);
  if (theState == Active) {
  ...
  // activation callback
  if (theActiveHook != nullptr) {
    if (invokeActiveHook() == -1) DBUG_RETURN(BA_ERROR);
  }
 ...
 */
int NdbBlob::preCommit() {
  DBUG_ENTER("NdbBlob::preCommit");
  DBUG_PRINT("info", ("this=%p op=%p con=%p", this, theNdbOp, theNdbCon));
  if (theState == Closed) DBUG_RETURN(0);  // Nothing to do here
  if (theState == Invalid) DBUG_RETURN(-1);
...
*/
int NdbBlob::atNextResultNdbRecord(const char *keyinfo, Uint32 keyinfo_bytes) {
  DBUG_ENTER("NdbBlob::atNextResultNdbRecord");
  DBUG_PRINT("info", ("this=%p op=%p con=%p keyinfo_bytes=%lu", this, theNdbOp,
                      theNdbCon, (unsigned long)keyinfo_bytes));
  if (theState == Invalid) DBUG_RETURN(-1);
  assert(isScanOp());
  /* Get primary key. */
  memcpy(thePackKeyBuf.data, keyinfo, keyinfo_bytes);
...
int NdbBlob::atNextResultCommon() {
  DBUG_ENTER("NdbBlob::atNextResultCommon");
  // discard previous partition id before reading new one
  thePartitionId = noPartitionId();
  getHeadFromRecAttr();
  if (setPos(0) == -1) DBUG_RETURN(-1);
  if (theGetFlag) {
    assert(theGetSetBytes == 0 || theGetBuf != nullptr);
    Uint32 bytes = theGetSetBytes;
    if (readDataPrivate(theGetBuf, bytes) == -1) DBUG_RETURN(-1);
  }
  setState(Active);
  // activation callback
  if (theActiveHook != nullptr) {
    if (invokeActiveHook() == -1) DBUG_RETURN(-1);
  }
  DBUG_RETURN(0);
}
 ...
 */
int NdbBlob::atNextEvent() {
  DBUG_ENTER("NdbBlob::atNextEvent");
  Uint32 optype =
      SubTableData::getOperation(theEventOp->m_data_item->sdata->requestInfo);
  DBUG_PRINT("info", ("this=%p op=%p blob op=%p version=%d optype=%u", this,
                      theEventOp, theBlobEventOp, theEventBlobVersion, optype));
  if (theState == Invalid) DBUG_RETURN(-1);
  assert(theEventBlobVersion >= 0);
  if (optype >= NdbDictionary::Event::_TE_FIRST_NON_DATA_EVENT) DBUG_RETURN(0);
  getHeadFromRecAttr();
...
const NdbDictionary::Column *NdbBlob::getColumn() { return theColumn; }

// errors

void NdbBlob::setErrorCode(int anErrorCode, bool invalidFlag) {
  ...
  if (invalidFlag) setState(Invalid);
#ifdef VM_TRACE
...
void NdbBlob::setErrorCode(NdbOperation *anOp, bool invalidFlag) {
  int code = 0;
...
void NdbBlob::setErrorCode(NdbEventOperationImpl *anOp, bool invalidFlag) {
  int code = 0;
...
NdbBlob *NdbBlob::blobsFirstBlob() { return theNdbOp->theBlobList; }

NdbBlob *NdbBlob::blobsNextBlob() { return theNext; }

const NdbOperation *NdbBlob::getNdbOperation() const { return theNdbOp; }

int NdbBlob::close(bool execPendingBlobOps) {
  ...
  if (theNdbOp->theLockHandle) {
    DBUG_PRINT("info", ("Decrementing lockhandle Blob ref count to %d",
  ...
  if (theNdbOp->m_blob_lock_upgraded) {
    ...
    if (likely(theNdbOp->theLockHandle != nullptr)) {
      if (theNdbOp->theLockHandle->m_openBlobCount == 0) {
        DBUG_PRINT("info", ("Upgraded -> LM_Read lock "
        ...
        if (unlikely(op == nullptr)) {
          ...
          setErrorCode((NdbOperation *)nullptr,
                       true);  // Set Blob to invalid state
          DBUG_RETURN(-1);
        }
        ...
        if (unlikely(theNdbCon->releaseLockHandle(theNdbOp->theLockHandle) !=
                     0)) {
          setErrorCode(theNdbCon->theError.code,
                       true);  // Set Blob to invalid state
          DBUG_RETURN(-1);
        }
      }
...
bool NdbBlob::isOpTypeSafeWithBatch(const Uint32 batchOpTypes,
  ...
  if (batchOpTypes != 0) {
    ...
    if ((newOpType & notSafeOpTypes) != 0) {
      DBUG_PRINT("info", ("Not safe op type batchopTypes %x "
                          "newOpType %x",
    ...
    if (newOpType != batchOpTypes) {
      DBUG_PRINT("info", ("Not safe different op type "
  ...
  DBUG_PRINT("info",
             ("Safe batchopTypes %x newOpType %x", batchOpTypes, newOpType));
