...
ConfigRetriever::~ConfigRetriever() {
  ...
  if (m_handle) {
    if (ndb_mgm_is_connected(m_handle)) {
      if (m_end_session) ndb_mgm_end_session(m_handle);
...
ndb_mgm::config_ptr ConfigRetriever::getConfig(Uint32 nodeid) {
  if (!m_handle) return {};
  ...
  if (!verifyConfig(conf.get(), nodeid)) return {};
  
  return conf;
}

ndb_mgm::config_ptr ConfigRetriever::getConfig(NdbMgmHandle mgm_handle) {
  const int from_node = 0;
  ndb_mgm::config_ptr conf(ndb_mgm_get_configuration2(mgm_handle, m_version,
                                                      m_node_type, from_node));
  if (!conf) {
...
ndb_mgm::config_ptr ConfigRetriever::getConfig(const char *filename) {
  BaseString err;
...
void ConfigRetriever::setError(ErrorType et, const char *s) {
  errorString.assign(s ? s : "");
  latestErrorType = et;
  DBUG_PRINT("info", ("latestErrorType: %u, '%s'", latestErrorType,
                      errorString.c_str()));
}
...
void ConfigRetriever::resetError() { setError(CR_NO_ERROR, nullptr); }

int ConfigRetriever::hasError() { return latestErrorType != CR_NO_ERROR; }

const char *ConfigRetriever::getErrorString() { return errorString.c_str(); }

bool ConfigRetriever::verifyConfig(const ndb_mgm_configuration *conf,
  ...
  if (_type != (unsigned int)m_node_type) {
    ...
    alias_s = ndb_mgm_get_node_type_alias_string(
        (enum ndb_mgm_node_type)m_node_type, &type_s);
...
Uint32 ConfigRetriever::allocNodeId(int no_retries, int retry_delay_in_seconds,
  ...
  BaseString tmp;
  tmp.assfmt("%s: %s", ndb_mgm_get_latest_error_msg(m_handle),
