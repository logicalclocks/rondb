...
bool printDROP_DATABASE_REQ(FILE *output,
                            const Uint32 *theData,
                            Uint32 /* len */,
                            Uint16 /*receiverBlockNo*/)
{
  const DropDatabaseReq *const sig = (const DropDatabaseReq *)theData;
...
bool printDROP_DATABASE_CONF(FILE *output,
                             const Uint32 *theData,
                             Uint32 /* len */,
                             Uint16 /*receiverBlockNo*/)
{
  const DropDatabaseConf *const sig =
    (const DropDatabaseConf *)theData;
  fprintf(output, " senderRef: H\'%.8x, senderData: %.8u\n", 
	  sig->senderRef, sig->senderData);
  fprintf(output, " databaseId: %.8u, databaseVersion: %.8u\n", 
	  sig->databaseId, sig->databaseVersion);
...
bool printDROP_DATABASE_REF(FILE *output,
                            const Uint32 *theData,
                            Uint32 /* len */,
                            Uint16 /*receiverBlockNo*/)
{
  const DropDatabaseRef *const sig =
    (const DropDatabaseRef *)theData;
  fprintf(output, " senderRef: H\'%.8x, senderData: %.8u\n", 
	  sig->senderRef, sig->senderData);
  fprintf(output, " transId: H\'%.8x\n", 
	  sig->transId);
