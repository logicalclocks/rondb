...
static void remove_use_processor_set(Uint32 proc_set_id) {
...
static Uint32 find_processor_set(struct NdbThread *pThread) {
  const processor_set_handler *handler = NdbThread_LockGetCPUSetKey(pThread);
  if (handler == nullptr) return UNDEFINED_PROCESSOR_SET;
  return handler->index;
}

static void init_handler(processor_set_handler *handler, unsigned int i) {
  handler->ref_count = 0;
  handler->cpu_ids = nullptr;
  handler->num_cpu_ids = 0;
  handler->index = i;
...
static int use_processor_set(const Uint32 *cpu_ids, Uint32 num_cpu_ids,
                             Uint32 *proc_set_id, int is_exclusive) {
  int ret = 0;
  Uint32 i;
  ...
  for (i = 0; i < num_processor_sets; i++) {
    ...
    if (handler->num_cpu_ids == num_cpu_ids &&
        (memcmp(cpu_ids, handler->cpu_ids, sizeof(Uint32) * num_cpu_ids) ==
         0)) {
  ...
  while (1) {
    for (i = 0; i < num_processor_sets; i++) {
      ...
      if (handler->ref_count == 0)
      {
        ...
        if (is_exclusive) {
          ret = NdbThread_LockCreateCPUSetExclusive(cpu_ids, num_cpu_ids,
                                                    &handler->ndb_cpu_set);
        } else {
    ...
    memcpy(new_proc_set_array, proc_set_array,
...
int Ndb_UnlockCPU(struct NdbThread *pThread) {
  int error_no = 0;
  Uint32 proc_set_id;
...
int Ndb_LockCPUSet(struct NdbThread *pThread, const Uint32 *cpu_ids,
  ...
  NdbMutex_Lock(ndb_lock_cpu_mutex);
  if ((error_no = use_processor_set(cpu_ids, num_cpu_ids, &proc_set_id,
                                    is_exclusive))) {
    goto end;
  }
  ...
  if (is_exclusive) {
    if ((error_no = NdbThread_LockCPUSetExclusive(pThread, handler->ndb_cpu_set,
                                                  handler))) {
      remove_use_processor_set(proc_set_id);
    }
  } else {
    if ((error_no =
             NdbThread_LockCPUSet(pThread, handler->ndb_cpu_set, handler))) {
      remove_use_processor_set(proc_set_id);
    }
  }
...
int Ndb_LockCPU(struct NdbThread *pThread, Uint32 cpu_id) {
  int error_no;
  NdbMutex_Lock(ndb_lock_cpu_mutex);
  error_no = NdbThread_LockCPU(pThread, cpu_id, nullptr);
  NdbMutex_Unlock(ndb_lock_cpu_mutex);
  return error_no;
}
...
int NdbLockCpu_Init() {
  ...
  delete[] proc_set_array;
  proc_set_array = new (std::nothrow) processor_set_handler[num_processor_sets];
