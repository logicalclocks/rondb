...
class Expression {
 public:
  virtual ~Expression() {}
  virtual bool evaluate(SimpleCpcClient *,
                        const SimpleCpcClient::Process &) = 0;
};
...
class Match : public Expression {
 ...
 public:
  ...
  bool evaluate(SimpleCpcClient *c,
                const SimpleCpcClient::Process &p) override {
    if (m_cond.evaluate(c, p)) return m_apply.evaluate(c, p);
    return false;
  }
};
...
class OrExpr : public Expression {
 ...
 public:
  ...
  bool evaluate(SimpleCpcClient *c,
    ...
    if (run) return m_rule->evaluate(c, p);
    return false;
  }
...
void add_host(Vector<SimpleCpcClient *> &hosts, BaseString tmp) {
  ...
  if (split.size() > 1) port = atoi(split[1].c_str());
...
int connect(Vector<SimpleCpcClient *> &list) {
  for (unsigned i = 0; i < list.size(); i++) {
    if (list[i]->connect() != 0) {
      ndbout_c("Failed to connect to %s:%d", list[i]->getHost(),
               list[i]->getPort());
      delete list[i];
      list[i] = 0;
    }
...
int for_each(Vector<SimpleCpcClient *> &list, Expression &expr) {
  for (unsigned i = 0; i < list.size(); i++) {
    if (list[i] == 0) continue;
    Properties p;
    Vector<SimpleCpcClient::Process> procs;
    if (list[i]->list_processes(procs, p) != 0) {
      ndbout << "Failed to list processes on " << list[i]->getHost() << ":"
             << list[i]->getPort() << endl;
    }
...
bool Operate::evaluate(SimpleCpcClient *c, const SimpleCpcClient::Process &pp) {
  Uint32 id = pp.m_id;
  Properties p;
  int res;
  ...
  if (native_strcasecmp(cmd, "start") == 0)
    ...
    if (!sets.m_longl) {
      ndbout_c("%c%c\t%d\t%s\t%s\t%s(%s)", s, pp.m_type.c_str()[0], id,
               pp.m_owner.c_str(), pp.m_group.c_str(), pp.m_name.c_str(),
               pp.m_path.c_str());
    } else {
      ndbout_c("%c%c %s:%d:%d %s %s %s(%s)", s, pp.m_type.c_str()[0],
               c->getHost(), c->getPort(), id, pp.m_owner.c_str(),
  ...
  if (res != 0) {
    ...
    ndbout_c("Failed to %s %d on %s:%d - %s", cmd, id, c->getHost(),
             c->getPort(), msg.c_str());
    return false;
  }
