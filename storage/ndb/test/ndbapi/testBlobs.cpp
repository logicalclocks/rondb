...
static void printusage() {
  ...
  ndbout
    ...
    << "  -dbg        print program debug" << endl
      << "  -debug opt  also ndb api DBUG (if no ':' becomes d:t:F:L:o,opt)"
      << endl
    ...
    << d.m_timeout_retries << "]" << endl
      ...
      << endl
    << "metadata" << endl
      << "  -pk2len N   native length of PK2, zero omits PK2,PK3 ["
      << d.m_pk2chr.m_len << "]" << endl
    ...
    << "  -oneblob    only 1 blob attribute [default 2]" << endl
      << "  -rbatch     N Read parts batchsize (bytes) [default -1] -1=random"
      << endl
      << "  -wbatch     N Write parts batchsize (bytes) [default -1] -1=random"
      << endl
      << "disk or memory storage for blobs.  Don't apply to performance test"
      << endl
    ...
    << "example: -test makn0 (need all 4 parts)" << endl
      << "example: -test mhabkisrunwd012 (Everything except performance tests"
      << endl
    << "bug tests" << endl
    << "  -bug 4088   ndb api hang with mixed ops on index table" << endl
    << "  -bug 27018  middle partial part write clobbers rest of part" << endl
      << "  -bug 27370  Potential inconsistent blob reads for ReadCommitted "
         "reads"
      << endl
...
static bool testcase(char x) {
  if (x < 10) x += '0';

  return (g_opt.m_test == 0 || strchr(g_opt.m_test, x) != 0) &&
...
static const char *stylename[3] = {"style=getValue/setValue",
  "style=setActiveHook",
                                   "style=readData/writeData"};

// Blob API variants
static const char *apiName[2] = {"api=NdbRecAttr", "api=NdbRecord"};
...
static const char *storageName[2] = {"storage=memory", "storage=disk"};
...
static void printerror(int line, const char *msg) {
  ...
  if (g_con != 0 && g_con->getNdbError().code != 0) {
    ...
    if (g_opr != 0 && g_opr->getNdbError().code != 0) {
      ndbout << "opr: table=" << g_opr->getTableName() << " "
             << g_opr->getNdbError() << endl;
    }
    if (g_const_opr != 0 && g_const_opr->getNdbError().code != 0) {
      ndbout << "const_opr: table=" << g_const_opr->getTableName() << " "
             << g_const_opr->getNdbError() << endl;
    }
    if (g_opx != 0 && g_opx->getNdbError().code != 0) {
      ndbout << "opx: table=" << g_opx->getTableName() << " "
             << g_opx->getNdbError() << endl;
    }
    if (g_ops != 0 && g_ops->getNdbError().code != 0) {
      ndbout << "ops: table=" << g_ops->getTableName() << " "
             << g_ops->getNdbError() << endl;
    }
    ...
    if (ope != 0 && ope->getNdbError().code != 0) {
      if (ope != g_opr && ope != g_const_opr && ope != g_opx && ope != g_ops)
        ndbout << "ope: ptr=" << ope << " table=" << ope->getTableName()
               << " type= " << ope->getType() << " " << ope->getNdbError()
               << endl;
    }
  }
...
static int createDefaultTableSpace() {
  ...
  if (strcmp(lg.getName(), "DEFAULT-LG") != 0) {
    ...
    if (res != 0) {
      DBG("Failed to create logfilegroup:" << endl
                                           << g_dic->getNdbError() << endl);
      return -1;
    }
  }
  {
    ...
    if (strcmp(uf.getPath(), "undofile01.dat") != 0) {
      ...
      if (res != 0) {
        DBG("Failed to create undofile:" << endl
                                         << g_dic->getNdbError() << endl);
	return -1;
      }
    }
  ...
  if (strcmp(ts.getName(), g_tsName) != 0) {
    ...
    if (res != 0) {
      DBG("Failed to create tablespace:" << endl
                                         << g_dic->getNdbError() << endl);
      return -1;
    }
  }
  
  {
    ...
    if (strcmp(df.getPath(), "datafile01.dat") != 0) {
      ...
      if (res != 0) {
        DBG("Failed to create datafile:" << endl
                                         << g_dic->getNdbError() << endl);
	return -1;
      }
    }
...
static int dropTable() {
  ...
  if (g_key_record != NULL) g_dic->releaseRecord(g_key_record);
  if (g_blob_record != NULL) g_dic->releaseRecord(g_blob_record);
  if (g_full_record != NULL) g_dic->releaseRecord(g_full_record);

  if (g_opt.m_pk2chr.m_len != 0) {
    if (g_idx_record != NULL) g_dic->releaseRecord(g_idx_record);
    if (g_ord_record != NULL) g_dic->releaseRecord(g_ord_record);
  }
...
static unsigned urandom(unsigned n) { return n == 0 ? 0 : ndb_rand() % n; }

static int createTable(int storageType) {
  ...
  NdbDictionary::Column::StorageType blobStorageType =
      (storageType == STORAGE_MEM) ? NdbDictionary::Column::StorageTypeMemory
                                   : NdbDictionary::Column::StorageTypeDisk;

  NdbDictionary::Table tab(g_opt.m_tname);
  if (storageType == STORAGE_DISK) tab.setTablespaceName(g_tsName);
  ...
  Uint32 fragType =
      NdbDictionary::Object::DistrKeyHash + urandom(fragTypeRange);

  /* Value 8 is unused currently, map it to something else */
  if (fragType == 8) fragType = NdbDictionary::Object::UserDefined;
  ...
  if (pk2chr.m_len != 0) {
    ...
    if (pk2chr.m_csinfo != 0) col.setCharset(pk2chr.m_csinfo);
    if (g_opt.m_pk2part) col.setPartitionKey(true);
    tab.addColumn(col);
  }
  ...
  CHK((g_blob_record = g_dic->createRecord(dict_table, &spec[numpks], numblobs,
                                         sizeof(spec[0]))) != 0);
  CHK((g_full_record = g_dic->createRecord(
           dict_table, &spec[0], numpks + numblobs, sizeof(spec[0]))) != 0);
...
struct Bval {
  ...
  void alloc() { alloc(m_bcol.m_inline + m_bcol.m_partsize * g_opt.m_parts); }
...
NdbOut &operator<<(NdbOut &out, const Bval &v) {
...
struct Tup {
  ...
  Tup()
      : m_exists(false),
        m_pk2(
            new char[g_opt.m_pk2chr.m_totlen + 1]),  // nullterm for convenience
   ...
   */
  char *pk2() {
    if (g_opt.m_blob_version == 1) return m_pk2;
...
static void setUDpartId(const Tup &tup, NdbOperation *op) {
...
static void setUDpartIdNdbRecord(const Tup &tup,
...
static void calcBval(const Bcol &b, Bval &v, bool keepsize) {
  if (b.m_nullable && urandom(10) == 0) {
    ...
    for (unsigned i = 0; i < v.m_len; i++) v.m_val[i] = 'a' + urandom(26);
    v.m_val[v.m_len] = 0;
    delete[] v.m_buf;
...
static bool conHasTimeoutError() {
  ...
  if (!isTimeout)
    ndbout << "Connection error is not timeout, but is " << code << endl;
  
  return isTimeout;
}

static Uint32 conError() { return g_con->getNdbError().code; }

static void calcBval(Tup &tup, bool keepsize) {
  calcBval(g_blob1, tup.m_bval1, keepsize);
  if (!g_opt.m_oneblob) calcBval(g_blob2, tup.m_bval2, keepsize);
}

// dont remember what the keepsize was for..
static void calcTups(bool keys, bool keepsize = false) {
...
static void setBatchSizes() {
  if (g_opt.m_rbatch != 0) {
    Uint32 byteSize =
        (g_opt.m_rbatch == -1) ? urandom(~Uint32(0)) : g_opt.m_rbatch;
    
    DBG("Setting read batch size to " << byteSize << " bytes.");
    g_con->setMaxPendingBlobReadBytes(byteSize);
  }
  
  if (g_opt.m_wbatch != 0) {
    Uint32 byteSize =
        (g_opt.m_wbatch == -1) ? urandom(~Uint32(0)) : g_opt.m_wbatch;
    
    DBG("Setting write batch size to " << byteSize << " bytes.");
...
static int getBlobHandles(const NdbOperation *opr) {
  CHK((g_bh1 = opr->getBlobHandle("BL1")) != 0);
  if (!g_opt.m_oneblob) CHK((g_bh2 = opr->getBlobHandle("BL2")) != 0);
...
// and scans
static int getBlobHandles(NdbOperation *opr) {
  CHK((g_bh1 = opr->getBlobHandle("BL1")) != 0);
  if (!g_opt.m_oneblob) CHK((g_bh2 = opr->getBlobHandle("BL2")) != 0);
  setBatchSizes();
  return 0;
...
static int getBlobHandles(NdbScanOperation *ops) {
  CHK((g_bh1 = ops->getBlobHandle("BL1")) != 0);
  if (!g_opt.m_oneblob) CHK((g_bh2 = ops->getBlobHandle("BL2")) != 0);
  setBatchSizes();
  return 0;
}

static int getBlobLength(NdbBlob *h, unsigned &len) {
...
static int setBlobValue(NdbBlob *h, const Bval &v, int error_code = 0) {
  bool null = (v.m_val == 0);
  bool isNull;
  unsigned len;
  DBG("setValue " << h->getColumn()->getName() << " len=" << v.m_len
                  << " null=" << null << " " << v);
  if (null) {
...
static int setBlobValue(const Tup &tup, int error_code = 0) {
  CHK(setBlobValue(g_bh1, tup.m_bval1, error_code) == 0);
  if (!g_opt.m_oneblob) CHK(setBlobValue(g_bh2, tup.m_bval2, error_code) == 0);
  return 0;
}

static int getBlobValue(NdbBlob *h, const Bval &v) {
  DBG("getValue " <<  h->getColumn()->getName() << " buflen=" << v.m_buflen);
  CHK(h->getValue(v.m_buf, v.m_buflen) == 0);
  return 0;
}

static int getBlobValue(const Tup &tup) {
  CHK(getBlobValue(g_bh1, tup.m_bval1) == 0);
  if (!g_opt.m_oneblob) CHK(getBlobValue(g_bh2, tup.m_bval2) == 0);
  return 0;
}
...
static int presetBH1(int rowNumber) {
  ...
  if (variant == 0)
    ...
    CHK(setBlobValue(g_tups[(rowNumber + 1) % g_opt.m_rows]) ==
        0);  // Pre-set to something else
  };
  return 0;
}

static int verifyBlobValue(NdbBlob *h, const Bval &v) {
  bool null = (v.m_val == 0);
  bool isNull;
  unsigned len;
  if (null) {
    ...
    CHK(h->getNull(isNull) == 0 && isNull == false);
    CHK(getBlobLength(h, len) == 0 && len == v.m_len);
    for (unsigned i = 0; i < v.m_len; i++) CHK(v.m_val[i] == v.m_buf[i]);
  }
  return 0;
}

static int verifyBlobValue(const Tup &tup) {
  CHK(verifyBlobValue(g_bh1, tup.m_bval1) == 0);
  if (!g_opt.m_oneblob) CHK(verifyBlobValue(g_bh2, tup.m_bval2) == 0);
  return 0;
}

// readData / writeData

static int writeBlobData(NdbBlob *h, const Bval &v) {
  bool null = (v.m_val == 0);
  bool isNull;
  unsigned len;
  DBG("write " << h->getColumn()->getName() << " len=" << v.m_len
               << " null=" << null << " " << v);
  ...
  if (null) {
    ...
    if (error_code) return 0;
    CHK(h->setPos(0) ==
        0);  // Reset write pointer in case there was a previous write.
    unsigned n = 0;
    do {
      unsigned m = g_opt.m_full ? v.m_len : urandom(v.m_len + 1);
      if (m > v.m_len - n) m = v.m_len - n;
...
static int writeBlobData(Tup &tup, int error_code = 0) {
...
static int readBlobData(NdbBlob *h, const Bval &v) {
  bool null = (v.m_val == 0);
  bool isNull;
  unsigned len;
  DBG("read " << h->getColumn()->getName() << " len=" << v.m_len
              << " null=" << null);
  if (null) {
    isNull = false;
    CHK(h->getNull(isNull) == 0 && isNull == true);
...
static int readBlobData(const Tup &tup) {
  CHK(readBlobData(g_bh1, tup.m_bval1) == 0);
  if (!g_opt.m_oneblob) CHK(readBlobData(g_bh2, tup.m_bval2) == 0);
  return 0;
}
...
static int blobWriteHook(NdbBlob *h, void *arg) {
  DBG("blobWriteHook");
  Bval &v = *(Bval *)arg;
...
static int setBlobWriteHook(NdbBlob *h, Bval &v, int error_code = 0) {
...
static int setBlobWriteHook(Tup &tup, int error_code = 0) {
...
static int blobReadHook(NdbBlob *h, void *arg) {
  DBG("blobReadHook");
  Bval &v = *(Bval *)arg;
  unsigned len;
...
static int setBlobReadHook(NdbBlob *h, Bval &v) {
  DBG("setBlobReadHook");
  CHK(h->setActiveHook(blobReadHook, &v) == 0);
  return 0;
}

static int setBlobReadHook(Tup &tup) {
  CHK(setBlobReadHook(g_bh1, tup.m_bval1) == 0);
  if (!g_opt.m_oneblob) CHK(setBlobReadHook(g_bh2, tup.m_bval2) == 0);
  return 0;
}

static int tryRowLock(Tup &tup, bool exclusive) {
  NdbTransaction *testTrans;
  NdbOperation *testOp;
  CHK((testTrans = g_ndb->startTransaction()) != NULL);
  CHK((testOp = testTrans->getNdbOperation(g_opt.m_tname)) != 0);
  CHK(testOp->readTuple(exclusive ? NdbOperation::LM_Exclusive
  ...
  if (testTrans->execute(Commit, AbortOnError) == 0) {
    ...
    if (testTrans->getNdbError().code == 266) {
      ...
      DBG("Error on tryRowLock, exclusive = " << exclusive << endl
                                              << testTrans->getNdbError()
                                              << endl);
      testTrans->close();
      return -1;
    }
...
static int verifyRowLocked(Tup &tup) {
  CHK(tryRowLock(tup, true) == -2);
  return 0;
}

static int verifyRowNotLocked(Tup &tup) {
...
static int verifyHeadInline(const Bcol &b, const Bval &v, NdbRecAttr *ra) {
  if (v.m_val == 0) {
    CHK(ra->isNULL() == 1);
  } else {
    CHK(ra->isNULL() == 0);
...
static int verifyHeadInline(Tup &tup) {
  ...
  if (!g_opt.m_oneblob) CHK((ra2 = g_opr->getValue("BL2")) != 0);
...
static unsigned getvarsize(const char *buf) {
  const unsigned char *p = (const unsigned char *)buf;
  return p[0] + (p[1] << 8);
}

static int verifyBlobTable(const Bval &v, Uint32 pk1, Uint32 frag,
                           bool exists) {
  const Bcol &b = v.m_bcol;
  ...
  do {
    ...
    CHK(g_ops->readTuples(NdbScanOperation::LM_Read, g_scanFlags, g_batchSize,
                          g_parallel) == 0);
    if (b.m_version == 1) {
    ...
    while (1) {
      ...
      if (ret == -1) {
        ...
        DISP("Parts table scan failed due to timeout("
             << conError() << ").  Retries left : " << opTimeoutRetries - 1);
      ...
      if (ret == 1) break;
      if (b.m_version == 1) {
        if (pk1 != ra_pk->u_32_value()) continue;
      } else {
        if (pk1 != ra_pk1->u_32_value()) continue;
      }
      ...
      if (m > b.m_partsize) m = b.m_partsize;
      ...
      DBG("frags main=" << frag << " blob=" << frag2
                        << " stripe=" << b.m_stripe);
      if (b.m_stripe == 0) CHK(frag == frag2);
    }
    
    if (opState == Normal) {
      for (unsigned i = 0; i < partcount; i++) CHK(seen[i] == 1);
    }
    delete[] seen;
    g_ops->close();
    g_ndb->closeTransaction(g_con);
...
static int verifyBlobTable(const Tup &tup) {
...
static int rowIsLocked(Tup &tup) {
  NdbTransaction *testTrans;
...
static int insertPk(int style, int api) {
  ...
  do {
    ...
    for (; k < g_opt.m_rows; k++) {
      ...
      if (api == API_RECATTR) {
        ...
        NdbOperation::OperationOptions opts;
        setUDpartIdNdbRecord(
            tup, g_ndb->getDictionary()->getTable(g_opt.m_tname), opts);
        CHK((g_const_opr = g_con->insertTuple(g_full_record, tup.m_row, NULL,
                                              &opts, sizeof(opts))) != 0);
...
static int readPk(int style, int api) {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    do {
      ...
      if (api == API_RECATTR) {
        ...
        if (g_opt.m_pk2chr.m_len != 0) {
          memcpy(&tup.m_key_row[g_pk2_offset], tup.pk2(),
                 g_opt.m_pk2chr.m_totlen);
        ...
        CHK((g_const_opr = g_con->readTuple(g_key_record, tup.m_key_row,
                                            g_blob_record, tup.m_row, lm, NULL,
                                            &opts, sizeof(opts))) != 0);
      ...
      if (timeout) {
        ...
        CHK((g_opr ? g_opr : g_const_opr)->getLockMode() ==
...
static int readLockPk(int style, int api) {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    do {
      ...
      bool manualUnlock =
          ((lm == NdbOperation::LM_Read) || (lm == NdbOperation::LM_Exclusive));
      ...
      if (!timeout) {
        if (g_con->execute(NoCommit) == 0) {
          ...
          NdbOperation::LockMode lmused =
              (g_opr ? g_opr : g_const_opr)->getLockMode();
          ...
          if (urandom(200) == 0) CHK(verifyRowLocked(tup) == 0);
...
static int updatePk(int style, int api) {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    do {
      ...
      if (api == API_RECATTR) {
        ...
        if (mode == 0) {
          ...
          CHK((g_const_opr = g_con->updateTuple(g_key_record, tup.m_key_row,
                                                g_blob_record, tup.m_row, NULL,
                                                &opts, sizeof(opts))) != 0);
        } else if (mode == 1) {
          DBG("using readTuple exclusive");
          CHK((g_const_opr =
                   g_con->readTuple(g_key_record, tup.m_key_row, g_blob_record,
                                    tup.m_row, NdbOperation::LM_Exclusive, NULL,
                                    &opts, sizeof(opts))) != 0);
        } else {
          DBG("using readTuple - will fail and retry");
          CHK((g_const_opr = g_con->readTuple(
                   g_key_record, tup.m_key_row, g_blob_record, tup.m_row,
                   NdbOperation::LM_Read, NULL, &opts, sizeof(opts))) != 0);
        }
        CHK(getBlobHandles(g_const_opr) == 0);
      }
      ...
      if (error_code) opState = Retrying;
...
static int writePk(int style, int api) {
    ...
    do {
      ...
      if (api == API_RECATTR) {
        ...
        setUDpartIdNdbRecord(
            tup, g_ndb->getDictionary()->getTable(g_opt.m_tname), opts);
        CHK((g_const_opr =
                 g_con->writeTuple(g_key_record, tup.m_key_row, g_full_record,
                                   tup.m_row, NULL, &opts, sizeof(opts))) != 0);
        CHK(getBlobHandles(g_const_opr) == 0);
      }
      bool timeout = false;
      if (style == 0) {
    ...
    for (; k < g_opt.m_rows; k++) {
      ...
      if (api == API_RECATTR) {
        ...
        setUDpartIdNdbRecord(
            tup, g_ndb->getDictionary()->getTable(g_opt.m_tname), opts);
        CHK((g_const_opr =
                 g_con->deleteTuple(g_key_record, tup.m_key_row, g_full_record,
                                    NULL, NULL, &opts, sizeof(opts))) != 0);
      }
      if (++n == g_opt.m_batch) {
        if (g_con->execute(Commit) != 0) {
          CHK(conHasTimeoutError());
...
static int readIdx(int style, int api) {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    do {
      ...
      if (api == API_RECATTR) {
        CHK((g_opx = g_con->getNdbIndexOperation(g_opt.m_x1name,
                                                 g_opt.m_tname)) != 0);
      ...
      if (style == 0) {
        ...
        if (g_con->execute(NoCommit) || readBlobData(tup))
          CHK((timeout = conHasTimeoutError()) == true);
      }
      if (!timeout) {
        if (g_con->execute(Commit) != 0) {
      ...
      if (!timeout) {
        ...
        CHK((g_opx ? g_opx : g_const_opr)->getLockMode() ==
            NdbOperation::LM_Read);
        if (style == 0 || style == 1) {
          CHK(verifyBlobValue(tup) == 0);
        }
...
static int updateIdx(int style, int api) {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    do {
      ...
      if (style == 0) {
        ...
        if (g_con->execute(NoCommit) || writeBlobData(tup))
          CHK((timeout = conHasTimeoutError()) == true);
      }
      if (!timeout) {
        if (g_con->execute(Commit) != 0)
...
static int writeIdx(int style, int api) {
...
static int readScan(int style, int api, bool idx) {
  DBG("--- "
      << "readScan" << (idx ? "Idx" : "") << " " << stylename[style] << " "
      << apiName[api] << " ---");
  Tup tup;
  ...
  do {
    ...
    if (api == API_RECATTR) {
      if (!idx) {
        ...
        CHK((g_ops = g_con->getNdbIndexScanOperation(g_opt.m_x2name,
                                                     g_opt.m_tname)) != 0);
      }
      CHK(g_ops->readTuples(lm, g_scanFlags, g_batchSize, g_parallel) == 0);
      ...
      /* Don't bother setting UserDefined partitions for scan tests */
      if (!idx)
        CHK((g_ops = g_con->scanTable(g_full_record, lm)) != 0);
      else 
        CHK((g_ops = g_con->scanIndex(g_ord_record, g_full_record, lm)) != 0);
      CHK(getBlobHandles(g_ops) == 0);
    }
    ...
    if (opState == Normal) CHK(g_opt.m_rows == rows);
...
static int updateScan(int style, int api, bool idx) {
  DBG("--- "
      << "updateScan" << (idx ? "Idx" : "") << " " << stylename[style] << " "
      << apiName[api] << " ---");
  Tup tup;
  tup.alloc();  // allocate buffers
  ...
  do {
    ...
    if (api == API_RECATTR) {
      ...
      CHK(g_ops->readTuples(NdbOperation::LM_Exclusive, g_scanFlags,
                            g_batchSize, g_parallel) == 0);
      CHK(g_ops->getValue("PK1", (char *)&tup.m_pk1) != 0);
      if (g_opt.m_pk2chr.m_len != 0) {
        CHK(g_ops->getValue("PK2", tup.m_pk2) != 0);
        CHK(g_ops->getValue("PK3", (char *)&tup.m_pk3) != 0);
    ...
    while (1) {
      ...
      if (api == API_RECATTR) {
        ...
        CHK((g_const_opr = g_ops->updateCurrentTuple(g_con, g_blob_record,
                                                     tup.m_row)) != 0);
        CHK(getBlobHandles(g_const_opr) == 0);
      }
      bool timeout = false;
      if (style == 0) {
        ...
        CHK(g_con->execute(NoCommit) == 0);
        if (writeBlobData(tup)) CHK((timeout = conHasTimeoutError()) == true);
      }
      if (!timeout && (g_con->execute(NoCommit)))
        CHK((timeout = conHasTimeoutError()) == true);
...
static int lockUnlockScan(int style, int api, bool idx) {
  DBG("--- "
      << "lockUnlockScan" << (idx ? "Idx" : "") << " " << stylename[style]
      << " " << apiName[api] << " ---");
  Tup tup;
  tup.alloc();  // allocate buffers
  ...
  do {
    ...
    if (urandom(2) == 0) lm = NdbOperation::LM_Exclusive;
    ...
    if (api == API_RECATTR) {
      ...
      CHK(g_ops->readTuples(lm, scanFlags, g_batchSize, g_parallel) == 0);
      CHK(g_ops->getValue("PK1", (char *)&tup.m_pk1) != 0);
      if (g_opt.m_pk2chr.m_len != 0) {
        CHK(g_ops->getValue("PK2", tup.m_pk2) != 0);
        CHK(g_ops->getValue("PK3", (char *)&tup.m_pk3) != 0);
      ...
      /* Don't bother setting UserDefined partitions for scan tests */
      if (!idx)
        CHK((g_ops = g_con->scanTable(g_key_record, lm, 0, &opts,
                                      sizeof(opts))) != 0);
      else
        CHK((g_ops = g_con->scanIndex(g_ord_record, g_key_record, lm, 0, 0,
                                      &opts, sizeof(opts))) != 0);
    }
    CHK(g_con->execute(NoCommit) == 0);
    unsigned rows = 0;
    while (1) {
      ...
      DBG("lockUnlockScan" << (idx ? "Idx" : "") << " pk1=" << hex
                           << tup.m_pk1);
      ...
      if (api == API_RECATTR) {
        ...
        NdbOperation::OperationOptions opts;
        opts.optionsPresent = NdbOperation::OperationOptions::OO_LOCKHANDLE;
        CHK((g_const_opr = g_ops->lockCurrentTuple(
                 g_con, g_blob_record, tup.m_row, 0, &opts, sizeof(opts))) !=
            0);
        CHK(getBlobHandles(g_const_opr) == 0);
      }
      bool timeout = false;
      if (style == 0) {
        ...
        CHK(g_con->execute(NoCommit) == 0);
        if (readBlobData(tup)) CHK((timeout = conHasTimeoutError()) == true);
      }
      if (!timeout) {
        if (g_con->execute(NoCommit) == 0) {
          ...
          /* Close Blob handles */
          CHK(g_bh1->close() == 0);
          if (!g_opt.m_oneblob) CHK(g_bh2->close() == 0);
...
static int deleteScan(int api, bool idx) {
  DBG("--- "
      << "deleteScan" << (idx ? "Idx" : "") << apiName[api] << " ---");
  Tup tup;
  Uint32 opTimeoutRetries = g_opt.m_timeout_retries;
  enum OpState opState;
  unsigned rows = 0;
      ...
      while (1) {
        ...
        if (api == API_RECATTR)
          ...
          if (ret == 0) {
            memcpy(&tup.m_pk1, &out_row[g_pk1_offset], sizeof(tup.m_pk1));
            if (g_opt.m_pk2chr.m_len != 0) {
              memcpy(tup.m_pk2, &out_row[g_pk2_offset],
                     g_opt.m_pk2chr.m_totlen);
              memcpy(&tup.m_pk3, &out_row[g_pk3_offset], sizeof(tup.m_pk3));
            }
          }
...
static const char *operationName(OpTypes optype) {
  switch (optype) {
  case PkRead:
    return "Pk Read";
  case PkInsert:
...
static const char *aoName(int abortOption) {
  if (abortOption == 0) return "AbortOnError";
  return "IgnoreError";
}

static int setupOperation(NdbOperation *&op, OpTypes optype, Tup &tup) {
  bool pkop;
  switch (optype) {
    case PkRead:
    case PkInsert:
  ...
  DBG("SetupOperation : " << operationName(optype) << " pk1=" << hex
                          << tup.m_pk1);
...
>>>>>>> 6dcee9fa4b19e67dea407787eba88e360dd679d9
  ...
  DBG("                Also 36851 : Insert IgnoreError of existing tuple "
      "aborts");
  ...
  for (int iterations = 0; iterations < 50; iterations++) {
    ...
    for (int optype = PkRead; optype <= UkDelete; optype++) {
      ...
      for (int abortOption = 0; abortOption < 2; abortOption++) {
        ...
        DBG("execute returned " << rc << " Trans err "
                                << g_con->getNdbError().code << " Opr1 err "
                                << opr1->getNdbError().code << " Opr2 err "
                                << opr2->getNdbError().code << " CommitStatus "
                                << g_con->commitStatus());
...
static int bugtest_45768() {
  ...
  for (int iteration = 0; iteration < numIterations; iteration++) {
    ...
    do {
      ...
      if (g_con->getNdbError().code == 630) break; /* Expected */
  ...
  */
  ...
  for (int scenario = 0; scenario < 4; scenario++) {
    ...
    NdbTransaction::ExecType et =
        (scenario & 0x2) ? NdbTransaction::NoCommit : NdbTransaction::Commit;

    DBG("  Executing with execType = "
        << ((et == NdbTransaction::NoCommit) ? "NoCommit" : "Commit"));
    int rc = g_con->execute(NdbTransaction::NoCommit);
...
static int bugtest_28746560() {
  ...
  for (Uint32 pass = 0; pass < 2; pass++) {
    ...
    for (Uint32 row = 0; row < g_opt.m_rows; row++) {
      ...
      DBG("Row " << row);
      if (pass == 0) {
        OpTypes insType = ((urandom(2) == 1) ? PkInsert : PkWrite);
        NdbOperation *op;
      ...
      for (Uint32 mod = 0; mod < numBatchedModifies; mod++) {
        ...
        if (modifyStyle == 0 || modifyStyle == 1) {
          NdbOperation *op;
          CHK(setupOperation(op, (modifyStyle == 0 ? PkUpdate : PkWrite),
                             values[mod]) == 0);
          ...
          NdbOperation *delOp;
          CHK(setupOperation(delOp, PkDelete, values[mod]) == 0);

          NdbOperation *insOp;
          CHK(setupOperation(insOp, insOpType, values[mod]) == 0);

          DBG("  DEL" << name << " \t" << (void *)delOp << (void *)insOp);
        }
      ...
      if (pass == 1) {
        // define deleteRow
        NdbOperation *op;
        CHK(setupOperation(op, PkDelete, g_tups[row]) == 0);
      ...
      Tup &finalValue =
          (numBatchedModifies ? values[numBatchedModifies - 1] : g_tups[row]);
      ...
      if (pass == 0) {
        ...
        if (readOp->getNdbError().code != 626) {
          ndbout_c("Error, expected 626 but found %u %s",
                   readOp->getNdbError().code, readOp->getNdbError().message);
          return -1;
        }
        DBG("  READ DEL OK");
      }
...
static int testOpBatching(OpTypes opType, int ao, int minRts, int maxRts) {
  ...
  const bool readOp = (opType == PkRead || opType == UkRead);
  const bool deleteOp = (opType == PkDelete || opType == UkDelete);
  ...
  do {
    ...
    for (; k < g_opt.m_rows;) {
      ...
      r++;
      k++;
      if (r == rowsPerBatch) break;
    }

    DBG("commit abortoption " << aoName(ao) << " for " << rowsPerBatch << " ops"
                              << " expects between " << minRts << " and "
                              << maxRts << " waits");
    ...
    if (g_con->execute(Commit, (AbortOption)ao) == 0) {
      ...
      DBG("Execution of " << rowsPerBatch << " " << operationName(opType)
                          << " required " << total_waits << " waits.  MinRts "
                          << minRts << " MaxRts " << maxRts);
      ...
      CHK(conHasTimeoutError());
      DISP(operationName(opType)
           << " failed due to timeout(" << conError() << ")  "
           << " Operations lost : " << rowsPerBatch << " retries left "
           << opTimeoutRetries - 1);
      CHK(--opTimeoutRetries);
      r -= rowsPerBatch;
      k -= rowsPerBatch;
...
static int testBatchParallelism() {
...
static int testOpBatchBreaks(OpTypes opType, int numDupOps, int minRts,
                             int maxRts) {
  ...
  do {
    ...
    if (g_con->execute(Commit, (AbortOption)ao) == 0) {
      total_waits =
          (int)(g_ndb->getClientStat(Ndb::WaitExecCompleteCount) - start_waits);
      DBG("Execution of " << numDupOps << " " << operationName(opType)
                          << " duplicate ops required " << total_waits
                          << " waits.  Min " << minRts << " Max " << maxRts);

      // Improvement : Check data read is correct

      if (total_waits < minRts || total_waits > maxRts) {
        DISP("Error, execution of " << numDupOps << " " << operationName(opType)
                                    << " duplicate ops required " << total_waits
                                    << " waits.  Min " << minRts << " Max "
                                    << maxRts);
        CHK(false);
      }
      ...
      DISP(operationName(opType)
           << " failed due to timeout(" << conError() << ")  "
           << " Operations lost : " << numDupOps << " retries left "
           << opTimeoutRetries - 1);
      CHK(--opTimeoutRetries);
      NdbSleep_SecSleep(1);
    }
...
static int testBatchSerialisation() {
  ...
  CHK(testOpBatchBreaks(PkUpdate, numOps, 2 * numOps, 2 * numOps) == 0);

  /* Writes require 2 RTs per op */
  CHK(testOpBatchBreaks(PkWrite, numOps, 2 * numOps, 2 * numOps) == 0);

  /* Delete require 2 RTs */
  CHK(testOpBatchBreaks(PkDelete, numOps, 2, 2) == 0);

  /* Write with no previous rows, 2 or 3 per op */
  CHK(testOpBatchBreaks(PkWrite, numOps, 2 * numOps, 3 * numOps) == 0);

  /* Reads require 1,2 or 3, depending on length */
  CHK(testOpBatchBreaks(UkRead, numOps, 1, 3) == 0);

  /* Updates require 2 RTs per op */
  CHK(testOpBatchBreaks(UkUpdate, numOps, 2 * numOps, 2 * numOps) == 0);
...
static void setBval(Bval &v, int sz) {
  delete[] v.m_val;
  v.m_len = sz;
...
static void setBvals(Tup &tup, int sz) {
  /* Force Blob values to given length */
  setBval(tup.m_bval1, sz);
  if (!g_opt.m_oneblob) setBval(tup.m_bval2, sz);
}

static int calcMaxRts(unsigned numOps, int numBlobs, int blobSize,
                      int batchSize) {
...
static int testOpBatchLimits(OpTypes opType, unsigned numOps, int sz,
                             int minRts, int maxRts) {
  ...
  do {
    ...
    DBG("commit "
        << " for " << numOps << " ops"
        << " expects between " << minRts << " and " << maxRts << " waits");

    const Uint64 start_waits = g_ndb->getClientStat(Ndb::WaitExecCompleteCount);
    if (g_con->execute(Commit) == 0) {
      const Uint64 total_waits =
        g_ndb->getClientStat(Ndb::WaitExecCompleteCount) - start_waits;
      DBG("Execution of " << numOps << " " << operationName(opType)
                          << " required " << total_waits << " waits.  MinRts "
                          << minRts << " MaxRts " << maxRts);
      ...
      DISP(operationName(opType)
           << " failed due to timeout(" << conError() << ")  "
           << " Operations lost : " << numOps << " retries left "
           << opTimeoutRetries - 1);
      CHK(--opTimeoutRetries);
      k = 0;
      NdbSleep_SecSleep(1);
...
static int testBatchLimits() {
  ...
  DISP("     1 - " << maxRows << " rows size " << 1 * stepSize << " to "
                   << steps * stepSize << " bytes");
  for (int r = 1; r < 10; r++) {
    DISP("  --- " << r << " rows");
    for (int s = 1; s < steps; s++) {
      int blobSize = s * stepSize;
      int expectedMaxRts = calcMaxRts(r, blobs_per_row, blobSize, batchSize);
      DBG("  rows " << r);
      DBG("    size per blob " << blobSize << " row size "
          << blobSize * blobs_per_row);
      DBG("    expected max Rts " << expectedMaxRts);
...
static int testmain() {
  ...
  if (testcase('h')) createDefaultTableSpace();

  if (g_opt.m_seed == -1) g_opt.m_seed = NdbHost_GetProcessId();
  if (g_opt.m_seed != 0) {
  ...
  for (g_loop = 0; g_opt.m_loop == 0 || g_loop < g_opt.m_loop; g_loop++) {
    for (int storage = 0; storage < 2; storage++) {
      if (!testcase(storageSymbol[storage])) continue;
      ...
      { /* Dump created table information */
        ...
        CHK(NdbBlob::getBlobTableName(b1.m_btname, g_ndb, g_opt.m_tname,
                                      "BL1") == 0);
        DBG("BL1: inline=" << b1.m_inline << " part=" << b1.m_partsize
                           << " table=" << b1.m_btname);
        if (!g_opt.m_oneblob) {
          Bcol &b2 = g_blob2;
          CHK(NdbBlob::getBlobTableName(b2.m_btname, g_ndb, g_opt.m_tname,
                                        "BL2") == 0);
          DBG("BL2: inline=" << b2.m_inline << " part=" << b2.m_partsize
                             << " table=" << b2.m_btname);
        }
      }
      ...
      // TODO Remove/resolve
      DBG("Settings : usingdisk " << g_usingDisk << " batchSize " << g_batchSize
                                  << " parallel " << g_parallel << " scanFlags "
                                  << g_scanFlags);

      int style;
      int api;
      DBG("=== loop " << g_loop << " ===");
      if (g_opt.m_seed == 0) ndb_srand(g_loop);
      ...
      for (api = 0; api <= 1; api++) {
        // pk
        if (!testcase(apiSymbol[api])) continue;
        for (style = 0; style <= 2; style++) {
          if (!testcase('k') || !testcase(style)) continue;
        ...
        // hash index
        for (style = 0; style <= 2; style++) {
          if (!testcase('i') || !testcase(style)) continue;
        ...
        // scan table
        for (style = 0; style <= 2; style++) {
          if (!testcase('s') || !testcase(style)) continue;
        ...
        // scan index
        for (style = 0; style <= 2; style++) {
          if (!testcase('r') || !testcase(style)) continue;
...
struct Tmr {    // stolen from testOIBasic
  Tmr() { clr(); }
  void clr() { m_on = m_ms = m_cnt = m_time[0] = m_text[0] = 0; }
  void on() {
    require(m_on == 0);
  ...
  const char *time() {
    if (m_cnt == 0)
      ...
      sprintf(m_time, "%u ms per %u ( %llu ms per 1000 )", (Uint32)m_ms, m_cnt,
              (1000 * m_ms) / m_cnt);
    return m_time;
  }
...
static int bugtest_4088() {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    CHK((g_opx = g_con->getNdbIndexOperation(g_opt.m_x1name, g_opt.m_tname)) !=
        0);
    ...
    CHK(memcmp(tup.m_bval1.m_val, tup.m_bval1.m_buf, 8 + g_blob1.m_inline) ==
        0);
  }
  return 0;
}

static int bugtest_27018() {
  ...
  for (unsigned k = 0; k < g_opt.m_rows; k++) {
    ...
    NdbOperation::OperationOptions opts;
    setUDpartIdNdbRecord(tup, g_ndb->getDictionary()->getTable(g_opt.m_tname),
                         opts);
    CHK((g_const_opr = g_con->updateTuple(g_key_record, tup.m_key_row,
    ...
    CHK((g_const_opr =
             g_con->readTuple(g_key_record, tup.m_key_row, g_blob_record,
                              tup.m_row, NdbOperation::LM_Read,
                                       NULL, // mask
                              &opts, sizeof(opts))) != 0);
    CHK(getBlobHandles(g_const_opr) == 0);
    ...
    // CHK(memcmp(tup.m_bval1.m_buf, tup.m_bval1.m_val, tup.m_bval1.m_len) ==
    // 0);
    Uint32 i = 0;
...
struct bug27370_data {
  bug27370_data() : m_ndb(NULL), m_writebuf(NULL), m_key_row(NULL) {}
...
void *bugtest_27370_thread(void *arg) {
  ...
  while (!data->m_thread_stop) {
    ...
    if ((opr = con->writeTuple(
             g_key_record, data->m_key_row, g_full_record, data->m_write_row,
             NULL,  // mask
             data->opts, sizeof(NdbOperation::OperationOptions))) == 0)
...
static int bugtest_27370() {
  DBG("bug test 27370 - Potential inconsistent blob reads for ReadCommitted "
      "reads");
  ...
  bool isUserDefined =
      (t->getFragmentType() == NdbDictionary::Object::UserDefined);
  ...
  while (seen_updates < 50) {
    CHK((g_con = g_ndb->startTransaction()) != 0);
    CHK((g_const_opr =
             g_con->readTuple(g_key_record, data.m_key_row, g_blob_record,
                              data.m_read_row, NdbOperation::LM_CommittedRead,
                                       NULL, // mask
                              &opts, sizeof(opts))) != 0);
    CHK((g_bh1 = g_const_opr->getBlobHandle("BL1")) != 0);
    ...
    for (readloop = 0;; readloop++) {
      if (readloop > 0) {
        if (readloop > 1) {
          ...
          */
          if (original_read_char != read_char) seen_updates++;
          original_read_char = read_char;
        }
      }
      if (readloop > loop_max) break;
      Uint32 readSize = 1;
...
static int bugtest_28590428_init(uint inline_size, uint part_size) {
  g_bh1 = g_bh2 = 0;
  g_dic = g_ndb->getDictionary();
  NdbDictionary::Table tab(g_opt.m_tname);
  if (g_dic->getTable(tab.getName()) != 0)
...
static int corrupt_blobs() {
  char blob_table_name[200];
  Uint32 part_id = 0;
  CHK(NdbBlob::getBlobTableName(blob_table_name, g_ndb, g_opt.m_tname, "C") ==
      0);
...
static int read_blob_rows(int style, uint inline_size, uint part_size,
  ...
  for (Uint32 k = startidx; k < 10; k++) {
    ...
    if (style != 0) {
      const bool partialLastPart =
          (total_part_len > (full_parts_read * part_size));
...
static int bugtest_28590428_cleanup() {
  NdbDictionary::Table tab(g_opt.m_tname);
  CHK(g_dic->dropTable(tab.getName()) == 0);
  return 0;
}

static int bugtest_28590428() {
...
static int bugtest_27772916() {
  ...
  const unsigned blob2InlineSize =
      g_opt.m_oneblob ? 0 : tab->getColumn("BL2")->getInlineSize();
  ...
  /* Insert rows */
  {
    ...
    for (unsigned k = 0; k < 10; k++) {
      ...
      if (!g_opt.m_oneblob) CHK(g_bh2->setValue(buff, blobWriteSize) == 0);
    }
   ...
   * 3 : PK update with Blob, Define Blob reading op, executePendingBlobOps() -
   * OK
   */
...
static struct {
  int m_bug;
  int (*m_test)();
} g_bugtest[] = {{4088, bugtest_4088},         {27018, bugtest_27018},
                 {27370, bugtest_27370},       {36756, bugtest_36756},
                 {45768, bugtest_45768},       {48040, bugtest_48040},
                 {28116, bugtest_28116},       {62321, bugtest_62321},
                 {28746560, bugtest_28746560}, {28590428, bugtest_28590428},
...
int main(int argc, char **argv) {
  ...
  while (++argv, --argc > 0) {
	...
	g_opt.m_blob_version = atoi(argv[0]);
        if (g_opt.m_blob_version == 1 || g_opt.m_blob_version == 2) continue;
      }
    }
    // metadata
	...
	g_opt.m_bug = atoi(argv[0]);
        for (unsigned i = 0; i < sizeof(g_bugtest) / sizeof(g_bugtest[0]);
             i++) {
  ...
  if (pk2chr.m_len == 0) {
    char b[100];
    b[0] = 0;
    if (g_opt.m_skip != 0) strcpy(b, g_opt.m_skip);
    strcat(b, "i");
    strcat(b, "r");
