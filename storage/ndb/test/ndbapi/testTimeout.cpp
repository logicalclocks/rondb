...
int setTransactionTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  int timeout = ctx->getProperty("TransactionInactiveTimeout", TIMEOUT);

  NdbConfig conf;
  if (!conf.getProperty(conf.getMasterNodeId(), NODE_TYPE_DB,
                        CFG_DB_TRANSACTION_INACTIVE_TIMEOUT, &g_org_timeout)) {
    return NDBT_FAILED;
  }
...
int resetTransactionTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
...
int setDeadlockTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  int timeout = ctx->getProperty("TransactionDeadlockTimeout", TIMEOUT);
  
  NdbConfig conf;
  if (!conf.getProperty(conf.getMasterNodeId(), NODE_TYPE_DB,
                        CFG_DB_TRANSACTION_DEADLOCK_TIMEOUT, &g_org_deadlock))
    return NDBT_FAILED;
...
int getDeadlockTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  
  Uint32 val = 0;
  NdbConfig conf;
  if (!conf.getProperty(conf.getMasterNodeId(), NODE_TYPE_DB,
                        CFG_DB_TRANSACTION_DEADLOCK_TIMEOUT, &val))
    return NDBT_FAILED;
...
int resetDeadlockTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
...
int runTimeoutTrans2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int l = 0; l < loops && !ctx->isTestStopped() && result == NDBT_OK;
       l++) {
	...
	break;
      ...
      if (op1 != 0 && res != 266) {
        g_err << stepNo << ": Fail: " << res << "!= 237, op1=" << op1
              << ", op2=" << op2 << endl;
        result = NDBT_FAILED;
        break;
      }
...
int runError4012(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  // int loops = ctx->getNumLoops();
  // int stepNo = step->getStepNo();
...
TESTCASE("BuddyTransTimeout1", "Start a scan and check that it gets aborted") {
  INITIALIZER(runLoadTable);
  INITIALIZER(setTransactionTimeout);
