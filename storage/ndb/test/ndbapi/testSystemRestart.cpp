...
int clearOldBackups(NDBT_Context *ctx, NDBT_Step *step) {
  NdbBackup backup;
...
int runSystemRestart3(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  Vector<int> nodeIds;
  for (i = 0; i < nodeCount; i++) nodeIds.push_back(restarter.getDbNodeId(i));
  ...
  while (i <= loops && result != NDBT_FAILED) {
    ...
    CHECK(restarter.restartOneDbNode(nodeIds[currentRestartNodeIndex], false,
                                     true, false) == 0);
...
int runSystemRestartLCP_1(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (++i <= loops && result != NDBT_FAILED) {
    ...
    for (Uint32 start = 0; start < Uint32(remaining_records);
         start += num_records) {
      CHECK(hugoTrans.pkDelRecords(pNdb, num_records, batch, true, 0, start,
                                   row_step) == NDBT_OK);
      if (result == NDBT_FAILED) return result;
      CHECK(hugoTrans.loadTableStartFrom(pNdb, start, num_records, batch, true,
    ...
    Uint32 start = 0;
    row_step = 10;
    CHECK(hugoTrans.pkDelRecords(pNdb, remaining_records, 100, true, 0, start,
                                 row_step) == 0);
    if (result == NDBT_FAILED) return result;
    CHECK(utilTrans.selectCount(pNdb, 64, &count) == 0);
    CHECK(count == 0);
    CHECK(hugoTrans.scanReadRecords(pNdb, 0, 0, 64, NdbOperation::LM_Read, 0,
                                    1) == 0);
  }
  return NDBT_OK;
}
...
int runSystemRestartLCP_2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (++i <= loops && result != NDBT_FAILED) {
...

    {
      ...
      Uint32 row_step = 1;
      CHECK(hugoTrans.pkDelRecords(pNdb, num_deleted_records, 100, true, 0,
                                   start, row_step) == 0);
      if (result == NDBT_FAILED) return result;

      g_err << "Reinsert deleted rows again" << endl;
      NdbSleep_SecSleep(5);
      CHECK(hugoTrans.loadTableStartFrom(pNdb, start, num_deleted_records, 100,
    ...
    CHECK(utilTrans.selectCount(pNdb, 64, &count) == 0);
    CHECK(count == records);
    CHECK(hugoTrans.scanReadRecords(pNdb, records, 0, 64, NdbOperation::LM_Read,
                                    0, 1) == 0);
    if (result == NDBT_FAILED) return result;
  }
  return NDBT_OK;
...
int runSystemRestartLCP_3(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (++i <= loops && result != NDBT_FAILED) {
    ...
    for (Uint32 k = 0; k < 6; k++) {
      ...
      for (Uint32 start = 1; start < 10; start++) {
        CHECK((result = hugoTrans.loadTableStartFrom(
                   pNdb, start, num_deleted_records, 100, true, 0, false, 0,
                   false, false, 10)) == NDBT_OK);
        if (result == NDBT_FAILED) return result;
      }
      CHECK(utilTrans.selectCount(pNdb, 64, &count) == 0);
      CHECK(count == records);
...
int runSystemRestartLCP_4(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (++i <= loops && result != NDBT_FAILED) {
    ...
    CHECK(hugoTrans.pkDelRecords(pNdb, num_rows, 100, true, 0, start, 1) == 0);
    if (result == NDBT_FAILED) return result;
    ...
    g_err << "Loading " << records << " records..." << endl;
    CHECK(hugoTrans.loadTableStartFrom(pNdb, start, num_rows, 100, true, 0,
                                       false, 0, false, false, 1) == NDBT_OK);
    if (result == NDBT_FAILED) return result;
...
int runSystemRestartLCP_5(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (++i <= loops && result != NDBT_FAILED) {
    ...
    CHECK(hugoTrans.loadTableStartFrom(pNdb, start, 2 * num_rows, 100, true, 0,
                                       false, 0, false, false, 1) == NDBT_OK);
    if (result == NDBT_FAILED) return result;
...
int runSystemRestart4(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  Uint32 loops = ctx->getNumLoops();
  int records =
      int((Uint64(1000) * Uint64(ctx->getNumRecords()) / Uint64(1000)));
  int count;
  ...
  while (i <= loops && result != NDBT_FAILED) {
    ...
    if (ctx->getProperty("StopOneNode")) {
      /*** 1 ***/
      g_err << "1 - Stopping one node = " << nodeIds[currentRestartNodeIndex]
            << endl;
      CHECK(restarter.restartOneDbNode(nodeIds[currentRestartNodeIndex], false,
                                       true, false) == 0);
      currentRestartNodeIndex = (currentRestartNodeIndex + 1) % nodeCount;
    ...
    if (ctx->getProperty("StopOneNode")) {
      g_err << "2 - Stopping one node = " << nodeIds[currentRestartNodeIndex]
            << endl;
      CHECK(restarter.restartOneDbNode(nodeIds[currentRestartNodeIndex], false,
                                       true, false) == 0);
      currentRestartNodeIndex = (currentRestartNodeIndex - 1) % nodeCount;
    ...
    for (int part = 0; part < num_parts; part++) {
      ...
      if (row_step == 1) {
        g_err << "Deleting " << remove_records << " records at the end..."
              << endl;
      } else {
        g_err << "Deleting " << remove_records << " records ..." << endl;
      }
      int start = (row_step == 1) ? (remaining_records - remove_records) : part;
      CHECK(hugoTrans.pkDelRecords(pNdb, remove_records, batch, true, 0, start,
                                   row_step) == 0);
      if (result == NDBT_FAILED) return result;
      ...
      if (ctx->getProperty("VerifyDelete")) {
        g_err << "Verifying " << remaining_records << " records remain..."
              << endl;
        if (row_step == 1) {
      ...
      if (ctx->getProperty("StopOneNode")) {
        g_err << "3 - Stopping one node = " << nodeIds[currentRestartNodeIndex]
              << endl;
    ...
    if (ctx->getProperty("StopOneNode")) {
      g_err << "4 - Stopping one node = " << nodeIds[currentRestartNodeIndex]
            << endl;
      CHECK(restarter.restartOneDbNode(nodeIds[currentRestartNodeIndex], false,
                                       true, false) == 0);
      currentRestartNodeIndex = (currentRestartNodeIndex - 1) % nodeCount;
...
int runSystemRestart10(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops && result != NDBT_FAILED) {
    ...
    CHECK(restarter.restartOneDbNode(a_nodeIds[nodeCount - 2], false, true,
                                     false) == 0);
    ...
    if (i == 0) {
      g_info << "Inject Error 1021 into last node to stop it in phase 1"
             << endl;
      CHECK(restarter.insertErrorInNode(nodeId, 1021) == 0);
    } else if (i == 1) {
      g_info << "Inject Error 1010 into last node to stop it in phase 4"
             << endl;
...
int runBug18385(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (node1 == -1 || node2 == -1) return NDBT_OK;
  ...
  do {
    ...
    for (Uint32 i = 0; i < nodeCount; i++)
      if ((nodes[cnt] = restarter.getDbNodeId(i)) != node2) cnt++;
...
int runBug21536(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  const Uint32 nodeCount = restarter.getNumDbNodes();
  if (nodeCount != 2) {
...
int runBug24664(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  NdbRestarter restarter;
  Ndb *pNdb = GETNDB(step);
...
int runBug27434(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  NdbRestarter restarter;
  const Uint32 nodeCount = restarter.getNumDbNodes();

  if (nodeCount < 2) return NDBT_OK;
...
int runOneNodeWithCleanFilesystem(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int i = 0; i < nodeCount; i++) {
    ...
    if (restarter.getNodeGroup(node) == other_ng) nodeIds.push_back(node);
  }
  ...
  do {
    ...
    restarter.startAll();
    // Wait a short while for start phase 0, but cluster might already stopped
    // again
...
int runBug28770(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbRestarter restarter;
  int result = NDBT_OK;
  int count = 0;
...
int runStopper(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  Uint32 stop = 0;
loop:
...
int runSR_DD_1(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  HugoTransactions hugoTrans(*ctx->getTab());
  while (i <= loops && result != NDBT_FAILED) {
    if (i > 0 && ctx->closeToTimeout(30)) break;
    ...
    do {
      ...
      if (hugoTrans.loadTableStartFrom(pNdb, row, 1000) != 0) break;
      row += 1000;
    ...
    CHECK(hugoTrans.clearTable(pNdb, NdbScanOperation::SF_TupScan, cnt) == 0);
...
int runSR_DD_2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i <= loops && result != NDBT_FAILED) {
    ...
    const NDB_TICKS start = NdbTick_getCurrentTicks();
    do {
      if (hugoTrans.loadTable(pNdb, rows) != 0) break;
  ...
  while (i <= loops && result != NDBT_FAILED) {
    ...
    do {
      ...
      for (; cnt < NDB_ARRAY_SIZE(deletedrows); cnt++) {
        ...
        if (hugoTrans.startTransaction(pNdb)) break;
        if (hugoTrans.pkDeleteRecord(pNdb, deletedrows[cnt])) break;
        if (hugoTrans.execute_Commit(pNdb)) break;
        hugoTrans.closeTransaction(pNdb);
      }
      if (hugoTrans.getTransaction() != 0) hugoTrans.closeTransaction(pNdb);

      if (hugoTrans.scanUpdateRecords(pNdb, NdbScanOperation::SF_TupScan, 0) !=
          0)
        break;

      for (Uint32 n = 0; n < cnt; n++) {
        if (hugoTrans.startTransaction(pNdb)) break;
        if (hugoTrans.pkInsertRecord(pNdb, deletedrows[n], 1, rand())) break;
        if (hugoTrans.execute_Commit(pNdb)) break;
        hugoTrans.closeTransaction(pNdb);
      }
      if (hugoTrans.getTransaction() != 0) hugoTrans.closeTransaction(pNdb);
      ...
      if (hugoTrans.scanUpdateRecords(pNdb, NdbScanOperation::SF_TupScan, 0) !=
          ...
          !hugoTrans.getRetryMaxReached())
        break;
    } while (NdbTick_Elapsed(start, NdbTick_getCurrentTicks()).milliSec() <
             13000);
...
/**
 * Test repeated node restarts with holes in the disk data UNDO log.
 *
 * Like runSR_DD_UndoHole, but the error insert crash and restart is
 * repeated back to back several times within each loop. Later cycles
 * restart on an UNDO log that already contains filler pages written by
 * the backward hole scan of the previous cycle's restart, and any hole
 * left unrewritten would sit below the log head where a later restart
 * treats it as UNDO log corruption once records have been applied.
 * This verifies that recovery handles repeated crashes with partially
 * persisted UNDO log writes and that the backward hole scan leaves the
 * UNDO log contiguous on disk.
 */
int runSR_DD_UndoHoleMulti(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  int result = NDBT_OK;
  Uint32 loops = ctx->getNumLoops();
  Uint32 rows = ctx->getNumRecords();
  Uint32 holeCycles = ctx->getProperty("HoleCycles", (unsigned)3);
  NdbRestarter restarter;
  ...
  while (i <= loops && result != NDBT_FAILED) {
    ...
    /**
     * Use the same node in every cycle so that each cycle's restart
     * runs on the UNDO log produced by the previous cycles.
     */
    int nodeId = restarter.getDbNodeId(rand() % restarter.getNumDbNodes());
    int cnt = 0;
    for (Uint32 cycle = 1; cycle <= holeCycles; cycle++) {
      if (result == NDBT_FAILED) break;
      ndbout << "Cycle " << cycle << "/" << holeCycles
             << ": inserting error 15002 (UNDO log hole) in node " << nodeId
             << endl;
      CHECK(restarter.dumpStateOneNode(nodeId, val, 2) == 0);
      CHECK(restarter.insertErrorInNode(nodeId, 15002) == 0);
    ...
    /**
     * Final clean restart to verify that UNDO logging resumed with
     * correct LSNs after the sequence of restarts with holes.
     */
    ndbout << "Clean restart of node " << nodeId << endl;
    CHECK(restarter.restartOneDbNode(nodeId, false, true, true) == 0);
    CHECK(restarter.waitNodesNoStart(&nodeId, 1) == 0);
  ...
  ndbout << "runSR_DD_UndoHoleMulti finished" << endl;
  ctx->stopTest();
  return result;
}

/**
 * Test node restart with a torn page in the disk data UNDO log.
 *
 * Error insert 15004 makes LGMAN corrupt data words in the second half
 * of one page near the end of the next multi-page UNDO log write,
 * after the page checksum has been computed, and crash the node once
 * the write and its fsync have completed (error insert 15003). This
 * simulates a page whose first sectors were persisted but whose tail
 * sectors still hold stale data after a crash in the middle of the
 * page write: the page header including the version stamp and the
 * checksum are intact, so only checksum verification can detect that
 * the page content is not valid.
 *
 * The restart of the node must detect the torn page through checksum
 * verification, treat it as a hole, rewrite it with a filler page in
 * the backward hole scan and execute the UNDO log without failures.
 * The final clean restart in each loop verifies that logging resumed
 * with correct LSNs.
 */
int runSR_DD_UndoTornPage(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  int result = NDBT_OK;
  Uint32 loops = ctx->getNumLoops();
  Uint32 rows = ctx->getNumRecords();
  ...
  while (i <= loops && result != NDBT_FAILED) {
    ...
    int nodeId = restarter.getDbNodeId(rand() % restarter.getNumDbNodes());
    ndbout << "Inserting error 15004 (torn UNDO log page) in node " << nodeId
           << endl;
    CHECK(restarter.dumpStateOneNode(nodeId, val, 2) == 0);
    CHECK(restarter.insertErrorInNode(nodeId, 15004) == 0);

    /**
     * Update disk data rows to produce UNDO log records until the
     * tampered node has seen a multi-page UNDO log write, torn a
     * page in it and crashed itself. Large batches are used to get
     * bursts of UNDO log records so that multi-page writes occur.
     */
    ndbout << "Updating disk data rows until node " << nodeId << " dies"
    ...
    while (!node_died && result != NDBT_FAILED) {
      ...
      if (!node_died && NdbTick_Elapsed(start, now).milliSec() > 600000) {
        ndbout << "Node " << nodeId
               << " did not crash on error insert 15004" << endl;
        result = NDBT_FAILED;
      }
    }
    CHECK(result == NDBT_OK);
    ...
    /**
     * Restart the crashed node. Its LGMAN must detect the torn page
     * through checksum verification, treat it as a hole and execute
     * the UNDO log without failures.
     */
    ndbout << "Starting node " << nodeId << endl;
    CHECK(restarter.startNodes(&nodeId, 1) == 0);
    CHECK(restarter.waitClusterStarted() == 0);
  ...
  ndbout << "runSR_DD_UndoTornPage finished" << endl;
  ctx->stopTest();
  return result;
}
...
int runDropAllTables(NDBT_Context *ctx, NDBT_Step *step) {
  NDBT_Tables::dropAllTables(GETNDB(step));
  return NDBT_OK;
}

int runTO(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  int result = NDBT_OK;
  Uint32 loops = ctx->getNumLoops();
  Uint32 rows = ctx->getNumRecords();
  ...
  Uint32 i = 0;
  while (i <= loops && result != NDBT_FAILED) {
    if (i > 0 && ctx->closeToTimeout(35)) break;
    ...
    while (!notstopped.isclear()) {
      ...
      for (Uint32 j = 0; j < 25; j++) {
        if (!(hugoTrans.scanUpdateRecords(pNdb, 0) == 0)) break;
      }
      while (ndb_logevent_get_next(handle, &event, 0) >= 0 &&
             event.type != NDB_LE_LocalCheckpointCompleted)
        ;
...
int runBug45154(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (createCopy(ctx, step,
                 NdbDictionary::Object::PartitionBalance_ForRAByLDMx4) !=
      NDBT_OK) {
    return NDBT_FAILED;
  }
  ...
  for (int i = 0; i < ctx->getNumLoops(); i++) {
    ...
    */
    ...
    // Insert rows
    {
      HugoTransactions hugoTrans(*actualTable);
      hugoTrans.loadTable(pNdb, rows);
      for (Uint32 pp = 0; pp < 3; pp++) hugoTrans.scanUpdateRecords(pNdb, rows);
    }
...
int runBug46412(NDBT_Context *ctx, NDBT_Step *step) {
  Uint32 partialSR = ctx->getProperty("PartialSRvar");
  Uint32 loops = ctx->getNumLoops();
  NdbRestarter res;
  ...
  for (Uint32 l = 0; l < loops; l++) {
    ...
    Bitmask<256 / 32> ng;
    int cnt = 0;
    int cnt_stop_partial = 0;
    int restartnodes[256];
    int stopnodes_partial[256];
    ...
    for (Uint32 i = 0; i < limit; i++) {
      ...
      if (tmp != NDBT_NO_NODE_GROUP_ID) {
        ...
        if (ng.get(tmp)) {
          ...
          ndbout_c(" COLLISION");
          limit++;
          if (limit > nodeCount) limit = nodeCount;
        } else {
          stopnodes_partial[cnt_stop_partial++] = nodes[i];
          ng.set(tmp);
          ndbout_c(" OK");
        }
      }
    ...
    if (cnt) {
      ...
      for (int i = 0; i < cnt; i++) printf("%d ", restartnodes[i]);
      printf("\n");
      for (int i = 0; i < cnt; i++) {
    ...
    if (partialSR == 0) {
      ...
      ndbout_c("cnt_stop_partial = %u", cnt_stop_partial);
      for (Uint32 i = 0; i < nodeCount; i++) {
        int node = nodes[i];
        bool found = false;
        for (int j = 0; j < cnt_stop_partial; j++) {
          if (stopnodes_partial[j] == node) found = true;
        }
        if (found) {
          ndbout_c("Stop node %u", node);
          res.restartOneDbNode(node, false, true, true, true);
        }
      }
...
int runBug48436(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;
  Uint32 loops = ctx->getNumLoops();
  const Uint32 nodeCount = res.getNumDbNodes();
  if (nodeCount < 2) {
...
int runBug54611(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;
  Uint32 loops = ctx->getNumLoops();
  Ndb *pNdb = GETNDB(step);
...
int runBug56961(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;
  Uint32 loops = ctx->getNumLoops();
  Ndb *pNdb = GETNDB(step);
  int rows = ctx->getNumRecords();
...
int runAddNodes(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (ndb_mgm_create_nodegroup(restarter.handle, newNodes.getBase(), &ng,
                               NULL) != 0) {
    g_err << "ERR: " << step->getName() << " failed on line " << __LINE__
          << endl;
...
int runAlterTableAndOptimize(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  reorganize_table:
    ...
    if (!sql.doQuery(query.c_str(), resultSet)) {
      if (nodesKilledDuringStep && resultSet.mysqlErrno() == 0) {
...
int runRestartOneNode(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  **/
  ...
  while (cnt-- && result != NDBT_FAILED) {
    /*** 2 ***/
    g_info << "2- Restarting node : " << nodeIds[currentRestartNodeIndex]
           << endl;
    ...
    CHECK(restarter.waitNodesNoStart(&nodeIds[currentRestartNodeIndex], 1,
                                     timeout) == 0);
    CHECK(restarter.startNodes(&nodeIds[currentRestartNodeIndex], 1) == 0);
    CHECK(restarter.waitNodesStarted(&nodeIds[currentRestartNodeIndex], 1,
                                     timeout) == 0);
    currentRestartNodeIndex = (currentRestartNodeIndex + 1) % nodeCount;
  }
  ...
  if (hugoTrans.selectCount(pNdb, 64, &count)) return NDBT_FAILED;
  if (hugoTrans.clearTable(pNdb)) return NDBT_FAILED;
...
int runMixedModeRestart(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  **/
  ...
  int otherNodeToKill =
      restarter.getRandomNodeOtherNodeGroup(nodeToKill, rand());
  if (otherNodeToKill == -1) return NDBT_FAILED;

  int kill[] = {9999, 3000, 10000};

  g_info << "    Killing node : " << nodeToKill << endl;
  if (restarter.dumpStateOneNode(nodeToKill, val, 2)) return NDBT_FAILED;
  if (restarter.dumpStateOneNode(nodeToKill, kill, 3)) return NDBT_FAILED;

  g_info << "    Killing node : " << otherNodeToKill << endl;
  if (restarter.dumpStateOneNode(otherNodeToKill, val, 2)) return NDBT_FAILED;
  if (restarter.dumpStateOneNode(otherNodeToKill, kill, 3)) return NDBT_FAILED;
  ...
  if (restarter.restartOneDbNode(nodeToKill,
    ...
    return NDBT_FAILED;
  if (restarter.waitNodesNoStart(&nodeToKill, 1, timeout)) return NDBT_FAILED;
  if (restarter.startNodes(&nodeToKill, 1)) return NDBT_FAILED;
  if (restarter.waitNodesStarted(&nodeToKill, 1, timeout)) return NDBT_FAILED;
  ...
  if (restarter.waitNodesNoStart(&otherNodeToKill, 1, timeout))
    return NDBT_FAILED;
  if (restarter.startNodes(&otherNodeToKill, 1)) return NDBT_FAILED;
...
int runMixedModeRestart4Node(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  **/
  ...
  for (int i = 0; i < nodeCount; i++) {
    ...
    if (nodeGroup == NDBT_NO_NODE_GROUP_ID) continue;
  ...
  for (int i = 0; i < nodeCount; i++) {
    ...
    if (seen_groups.get(nodeGroup)) {
      ...
      g_info << "Breaking because master node belongs to the group whoes one"
             << "node is already down. Master = " << master
             << ", node Group = " << nodeGroup << endl;
      break;
    }
    seen_groups.set(nodeGroup);
    nodesarray[cnt++] = master;
    newMaster = restarter.getNextMasterNodeId(master);
    g_info << "   killing node : " << master << " group : " << nodeGroup
           << endl;
...
int runLoad(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (!ctx->isTestStopped() && ctx->getProperty("CreateLoad", (Uint32)1)) {
    HugoTransactions hugoTrans(tab);
    CHK(hugoTrans.loadTable(pNdb, records) == 0, hugoTrans.getNdbError());
...
int runCheckStaleNodeTakeoverDuringSR(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
...
int runCheckLaggardShutdown(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  ...
  for (Uint32 e = 0; e < 3; e++) {
    ...
    CHK((restarter.restartNodes(NULL, 0, 0) == 0), "System restart failed.");
...
TESTCASE("SR10", "More tests of partitioned system restarts\n") {
  INITIALIZER(runWaitStarted);
  INITIALIZER(runClearTable);
...
TESTCASE("Bug24664", "Check handling of LCP skip/keep") {
  INITIALIZER(runWaitStarted);
  INITIALIZER(runClearTable);
  STEP(runBug24664);
}
...
TESTCASE("SR_DD_UndoHoleMulti",
         "Repeated back to back node restarts with holes in the disk "
         "data UNDO log, verifying that recovery rewrites holes with "
         "filler pages and handles repeated crashes with partially "
         "persisted UNDO log writes") {
  INITIALIZER(runWaitStarted);
  STEP(runSR_DD_UndoHoleMulti);
}
TESTCASE("SR_DD_UndoTornPage",
         "Restart a node with a torn page in the disk data UNDO log, "
         "verifying that checksum verification detects the page and "
         "recovery treats it as a hole") {
  INITIALIZER(runWaitStarted);
  STEP(runSR_DD_UndoTornPage);
}
TESTCASE("Bug29167", "") {
  INITIALIZER(runWaitStarted);
  STEP(runBug29167);
...
TESTCASE("Bug45154", "") { INITIALIZER(runBug45154); }
TESTCASE("Bug46651", "") { INITIALIZER(runBug46651); }
TESTCASE("Bug46412", "") {
  TC_PROPERTY("PartialSRvar", Uint32(0));
  INITIALIZER(runBug46412);
}
TESTCASE("PartialSR", "") {
  TC_PROPERTY("PartialSRvar", Uint32(1));
  INITIALIZER(runBug46412);
}
TESTCASE("Bug48436", "") {
  INITIALIZER(runLoadTable);
