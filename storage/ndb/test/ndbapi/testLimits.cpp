...
#define CHECKEQUAL(v, e)                                                 \
  if ((e) != (v)) {                                                      \
    ndbout << "Error at line " << __LINE__ << " expected " << v << endl; \
    NDB_ERR(trans->getNdbError());                                       \
    trans->close();                                                      \
    return NDBT_FAILED;                                                  \
  }
 ...
 */
int activateErrorInsert(NdbTransaction *trans, const NdbRecord *record,
                        const NdbDictionary::Table *tab, const char *buf,
                        NdbRestarter *restarter, Uint32 val) {
  ...
  CHECKEQUAL(
      0, insert->equal((Uint32)0, NdbDictionary::getValuePtr(record, buf, 0)));
  CHECKEQUAL(0,
             insert->setValue(1, NdbDictionary::getValuePtr(record, buf, 1)));
...
int testSegmentedSectionPk(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (strcmp(ctx->getTab()->getName(), "WIDE_2COL") != 0) return NDBT_OK;
  ...
  Uint32 smallKeySize = setLongVarchar(&smallKey[0], "ShortKey", 8);
   ...
   */
  setLongVarchar(NdbDictionary::getValuePtr(record, smallRowBuf, 0), "ShortKey",
                 8);
  NdbDictionary::setNull(record, smallRowBuf, 0, false);

  setLongVarchar(NdbDictionary::getValuePtr(record, smallRowBuf, 1),
                 "ShortData", 9);
  NdbDictionary::setNull(record, smallRowBuf, 1, false);

  /* Big key buffer with big key and small data*/
  setLongVarchar(NdbDictionary::getValuePtr(record, bigKeyRowBuf, 0),
                 &srcBuff[0], maxKeyBytes);
  NdbDictionary::setNull(record, bigKeyRowBuf, 0, false);

  setLongVarchar(NdbDictionary::getValuePtr(record, bigKeyRowBuf, 1),
                 "ShortData", 9);
  NdbDictionary::setNull(record, bigKeyRowBuf, 1, false);

  /* Big AttrInfo buffer with small key and big data */
  setLongVarchar(NdbDictionary::getValuePtr(record, bigAttrRowBuf, 0),
                 "ShortKey", 8);
  NdbDictionary::setNull(record, bigAttrRowBuf, 0, false);

  setLongVarchar(NdbDictionary::getValuePtr(record, bigAttrRowBuf, 1),
                 &srcBuff[0], maxAttrBytes);
  ...
  NdbTransaction *trans =
      pNdb->startTransaction(ctx->getTab(), &smallKey[0], smallKeySize);
  CHECKTRANS(trans);

  /* Activate error insert 8065 in this transaction, limits
   * any single import/append to 1 section
   */
  CHECKEQUAL(NDBT_OK, activateErrorInsert(trans, record, ctx->getTab(),
                                          smallRowBuf, &restarter, 8065));
  ...
  CHECKTRANS(trans = pNdb->startTransaction(ctx->getTab(), &smallKey[0],
                                            smallKeySize));
  ...
  CHECKEQUAL(
      0, bigInsertOldApi->equal(
             (Uint32)0, NdbDictionary::getValuePtr(record, bigKeyRowBuf, 0)));
  CHECKEQUAL(0, bigInsertOldApi->setValue(
                    1, NdbDictionary::getValuePtr(record, bigKeyRowBuf, 1)));
  ...
  CHECKEQUAL(
      0, bigInsertOldApi->equal(
             (Uint32)0, NdbDictionary::getValuePtr(record, bigAttrRowBuf, 0)));
  CHECKEQUAL(0, bigInsertOldApi->setValue(
                    1, NdbDictionary::getValuePtr(record, bigAttrRowBuf, 1)));
...
int testSegmentedSectionIx(NDBT_Context *ctx, NDBT_Step *step) {
   ...
   */
  /* We just run on one table */
  if (strcmp(ctx->getTab()->getName(), "WIDE_2COL_IX") != 0) return NDBT_OK;
  ...
  const NdbRecord *ixRecord =
      pNdb->getDictionary()
          ->getIndex(indexName, ctx->getTab()->getName())
          ->getDefaultRecord();

  /* Setup buffers
   * Small row buffer with short key and data in base table record format
   */
  setLongVarchar(NdbDictionary::getValuePtr(baseRecord, smallRowBuf, 0),
                 "ShortKey", 8);
  NdbDictionary::setNull(baseRecord, smallRowBuf, 0, false);

  setLongVarchar(NdbDictionary::getValuePtr(baseRecord, smallRowBuf, 1),
                 "ShortData", 9);
  ...
  setLongVarchar(NdbDictionary::getValuePtr(ixRecord, bigKeyIxBuf, 1),
                 &srcBuff[0], maxIndexKeyBytes);
  NdbDictionary::setNull(ixRecord, bigKeyIxBuf, 1, false);

  /* Big AttrInfo buffer
   * Small key and large attrinfo in base table record format */
  setLongVarchar(NdbDictionary::getValuePtr(baseRecord, bigAttrIxBuf, 0),
                 "ShortIXKey", 10);

  NdbDictionary::setNull(baseRecord, bigAttrIxBuf, 0, false);

  setLongVarchar(NdbDictionary::getValuePtr(baseRecord, bigAttrIxBuf, 1),
                 &srcBuff[0], maxIndexKeyBytes);
   ...
   */
  setLongVarchar(NdbDictionary::getValuePtr(baseRecord, bigKeyRowBuf, 0),
                 &srcBuff[0], mediumPrimaryKeyBytes);

  NdbDictionary::setNull(baseRecord, bigKeyRowBuf, 0, false);

  setLongVarchar(NdbDictionary::getValuePtr(baseRecord, bigKeyRowBuf, 1),
                 "ShortIXKey", 10);
   ...
   */
  CHECKNOTNULL(trans->insertTuple(baseRecord, bigKeyRowBuf));

  CHECKEQUAL(0, trans->execute(NdbTransaction::Commit));

  NdbRestarter restarter;
  /* Start a transaction on a specific node */
  trans = pNdb->startTransaction(ctx->getTab(), &smallKey[0], smallKeySize);
  CHECKTRANS(trans);

  /* Activate error insert 8065 in this transaction, limits any
   * single append/import to 10 sections.
   */
  CHECKEQUAL(NDBT_OK, activateErrorInsert(trans, baseRecord, ctx->getTab(),
                                          smallRowBuf, &restarter, 8065));
   ...
   * the same TC.
   */
  const NdbOperation *bigRead =
      trans->readTuple(ixRecord, bigKeyIxBuf, baseRecord, resultSpace);
   ...
   */
  CHECKEQUAL(NDBT_OK, activateErrorInsert(trans, baseRecord, ctx->getTab(),
                                          smallRowBuf, &restarter, 8066));
  CHECKEQUAL(0, trans->execute(NdbTransaction::NoCommit));

  CHECKNOTNULL(bigRead = trans->readTuple(ixRecord, bigAttrIxBuf, baseRecord,
                                          resultSpace));
...
int testSegmentedSectionScan(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (Uint32 w = 0; w < 2500; w++) CHECKEQUAL(0, prog.load_const_null(1));
...
int testWorker(NDBT_Context *ctx, NDBT_Step *step) {
  /* Run as a 'T1' testcase - do nothing for other tables */
  if (strcmp(ctx->getTab()->getName(), "T1") != 0) return NDBT_OK;
      ...
      if (p == 3) {
        switch ((Scenarios)scenario) {
          ...
          case RESTART_MASTER: {
            ...
            ctx->setProperty("DIHWritesRequestType",
                             (Uint32)MASTER_RESTART_REQ);
            ctx->setProperty("DIHWritesRequest", (Uint32)1);
...
int testNdbfsBulkOpen(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  g_err << "Restarting nodes to get rid of error insertion effects" << endl;
...
TESTCASE("SlowDihFileWrites", "Test behaviour of slow Dih table file writes") {
  INITIALIZER(create100Tables);
  STEP(testWorker);
  STEP(testSlowDihFileWrites);
  FINALIZER(drop100Tables);
}
TESTCASE("NdbfsBulkOpen", "Test behaviour of NdbFs bulk file open") {
