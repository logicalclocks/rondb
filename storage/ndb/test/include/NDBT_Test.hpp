...
class NDBT_ParallelStep : public NDBT_Step {
 public:
  NDBT_ParallelStep(NDBT_TestCase *ptest, const char *pname,
                    NDBT_TESTFUNC *pfunc, int num = 0, int count = 1);
...
class NDBT_Initializer : public NDBT_Step {
 public:
  NDBT_Initializer(NDBT_TestCase *ptest, const char *name, NDBT_TESTFUNC *func);
  ~NDBT_Initializer() override {}
};

class NDBT_Finalizer : public NDBT_Step {
 public:
  NDBT_Finalizer(NDBT_TestCase *ptest, const char *name, NDBT_TESTFUNC *func);
  ~NDBT_Finalizer() override {}
};

enum NDBT_DriverType { DummyDriver, NdbApiDriver };
...
class NDBT_TestCaseImpl1 : public NDBT_TestCase {
 public:
  NDBT_TestCaseImpl1(NDBT_TestSuite *psuite, const char *name,
                     const char *comment);
...
class NDBT_TestSuite {
 public:
  ...
  void setCreateAllTables(
      bool);                   // Create all tables before testsuite is executed
  void setRunAllTables(bool);  // Run once with all tables
  void setConnectCluster(
      bool);  // Connect to cluster before testsuite is executed
 ...
 private:
  int executeOne(Ndb_cluster_connection &, const char *_tabname,
                 const char *testname = NULL);
  int executeAll(Ndb_cluster_connection &, const char *testname = NULL);
  void execute(Ndb_cluster_connection &, const NdbDictionary::Table *,
               const char *testname = NULL);
...
#define TESTCASE_DRIVER(type) pt->setDriverType(type);

#define TC_PROPERTY(propname, propval) pt->setProperty(propname, propval);
...
#define NDBT_TESTSUITE_INSTANCE(suitname) C##suitname suitname
