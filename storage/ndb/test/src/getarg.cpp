...
static size_t strlcpy(char *dst, const char *src, size_t dst_sz) {
  size_t n;
  char *p;
  for (p = dst, n = 0; n + 1 < dst_sz && *src != '\0'; ++p, ++src, ++n)
    *p = *src;
  *p = '\0';
  if (*src == '\0')
    return n;
...
#endif
#ifndef HAVE_STRLCAT
static size_t strlcat(char *dst, const char *src, size_t dst_sz) {
...
static size_t print_arg(char *string, size_t len, int mdoc, int longp,
                        struct getargs *arg) {
  const char *s;

  *string = '\0';

  if (ISFLAG(*arg) || (!longp && arg->type == arg_counter)) return 0;

  if (mdoc) {
    if (longp) strlcat(string, "= Ns", len);
...
static void mandoc_template(struct getargs *args, size_t num_args,
  ...
  for (i = 0; i < num_args; i++) {
    ...
    if (ISFLAG(args[i]) || args[i].short_name == 0 ||
        args[i].long_name == NULL) {
      printf(".Op ");

      if (args[i].short_name) {
        print_arg(buf, sizeof(buf), 1, 0, args + i);
        printf("Fl %c%s", args[i].short_name, buf);
        if (args[i].long_name) printf(" | ");
      }
      if (args[i].long_name) {
        print_arg(buf, sizeof(buf), 1, 1, args + i);
        printf("Fl -%s%s%s", args[i].type == arg_negative_flag ? "no-" : "",
  ...
  for (i = 0; i < num_args; i++) {
    ...
    if (args[i].short_name) {
      ...
      print_arg(buf, sizeof(buf), 1, 0, args + i);
      printf("%s", buf);
      if (args[i].long_name) printf(" Ns ,");
      printf("\n");
    }
    if (args[i].long_name) {
      printf(".Fl -%s%s", args[i].type == arg_negative_flag ? "no-" : "",
             args[i].long_name);
      print_arg(buf, sizeof(buf), 1, 1, args + i);
      printf("%s\n", buf);
    }
...
static int check_column(FILE *f, int col, int len, int columns) {
  if (col + len > columns) {
    fprintf(f, "\n");
    col = fprintf(f, "  ");
  }
...
void arg_printusage(struct getargs *args, size_t num_args, const char *progname,
...
#ifdef HAVE___PROGNAME
  if (progname == NULL) progname = __progname;
#endif
  ...
  for (i = 0; i < num_args; ++i) {
    ...
    if (args[i].long_name) {
      ...
      len += print_arg(buf + strlen(buf), sizeof(buf) - strlen(buf), 0, 1,
                       &args[i]);
      strlcat(buf, "]", sizeof(buf));
      if (args[i].type == arg_strings) strlcat(buf, "...", sizeof(buf));
    ...
    if (args[i].short_name) {
      ...
      len += 2;
      len += print_arg(buf + strlen(buf), sizeof(buf) - strlen(buf), 0, 0,
                       &args[i]);
      strlcat(buf, "]", sizeof(buf));
      if (args[i].type == arg_strings) strlcat(buf, "...", sizeof(buf));
      col = check_column(stderr, col, (int)strlen(buf) + 1, columns);
    ...
    if (args[i].long_name && args[i].short_name) len += 2; /* ", " */
  ...
  for (i = 0; i < num_args; ++i) {
    if (args[i].help) {
      ...
      if (args[i].long_name) {
        count += fprintf(stderr, "--");
        if (args[i].type == arg_negative_flag) count += fprintf(stderr, "no-");
        count += fprintf(stderr, "%s", args[i].long_name);
        print_arg(buf, sizeof(buf), 0, 1, &args[i]);
        count += fprintf(stderr, "%s", buf);
      }
      while (count++ <= max_len) putc(' ', stderr);
      fprintf(stderr, "%s\n", args[i].help);
    }
  }
}

static void add_string(getarg_strings *s, const char *value) {
  s->strings = (const char **)realloc(
      s->strings, (s->num_strings + 1) * sizeof(*s->strings));
  s->strings[s->num_strings] = value;
  s->num_strings++;
}

static int arg_match_long(struct getargs *args, size_t num_args, char *argv,
                          int argc, const char **rargv, int *optind) {
  unsigned int i;
  ...
  switch (current->type) {
    case arg_integer: {
      int tmp;
      if (sscanf(optarg + 1, "%d", &tmp) != 1) return ARG_ERR_BAD_ARG;
    ...
    case arg_negative_flag: {
      int *flag = (int *)current->value;
      if (*optarg == '\0' || strcmp(optarg + 1, "yes") == 0 ||
    ...
    case arg_double: {
      double tmp;
      if (sscanf(optarg + 1, "%lf", &tmp) != 1) return ARG_ERR_BAD_ARG;
...
static int arg_match_short(struct getargs *args, size_t num_args, char *argv,
                           int argc, const char **rargv, int *optind) {
  int j, k;
  ...
  for (j = 1; j > 0 && j < (int)strlen(rargv[*optind]); j++) {
    for (k = 0; k < (int)num_args; k++) {
      ...
      if (args[k].short_name == 0) continue;
      if (argv[j] == args[k].short_name) {
        ...
        if (args[k].type == arg_integer) {
          int tmp;
          if (sscanf(optarg, "%d", &tmp) != 1) return ARG_ERR_BAD_ARG;
          ...
          *(const char **)args[k].value = optarg;
          ...
          double tmp;
          if (sscanf(optarg, "%lf", &tmp) != 1) return ARG_ERR_BAD_ARG;
    ...
    if (k == (int)num_args) return ARG_ERR_NO_MATCH;
  }
  return 0;
}
...
int getarg(struct getargs *args, size_t num_args, int argc, const char **argv,
  ...
  for (i = *optind; i < argc; i++) {
    ...
    if (argv[i][1] == '-') {
      ...
      ret = arg_match_long(args, num_args, (char *)argv[i] + 2, argc, argv, &i);
    } else {
      ret = arg_match_short(args, num_args, (char *)argv[i], argc, argv, &i);
    }
    if (ret) break;
  }
  *optind = i;
