...
/* Build the same canonical shape as mb_build_30op_program(), but route all
 * add/SUM operations through overflow-checked stencils and append the hidden
 * OP_OVERFLOW_EXIT target. Input rows generated by mb_generate_rows() are small
 * enough that this variant should never overflow; it measures normal-path
 * checked-stencil overhead. */
