...
RS_Status find_project_id_int(Ndb *ndb_object, const char *feature_store_name, Uint32 *project_id) {
  NdbError err;
  ...
  char index_name[] = "projectname";
  status            = get_index_scan_op(ndb_object, tx, table_dict, index_name, &scanOp);
  if (status.http_code != SUCCESS) {
  ...
  if (filter.begin(NdbScanFilter::AND) < 0 ||
      filter.cmp(NdbScanFilter::COND_EQ, col_id, feature_store_name, API_KEY_PREFIX_SIZE) < 0 ||
      filter.end() < 0) {
...
RS_Status find_project_id(const char *feature_store_name, Uint32 *project_id) {
