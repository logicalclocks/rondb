    ...
    switch (val) {
      ...
      case '-i':
        ...
        for (m = 0; m < options.iterations.length; ++m) {
          n = options.iterations[m] if (isNaN(n)) {
...
Timer.prototype.start = function(mode, operation, numberOfIterations) {
  // console.log('lib.Timer.start', mode, operation, 'iterations:',
  // numberOfIterations);
  this.mode = mode;
...
Timer.prototype.stop = function() {
  function pad(str, count, onRight) {
    while (str.length < count) str = (onRight ? str + ' ' : ' ' + str);
    return str;
  }
  ...
  console.log(
      rpad(18, this.mode + ' ' + this.operation),
...
function verifyObject(that) {
  for (var prop in this) {
    ...
    if (this.hasOwnProperty(prop)) {
      ...
      if (this[prop] != that[prop])
        appendError(
            'Error: data mismatch for property ' + this.constructor.name + '.' +
            prop + ' expected: ' + JSON.stringify(this[prop]) +
...
function main() {
  ...
  try {
    ...
    if (e.message.indexOf('Cannot find module') === -1) {
      console.log(e);
      console.log(
          e.name, 'reading jscrund.config:', e.message,
  ...
  if (typeof JSCRUND.implementation.getConnectionProperties === 'function') {
    ...
    properties = new JSCRUND.mynode.ConnectionProperties(
        options.adapter, options.deployment);
  }
  ...
  var runTests = function(options) {
    ...
    if (options.tests) {  // Explicit test names
      ...
      switch (options.doClass) {
        case 'B':
          testNames =
              ['persist', 'setVarchar', 'find', 'clearVarchar', 'remove'];
          break;
        case 'A':
        default:
    ...
    /**
     * Recursively call the operation numberOfIterations times in autocommit
     * mode and then call the operationsDoneCallback
     */
    var indyOperationsLoop = function(err) {
      if (DETAIL)
        JSCRUND.udebug.log_detail(
            'jscrund.indyOperationsLoop iteration:', iteration, 'err:', err);
      // check result
      if (err) {
        appendError(err);
      }
      ...
      if (iteration < numberOfIterations) {
        operation.apply(
            JSCRUND.implementation,
            [parameters[iteration], indyOperationsLoop]);
        iteration++;
      } else {
        if (DETAIL)
          JSCRUND.udebug.log_detail(
              'jscrund.indyOperationsLoop iteration:', iteration, 'complete.');
        timer.stop();
        resultStats.push(
            {name: testName + "," + modeName, time: timer.interval});
    ...
    var indyTestsLoop = function() {
      ...
      if (testNumber < testNames.length) {
        ...
        if (DETAIL)
          JSCRUND.udebug.log_detail(
              'jscrund.indyTestsLoop', testNumber, 'of', testNames.length, ':',
              testName);
        iteration = 0;
    ...
    var eachCommitDoneCallback = function(err) {
      ...
      timer.stop();
      resultStats.push({name: testName + "," + modeName, time: timer.interval});
      setImmediate(operationsDoneCallback);
    };
    ...
    var eachOperationsLoop = function(err) {
      if (DETAIL)
        JSCRUND.udebug.log_detail(
            'jscrund.eachOperationsLoop iteration:', iteration, 'err:', err);
      // check result
      if (err) {
        appendError(err);
      }
      ...
      if (iteration < numberOfIterations) {
        operation.apply(
            JSCRUND.implementation,
            [parameters[iteration], eachOperationsLoop]);
        iteration++;
      } else {
        if (DETAIL)
          JSCRUND.udebug.log_detail(
              'jscrund.eachOperationLoop iteration:', iteration, 'complete.');
    ...
    var eachTestsLoop = function() {
      ...
      if (testNumber < testNames.length) {
        ...
        if (DETAIL)
          JSCRUND.udebug.log_detail(
              'jscrund.eachTestsLoop', testNumber, 'of', testNames.length, ':',
              testName);
        iteration = 0;
        timer.start(modeName, testName, numberOfIterations);
    ...
    var bulkTestsLoop = function() {
      ...
      if (testNumber < testNames.length) {
        ...
        if (DETAIL)
          JSCRUND.udebug.log_detail(
              'jscrund.bulkTestsLoop', testNumber, 'of', testNames.length, ':',
              testName);
        timer.start(modeName, testName, numberOfIterations);
        JSCRUND.implementation.createBatch(function(err) {
          for (iteration = 0; iteration < numberOfIterations; ++iteration) {
            operation.apply(
                JSCRUND.implementation,
                [parameters[iteration], bulkCheckOperationCallback]);
          }
  ...
  // create database
  JSCRUND.metadataManager =
      require("jones-ndb").getDBMetadataManager(properties);

  JSCRUND.metadataManager.runSQL(
      path.join(__dirname, "./create.sql"), function(err) {
        if (err) {
