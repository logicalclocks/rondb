...
import testsuite.clusterj.model.VarcharPK;

public class VarcharPKTest extends AbstractClusterJTest {

    protected int NUMBER_OF_INSTANCES = 15;
    protected List<VarcharPK> instances = new ArrayList<VarcharPK>();
    ...
    public void localSetUp() {
        ...
        try {
            tx.begin();
            session.deletePersistentAll(VarcharPK.class);
            tx.commit();
        } catch (Throwable t) {
            // ignore errors while deleting
        }
        createInstances();
        addTearDownClasses(VarcharPK.class);
    }
    ...
    protected void find() {
        for (int i = 0; i < NUMBER_OF_INSTANCES; ++i) {
            String key = getPK(i);
            VarcharPK result = session.find(VarcharPK.class, key);
    ...
    protected void update() {
        ...
        for (int i = 0; i < NUMBER_OF_INSTANCES; ++i) {
            if (0 == i % 4) {
                VarcharPK instance = createInstance(i);
        ...
        for (int i = 0; i < NUMBER_OF_INSTANCES; ++i) {
            if (0 == i % 4) {
                String key = getPK(i);
                VarcharPK instance = session.find(VarcharPK.class, key);
                verify(instance, i, true);
            }
        }
    }
    ...
    /** Create an instance of VarcharPK.
     * @param index the index to use to generate data
     * @return the instance
     */
    protected VarcharPK createInstance(int index) {
        VarcharPK instance = session.newInstance(VarcharPK.class);
    ...
    protected String toString(VarcharPK instance) {
        StringBuffer result = new StringBuffer();
        result.append("VarcharPK[");
    ...
    protected void verify(VarcharPK instance, int index, boolean updated) {
