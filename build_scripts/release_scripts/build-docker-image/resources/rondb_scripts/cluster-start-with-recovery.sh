...
if [ "X$USERID" = "Xroot" ]; then
    echo ""
    echo "You are starting cluster as user: 'root'."
...
if [ $? -ne 0 ] ; then
  ...
  echo "/srv/hops/mysql-cluster/mgmd"
  exit 1
fi
...
if [ $SKIP_NDBDS -eq 0 ] ; then
  ...
  ssh mysql@10.0.0.107 /srv/hops/mysql-cluster/ndb/scripts/ndbd-start.sh
...
if [ $SKIP_MYSQLDS -eq 0 ] ; then
    ...
    echo "For username: mysql "
  ssh mysql@10.0.0.79 "/srv/hops/mysql-cluster/ndb/scripts/mysql-server-start.sh $SKIP_USER_CHECK"
...
# /srv/hops/mysql-cluster/ndb/scripts/mysql-server-start.sh $SKIP_MYSQLDS
