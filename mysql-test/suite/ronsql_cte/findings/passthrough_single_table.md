...
plain NDB API path.  Cases st-1..24 lock in the supported envelope;
...
| WHERE IN-subquery on a single-table projection | `SELECT o_orderkey FROM orders WHERE o_custkey IN (SELECT ...);` | **GREEN — pinned coverage (st-23)**: the subquery substitution machinery (which predates shape C) works unchanged in a non-aggregate outer query; 60-row strict compare ×5 topologies | body_passthrough_single_table.inc |
| Scalar subquery constant on a single-table projection | `SELECT o_orderkey, o_custkey FROM orders WHERE o_orderkey = (SELECT MAX(...));` | **GREEN — pinned coverage (st-24)**: the substituted literal reaches the shape-C PK-lookup detection; strict compare ×5 topologies | body_passthrough_single_table.inc |
