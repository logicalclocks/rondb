...
|-------|---------------------|-------------|----------------------------|----------|
| IS NOT NULL in a **CTE body** WHERE (o_clerk) | `WHERE o_clerk IS NOT NULL` inside the CTE materialization | NEXT-PHASE-disabled | CTE-body single-table scan emits WHERE via `NdbScanFilter::apply_filter` (RonSQLPreparer.cpp:5934), whose switch has no `case T_IS:` → ERROR "Non-boolean term in WHERE condition" (RonSQLPreparer.cpp:8507). IS NULL/IS NOT NULL ARE supported on the main-query CTE_LOOKUP output (Phase I.1 `branch_linked_isnull`), just not in the CTE body. Was filter-10; D10 | body_filter.inc Group 3 (NEXT-PHASE) |
| IS NULL in a **CTE body** WHERE (o_clerk) | `WHERE o_clerk IS NULL` inside the CTE materialization | NEXT-PHASE-disabled | **This is the confirmed "Non-boolean term in WHERE condition" case** observed at record time. Same `apply_filter` T_IS gap as above. Was filter-11; D10 | body_filter.inc Group 3 (NEXT-PHASE) |
| GREATEST in a CTE-body WHERE | `WHERE GREATEST(o_shippriority, 1) > 2` | NEXT-PHASE-disabled | Hard parser "Syntax error: Unexpected at this point" — RonSQL grammar permits GREATEST/LEAST only in the top-level SELECT scalar position (RonSQLParser.y rules 421/448), not inside a WHERE term. Was filter-14; D11 | body_filter.inc Group 5 (NEXT-PHASE) |
| LEAST(col, col) in a CTE-body WHERE | `WHERE LEAST(l_quantity, l_partkey) < 10` | NEXT-PHASE-disabled | Same grammar gate as GREATEST — Syntax error. Was filter-15; D11 | body_filter.inc Group 5 (NEXT-PHASE) |
